...
jQuery(function( $ ) {
	...
	$( '#wpbody' ).on( 'click', '#doaction, #doaction2', function() {
		...
		var tax = 'product_tag';
		$( 'tr.inline-editor textarea[name="tax_input[' + tax + ']"]' ).autosuggest( ajaxurl + ( ajaxurl.indexOf( '?' ) > 0 ? '&' : '?' ) + 'action=ajax-tag-search&tax=' + tax, { delay: 500, minchars: 2, multiple: true, multipleSep: inlineEditL10n.comma } );
	});
