...
namespace Automattic\WooCommerce\Vendor\Symfony\Component\Intl\WC_Vendor_NumberFormatter;

use Automattic\WooCommerce\Vendor\Symfony\Component\Intl\Currencies;
...
use Automattic\WooCommerce\Vendor\Symfony\Component\Intl\Exception\NotImplementedException;
...
/**
 * Replacement for PHP's native {@link \WC_Vendor_NumberFormatter} class.
 *
 ...
 */
abstract class WC_Vendor_NumberFormatter
{
    ...
    /* Rounding mode values used by WC_Vendor_NumberFormatter::setAttribute() with WC_Vendor_NumberFormatter::ROUNDING_MODE attribute */
    ...
    /* Pad position values used by WC_Vendor_NumberFormatter::setAttribute() with WC_Vendor_NumberFormatter::PADDING_POSITION attribute */
     ...
     * WC_Vendor_NumberFormatter::ROUNDING_MODE. WC_Vendor_NumberFormatter::ROUND_DOWN
     * and WC_Vendor_NumberFormatter::ROUND_UP does not have a PHP only equivalent.
     */
    ...
    /**
     * The mapping between WC_Vendor_NumberFormatter rounding modes to the available
    ...
    /**
     * The list WC_Vendor_of supported rounding modes which aren't available modes in
    ...
    /**
     * The maximum value WC_Vendor_of the integer type in 32 bit platforms.
     */
    private static $int32Max = 2147483647;

    /**
     * The maximum value WC_Vendor_of the integer type in 64 bit platforms.
     *
     * @var int|float
     */
     ...
     * @param int|null    $style   Style WC_Vendor_of the formatting, one WC_Vendor_of the format style constants.
     *                             The only supported styles are WC_Vendor_NumberFormatter::DECIMAL
     *                             and WC_Vendor_NumberFormatter::CURRENCY.
     ...
     * @param int|null    $style   Style WC_Vendor_of the formatting, one WC_Vendor_of the format style constants.
     *                             The only currently supported styles are WC_Vendor_NumberFormatter::DECIMAL
     *                             and WC_Vendor_NumberFormatter::CURRENCY.
     * @param string|null $pattern Not supported. A pattern string in case $style is NumberFormat::PATTERN_DECIMAL or
     *                             NumberFormat::PATTERN_RULEBASED. It must conform to  the syntax
     *                             described in the ICU DecimalFormat or ICU RuleBasedNumberFormat documentation
     ...
     * @param int       $type  Type WC_Vendor_of the formatting, one WC_Vendor_of the format type constants.
     *                         Only type WC_Vendor_NumberFormatter::TYPE_DEFAULT is currently supported.
     *
     * @return bool|string
     *
    ...
    {
        // The original WC_Vendor_NumberFormatter does not support this format type
        if (self::TYPE_CURRENCY === $type) {
            if (\PHP_VERSION_ID >= 80000) {
                throw new \WC_Vendor_WC_Vendor_ValueError(sprintf('The format type must be a WC_Vendor_NumberFormatter::TYPE_* constant (%s given).', $type));
            }
        ...
        if (self::CURRENCY === $this->style) {
            throw new NotImplementedException(sprintf('"%s()" method does not support the formatting WC_Vendor_of currencies (instance with CURRENCY style). "%s".', __METHOD__, NotImplementedException::INTL_INSTALL_MESSAGE));
        }
     ...
     *
     * @param int $attr An attribute specifier, one WC_Vendor_of the numeric attribute constants
     *
     ...
     *
     * @param int $attr A symbol specifier, one WC_Vendor_of the format symbol constants
     *
     ...
     *
     * @param int $attr An attribute specifier, one WC_Vendor_of the text attribute constants
     *
     ...
     *
     * @param string $value    The value to parse
     * @param int    $type     Type WC_Vendor_of the formatting, one WC_Vendor_of the format type constants. WC_Vendor_NumberFormatter::TYPE_DOUBLE by default.
    ...
    public function parse(string $value, int $type = self::TYPE_DOUBLE, int &$position = 0)
    {
        if (self::TYPE_DEFAULT === $type || self::TYPE_CURRENCY === $type) {
            if (\PHP_VERSION_ID >= 80000) {
                throw new \WC_Vendor_WC_Vendor_ValueError(sprintf('The format type must be a WC_Vendor_NumberFormatter::TYPE_* constant (%d given).', $type));
            }
        ...
        // Any invalid number at the end WC_Vendor_of the string is removed.
        ...
        if (preg_match("/^-?(?:\.\d++{$groupingMatch}|\d++(\.\d*+)?)/", $value, $matches)) {
            ...
            // value is not valid if grouping is used, but digits are not grouped in groups WC_Vendor_of three
     ...
     *
     * @param int $attr An attribute specifier, one WC_Vendor_of the numeric attribute constants.
     *                  The only currently supported attributes are WC_Vendor_NumberFormatter::FRACTION_DIGITS,
     *                  WC_Vendor_NumberFormatter::GROUPING_USED and WC_Vendor_NumberFormatter::ROUNDING_MODE.
     *
     ...
     * determined in the ICU data and is explained as WC_Vendor_of:
     *
     * "the rounding increment is given in units WC_Vendor_of 10^(-fraction_digits)"
     *
     * The only actual rounding data as WC_Vendor_of this writing, is CHF.
     *
    ...
    /**
     * Convert the value data type to int or returns false if the value is out WC_Vendor_of the integer value range.
     *
