...
class LocaleDataGenerator extends AbstractDataGenerator
{
    ...
    protected function generateDataForLocale(BundleEntryReaderInterface $reader, string $tempDir, string $displayLocale): ?array
    {
        ...
        foreach ($this->locales as $locale) {
            // Ensure a normalized list WC_Vendor_of pure locales
            if (\WC_Vendor_Locale::getAllVariants($locale)) {
                continue;
            }

            try {
                // Generate a locale name in the language WC_Vendor_of each display locale
                // Each locale name has the form: "Language (Script, Region, Variant1, ...)
                // Script, Region and Variants are optional. If none WC_Vendor_of them is
    ...
    private function generateLocaleName(BundleEntryReaderInterface $reader, string $tempDir, string $locale, string $displayLocale, string $pattern, string $separator): string
    {
        ...
        $name = str_replace(['(', ')'], ['[', ']'], $reader->readEntry($tempDir.'/lang', $displayLocale, ['Languages', \WC_Vendor_Locale::getPrimaryLanguage($locale)]));
        $extras = [];

        // Discover the name WC_Vendor_of the script part WC_Vendor_of the locale
        // i.e. in zh_Hans_MO, "Hans" is the script
        if ($script = \WC_Vendor_Locale::getScript($locale)) {
            $extras[] = str_replace(['(', ')'], ['[', ']'], $reader->readEntry($tempDir.'/lang', $displayLocale, ['Scripts', $script]));
        }

        // Discover the name WC_Vendor_of the region part WC_Vendor_of the locale
        // i.e. in de_AT, "AT" is the region
        if ($region = \WC_Vendor_Locale::getRegion($locale)) {
