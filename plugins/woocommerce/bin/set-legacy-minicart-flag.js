 ...
 *
 * This mirrors the CI step in .github/workflows/legacy-minicart-e2e-tests.yml
 * but is implemented in Node so it can be reused from package.json scripts.
 */
...
function main() {
  ...
  const coreJson = path.join(pluginRoot, 'client', 'admin', 'config', 'core.json');
  const devJson = path.join(pluginRoot, 'client', 'admin', 'config', 'development.json');
  ...
  files.forEach((file) => {
    ...
    console.log(
      `Set ${flagKey} to ${json.features?.[flagKey]} in ${path.relative(process.cwd(), file)}`
    );
  });
}
