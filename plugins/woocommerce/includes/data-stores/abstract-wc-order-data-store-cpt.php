	...
	*/
	...
	protected function get_post_status( $order ) {
		...
		if ( ! $order_status ) {
			 ...
			 * @since 3.7.0
			 */
			$order_status = apply_filters( 'woocommerce_default_order_status', WC_Order::STATUS_PENDING );
		}
		...
		if ( ! in_array( $post_status, array( WC_Order::STATUS_AUTO_DRAFT, WC_Order::STATUS_DRAFT, WC_Order::STATUS_TRASH ), true ) && in_array( 'wc-' . $post_status, $valid_statuses, true ) ) {
