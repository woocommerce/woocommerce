...
class WC_Gateway_Pay_At_Location extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		// Make this method support "optional-billing".
		$this->supports[] = 'optional-billing';
	...
	protected function setup_properties() {
		$this->id = self::ID;
		/**
		 * Override the icon for the Pay At Location gateway.
		 *
		 * @since 10.4
		 */
	...
	public function init_form_fields() {
		$this->form_fields = array(
			'enabled'            => array(
				...
				'label'       => __( 'Enable pay at location', 'woocommerce' ),
				'type'        => 'checkbox',
				'description' => '',
				'default'     => 'no',
			),
			'title'              => array(
				...
				'default'     => __( 'Pay at location', 'woocommerce' ),
				'desc_tip'    => true,
			),
			'description'        => array(
			...
			'enable_for_methods' => array(
				...
				'css'               => 'width: 400px;',
				'default'           => '',
				'description'       => __( 'If Pay at Location is only available for certain methods, set it up here. Leave blank to enable for all methods that support local pickup.', 'woocommerce' ),
			...
			'enable_for_virtual' => array(
				...
				'label'   => __( 'Accept Pay at Location if the order is virtual', 'woocommerce' ),
				'type'    => 'checkbox',
				'default' => 'yes',
			),
		);
	 ...
	 *
	 * @return string[] An array of supported features.
	 */
	public function get_supported_features() {
		return array( 'products', 'optional-billing' );
	}
	...
	public function process_payment( $order_id ) {
		...
		if ( $order->get_total() > 0 ) {
			/**
			 * Filter the order status for Pay at Location orders.
			 *
			 * @since 2.6.0
			 *
	...
	/**
	 * Change payment complete order status to completed for Pay at Location orders.
	 *
	 * @since  3.1.0
