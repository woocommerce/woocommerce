...
class WC_Product_CSV_Importer extends WC_Product_Importer {
	...
	protected function expand_data( $data ) {
		...
		if ( isset( $data['images'] ) ) {
			...
			// still leaves existing images untouched. Gating on the gallery values too
			// keeps them imported when the featured-image slot is empty (e.g. a cell
			// starting with a separator). See https://github.com/woocommerce/woocommerce/issues/34839
			// and https://github.com/woocommerce/woocommerce/issues/66583.
			if ( ! empty( $data['raw_image_id'] ) || ! empty( $images ) ) {
