...
class WC_Session_Handler extends WC_Session {
	 ...
	 *
	 * If the current user is logged in, the session will replace the current user's session.
	 * If the current user is logged out, the session is resumed.
	 *
	 * Only guest sessions are resumed, hence the check for the t_ prefix on the customer ID.
	 *
	 * @return bool
	 */
