		...
		try {
			...
			$webhook_data = apply_filters(
				...
				array(
					...
					'post_title'    => ! empty( $data['name'] ) ? $data['name'] : sprintf( __( 'Webhook created on %s', 'woocommerce' ), strftime( _x( '%1$b %2$d, %Y @ %I:%M %p', 'Webhook created on date parsed by strftime', 'woocommerce' ) ) ),
				),
				$data,
				$this
			);
