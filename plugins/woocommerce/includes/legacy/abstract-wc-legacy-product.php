...
abstract class WC_Abstract_Legacy_Product extends WC_Data {
	 ...
	 * @return string|false|WP_Error Category list HTML on success. False when the product has no categories, and also when an ordering mode is selected and a `term_links-product_cat` callback returns a non-array; the default path passes that value to implode() the way core does. WP_Error if the terms or a term link cannot be resolved.
	 */
