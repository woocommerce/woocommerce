...
if ( ! class_exists( 'WC_Email_Low_Stock', false ) ) :

	/**
	 * Low Stock Email.
	 *
	 * An email sent to the admin when a product reaches the low stock threshold.
	 *
	 * @class   WC_Email_Low_Stock
	 * @version 11.2.0
	 * @package WooCommerce\Classes\Emails
	 */
	class WC_Email_Low_Stock extends WC_Email {
		...
		public function __construct() {
			$this->id             = 'low_stock';
			$this->title          = __( 'Low stock', 'woocommerce' );
			$this->email_group    = 'inventory';
			$this->template_html  = 'emails/low-stock.php';
			$this->template_plain = 'emails/plain/low-stock.php';
			...
			add_action( 'woocommerce_low_stock_notification', array( $this, 'trigger' ), 10, 1 );

			// Call parent constructor.
			parent::__construct();

			// Must be after parent's constructor which sets `email_improvements_enabled` and `block_email_editor_enabled` properties.
			$this->description = __( 'Low stock emails are sent to chosen recipient(s) when a product reaches the low stock threshold.', 'woocommerce' );

			if ( $this->block_email_editor_enabled ) {
				$this->description = __( 'Notifies admins when a product is running low on stock.', 'woocommerce' );
			}
		...
		public function append_threshold_link( $description, $email ) {
			...
			return $description . ' <a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=products&section=inventory#woocommerce_notify_low_stock_amount' ) ) . '">' . esc_html__( 'Manage threshold', 'woocommerce' ) . '</a>';
		}
		...
		public function get_default_subject() {
			return __( '[{site_title}] Product low in stock', 'woocommerce' );
		}
		...
		public function get_default_heading() {
			return __( 'Product low in stock', 'woocommerce' );
		}
		 ...
		 *
		 * @since 11.2.0
		 * @param WC_Product $product Product that reached the low stock threshold.
		 * @return void
		 */
		public function trigger( $product ) {
			 ...
			 *
			 * @param bool $send       Whether the low stock notification should be sent.
			...
			$this->placeholders['{stock_quantity}'] = (string) $product->get_stock_quantity();
			 ...
			 */
			$this->message = apply_filters( 'woocommerce_email_content_low_stock', $message, $product );
		...
		public function init_form_fields(): void {
			...
			$this->form_fields = array(
				'enabled'            => array(
					...
					'default' => get_option( 'woocommerce_notify_low_stock', 'yes' ),
				),
				'recipient'          => array(
