...
function wc_add_to_cart_message( $products, $show_qty = false, $return = false ) {
	...
	if ( 'yes' === get_option( 'woocommerce_cart_redirect_after_add' ) ) {
		...
		$message   = sprintf( '<div class="woocommerce-notice-content">%s <a href="%s" class="button wc-forward%s">%s</a></div>', esc_html( $added_text ), esc_url( $return_to ), esc_attr( $wp_button_class ), esc_html__( 'Continue shopping', 'woocommerce' ) );
	} else {
		$message = sprintf( '<div class="woocommerce-notice-content">%s <a href="%s" class="button wc-forward%s">%s</a></div>', esc_html( $added_text ), esc_url( wc_get_cart_url() ), esc_attr( $wp_button_class ), esc_html__( 'View cart', 'woocommerce' ) );
	}
