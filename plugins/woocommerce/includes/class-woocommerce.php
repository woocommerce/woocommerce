...
final class WooCommerce {
	...
	public function maybe_halt_cron_activity_for_this_request(): void {
		...
		if ( '' !== $request_uri && has_action( 'init', 'wp_cron' ) ) {
			...
			$rest_route   = (string) ( $_GET['rest_route'] ?? null ); // phpcs:ignore WordPress.Security
			$serving_rest = '' !== $rest_route && preg_match( '#^/(wc|wc-telemetry|wc-admin|wc-analytics)/#', $rest_route );
