...
class WC_AJAX {
	...
	private static function maybe_add_order_item( $order_id, $items, $items_to_add ) {
		try {
			...
			$order->add_order_note( sprintf( __( 'Added line items: %s', 'woocommerce' ), implode( ', ', $order_notes ) ), false, true, array( 'note_type' => OrderNoteType::ORDER_UPDATE ) );
	...
	public static function remove_order_coupon() {
		...
		try {
			...
			if ( $order->remove_coupon( $code ) ) {
				...
				$order->add_order_note( esc_html( sprintf( __( 'Coupon removed: "%s".', 'woocommerce' ), $code ) ), 0, true, array( 'note_type' => OrderNoteType::ORDER_UPDATE ) );
			}
			$order->calculate_taxes( $calculate_tax_args );
			$order->calculate_totals( false );
				...
				foreach ( $order_item_ids as $item_id ) {
					...
					if ( $item->is_type( 'line_item' ) ) {
						...
						if ( $changed_stock && ! is_wp_error( $changed_stock ) ) {
							...
							$order->add_order_note( sprintf( __( 'Deleted %1$s and adjusted stock (%2$s)', 'woocommerce' ), $item->get_name(), $changed_stock['from'] . '&rarr;' . $changed_stock['to'] ), false, true, array( 'note_type' => OrderNoteType::ORDER_UPDATE ) );
						} else {
							/* translators: %s item name. */
							$order->add_order_note( sprintf( __( 'Deleted %s', 'woocommerce' ), $item->get_name() ), false, true, array( 'note_type' => OrderNoteType::ORDER_UPDATE ) );
						}
					}
