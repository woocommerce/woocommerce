...
/**
 * Reconcile a product's displayed/charged price with its sale schedule at read time.
 *
 ...
 * so display and cart/checkout use the date-correct price without waiting for the AS
 * event. It is display-layer only: `get_prop()` applies this hook solely in 'view'
 * context, and the filter writes nothing, so admin/edit/CRUD/save flows — including the
 * scheduled-sale AS handlers, which read 'edit' context — and the stored `_price` meta
 * are untouched.
 *
 ...
 */
function wc_filter_scheduled_sale_active_price( $price, $product ) {
...
add_filter( 'woocommerce_product_get_price', 'wc_filter_scheduled_sale_active_price', 99, 2 );
