 ...
 * @return bool
 */
function wc_product_has_unique_id( $product_id, $unique_id ) {
	/**
	 * Gives plugins an opportunity to verify Unique ID uniqueness themselves.
	 *
	 * @since 9.1.0
	 *
	 * @param bool|null $has_unique_id Set to a boolean value to short-circuit the default Unique ID check.
	 * @param int $product_id The ID of the current product.
	 * @param string $sku The Unique ID to check for uniqueness.
	 */
	$has_unique_id = apply_filters( 'wc_product_pre_has_unique_id', null, $product_id, $unique_id );
	if ( ! is_null( $has_unique_id ) ) {
		return boolval( $has_unique_id );
	}

	$data_store      = WC_Data_Store::load( 'product' );
	$unique_id_found = $data_store->is_existing_unique_id( $product_id, $unique_id );
	/**
	 * Gives plugins an opportunity to verify Unique ID uniqueness themselves.
	 *
	 * @since 9.1.0
	 *
	 * @param boolean $unique_id_found Whether the Unique ID is found.
	 * @param int $product_id The ID of the current product.
	 * @param string $sku The Unique ID to check for uniqueness.
	 */
	if ( apply_filters( 'wc_product_has_unique_id', $unique_id_found, $product_id, $unique_id ) ) {
		return false;
	}
 ...
 * @return int
 */
function wc_get_product_id_by_unique_id( $unique_id ) {
	$data_store = WC_Data_Store::load( 'product' );
	return $data_store->get_product_id_by_unique_id( $unique_id );
}
