...
use Automattic\WooCommerce\Internal\DataStores\Orders\HposOrderDateRepairs;
...
/**
 * Give HPOS orders migrated without a created or updated date the dates their posts still hold. Batched, returns true
 * while rows remain.
 *
 * @return bool True to run again.
 */
function wc_update_1130_repair_hpos_order_dates_from_posts() {
	return wc_get_container()->get( HposOrderDateRepairs::class )->repair_dates_from_posts();
}

/**
 * Restore the paid and completed dates of HPOS orders migrated from posts that only carried the pre-3.0
 * '_paid_date' / '_completed_date' meta keys. Batched, returns true while rows remain.
 *
 * @since 11.3.0
 *
 * @return bool True to run again.
 */
function wc_update_1130_restore_hpos_legacy_paid_and_completed_dates() {
	return wc_get_container()->get( HposOrderDateRepairs::class )->restore_legacy_paid_and_completed_dates();
}
