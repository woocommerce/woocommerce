 ...
 * WC_Admin_POS_Staff.
 *
 * Server-side admin UI for assigning PINs to users with POS access. Gated behind
 * the `point_of_sale_staff` dev feature flag (which also requires the parent
 * `point_of_sale` flag); when either is off, the class is inert.
 *
 * @since 10.9.0
 */
class WC_Admin_POS_Staff {
	...
	public static function page_output(): void {
		...
		if ( isset( $_GET['edit-staff'] ) ) {
			...
			if ( $user_id && ! current_user_can( 'manage_pos_staff' ) ) {
	...
	private function save(): void {
		...
		if ( ! current_user_can( 'manage_pos_staff' ) ) {
			wp_die( esc_html__( 'You do not have permission to manage POS staff.', 'woocommerce' ) );
		}
