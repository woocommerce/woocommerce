...
function wc_render_action_buttons( $actions ) {
	...
	foreach ( $actions as $action ) {
		if ( isset( $action['group'] ) ) {
			...
			$actions_html .= sprintf( '<a class="button wc-action-button wc-action-button-%1$s %1$s" href="%2$s" aria-label="%3$s" title="%3$s">%4$s</a>', esc_attr( $action['action'] ), esc_url( $action['url'] ), esc_attr( $action['title'] ?? $action['name'] ), esc_html( $action['name'] ) );
		}
	}
