		...
		<tr>
			...
			<td>
				...
				if ( ! is_null( $version ) ) {
					...
					echo '<span class="hide-from-export">' . esc_html__( 'Yes', 'woocommerce' ) . ' <span aria-hidden="true">-</span></span>';
					...
					echo '<span class="hide-from-export">' . esc_html__( 'No', 'woocommerce' ) . ' <span aria-hidden="true">-</span> </span>';
		...
		<tr>
			...
			<td>
				...
				if ( ! is_null( $version ) ) {
					...
					echo '<span class="hide-from-export">' . esc_html__( 'Yes', 'woocommerce' ) . ' <span aria-hidden="true">-</span></span>';
					echo '<span class="screen-reader-text">' . esc_html__( 'Version', 'woocommerce' ) . ' </span> ' . esc_html( $version ) . ' ';
					echo '<code class="private">' . esc_html( $path ) . '</code></mark> ';
				} else {
					echo '<mark class="error"><span class="dashicons dashicons-warning" aria-hidden="true"></span> ';
					echo '<span class="hide-from-export">' . esc_html__( 'No', 'woocommerce' ) . ' <span aria-hidden="true">-</span> </span>';
					echo esc_html__( 'Unable to detect the Blocks package.', 'woocommerce' ) . '</mark> ';
				}
				?>
			</td>
		</tr>
		<tr>
			...
			<td>
				...
				if ( ! is_null( $version ) ) {
					...
					echo '<mark class="error"><span class="dashicons dashicons-warning" aria-hidden="true"></span> ';
					echo '<span class="hide-from-export">' . esc_html__( 'No', 'woocommerce' ) . ' <span aria-hidden="true">-</span> </span>';
					echo esc_html__( 'Unable to detect the Action Scheduler package.', 'woocommerce' ) . '</mark> ';
				}
				?>
			</td>
		</tr>
		<tr>
			...
			<td>
				...
				if ( $environment['log_directory_writable'] ) {
					echo '<mark class="yes"><span class="dashicons dashicons-yes" aria-hidden="true"></span> ';
					echo '<span class="hide-from-export">' . esc_html__( 'Yes', 'woocommerce' ) . ' <span aria-hidden="true">-</span></span>';
					echo '<span class="screen-reader-text">' . esc_html__( 'Path', 'woocommerce' ) . '</span> <code class="private">' . esc_html( $environment['log_directory'] ) . '</code></mark>';
				} else {
					echo '<mark class="error"><span class="dashicons dashicons-warning" aria-hidden="true"></span> ';
					echo '<span class="hide-from-export">' . esc_html__( 'No', 'woocommerce' ) . ' <span aria-hidden="true">-</span> </span>';
					/* Translators: %1$s: Log directory, %2$s: Log directory constant */
					echo sprintf( esc_html__( 'To allow logging, make %1$s writable or define a custom %2$s.', 'woocommerce' ), '<code>' . esc_html( $environment['log_directory'] ) . '</code>', '<code>WC_LOG_DIR</code>' ) . '</mark> ';
				}
				?>
			</td>
		</tr>
		<tr>
			...
			<td>
				...
				if ( version_compare( $environment['wp_version'], $latest_version, '<' ) ) {
					...
					echo '<mark class="error"><span class="dashicons dashicons-warning" aria-hidden="true"></span> ' . sprintf( esc_html__( '%1$s - There is a newer version of WordPress available (%2$s)', 'woocommerce' ), esc_html( $environment['wp_version'] ), esc_html( $latest_version ) ) . '</mark>';
				} else {
					echo '<mark class="yes">' . esc_html( $environment['wp_version'] ) . ' <span class="screen-reader-text">' . esc_html__( 'Up to date.', 'woocommerce' ) . '</span></mark>';
				}
				?>
			</td>
		</tr>
		<tr>
			...
			<td>
				...
				if ( $environment['wp_multisite'] ) {
					echo '<mark class="yes"><span class="dashicons dashicons-yes" aria-hidden="true"></span> ';
					echo '<span class="hide-from-export">' . esc_html__( 'Yes', 'woocommerce' ) . '</span></mark>';
				} else {
					echo '<mark class="no"><span class="screen-reader-text">' . esc_html__( 'No', 'woocommerce' ) . '</span><span aria-hidden="true">&ndash;</span></mark>';
				}
				?>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<?php if ( $environment['wp_debug_mode'] ) : ?>
					<mark class="yes"><span class="dashicons dashicons-yes" aria-hidden="true"></span> <span class="hide-from-export"><?php esc_html_e( 'Yes', 'woocommerce' ); ?></span></mark>
				<?php else : ?>
					<mark class="no"><span class="screen-reader-text"><?php esc_html_e( 'No', 'woocommerce' ); ?></span><span aria-hidden="true">&ndash;</span></mark>
				<?php endif; ?>
			</td>
		</tr>
		<tr>
	...
	<tbody>
		...
		<tr>
			...
			<td>
				...
				if ( version_compare( $environment['php_version'], '7.2', '>=' ) ) {
					...
					echo '<mark class="' . esc_attr( $class ) . '">' . ( 'recommendation' === $class ? '' : '<span class="dashicons dashicons-warning" aria-hidden="true"></span> ' ) . esc_html( $environment['php_version'] ) . ' - ' . wp_kses_post( $notice ) . '</mark>';
				}
				?>
			</td>
		</tr>
			...
			<tr>
				...
				<td>
					...
					if ( version_compare( $environment['mysql_version'], '5.6', '<' ) && ! strstr( $environment['mysql_version_string'], 'MariaDB' ) ) {
						...
						echo '<mark class="error"><span class="dashicons dashicons-warning" aria-hidden="true"></span> ' . sprintf( esc_html__( '%1$s - We recommend a minimum MySQL version of 5.6. See: %2$s.', 'woocommerce' ), esc_html( $environment['mysql_version_string'] ), '<a href="https://wordpress.org/about/requirements/" target="_blank">' . esc_html__( 'WordPress requirements', 'woocommerce' ) . '</a>' ) . '</mark>';
					} else {
		...
		<tr>
			...
			<td>
				...
				if ( 'UTC' !== $environment['default_timezone'] ) {
					...
					echo '<mark class="yes"><span class="dashicons dashicons-yes" aria-hidden="true"></span> <span class="hide-from-export">' . esc_html__( 'Yes', 'woocommerce' ) . '</span></mark>';
				}
				?>
			</td>
		</tr>
		<tr>
			...
			<td>
				...
				if ( $environment['fsockopen_or_curl_enabled'] ) {
					...
					echo '<mark class="error"><span class="dashicons dashicons-warning" aria-hidden="true"></span> ';
					echo '<span class="hide-from-export">' . esc_html__( 'No', 'woocommerce' ) . ' <span aria-hidden="true">-</span> </span>';
					echo esc_html__( 'Your server does not have fsockopen or cURL enabled - PayPal IPN and other scripts which communicate with other servers will not work. Contact your hosting provider.', 'woocommerce' );
					echo '</mark>';
				}
				?>
			</td>
		...
		<tr>
			...
			<td>
				...
				if ( $environment['soapclient_enabled'] ) {
					...
					echo sprintf( esc_html__( 'Your server does not have the %s class enabled - some gateway plugins which use SOAP may not work as expected.', 'woocommerce' ), '<a href="https://php.net/manual/en/class.soapclient.php">SoapClient</a>' );
					echo '</mark>';
				}
				?>
			</td>
		...
		<tr>
			...
			<td>
				...
				if ( $environment['gzip_enabled'] ) {
					...
					echo '<span class="hide-from-export">' . esc_html__( 'No', 'woocommerce' ) . ' <span aria-hidden="true">-</span> </span>';
					/* Translators: %s classname and link. */
					echo sprintf( esc_html__( 'Your server does not support the %s function - this is required to use the GeoIP database from MaxMind.', 'woocommerce' ), '<a href="https://php.net/manual/en/zlib.installation.php">gzopen</a>' );
					echo '</mark>';
				}
				?>
			</td>
		...
		<tr>
			...
			<td>
				...
				if ( $environment['mbstring_enabled'] ) {
					...
					echo '<span class="hide-from-export">' . esc_html__( 'No', 'woocommerce' ) . ' <span aria-hidden="true">-</span> </span>';
					/* Translators: %s classname and link. */
					echo sprintf( esc_html__( 'Your server does not support the %s functions - this is required for better character encoding. Some fallbacks will be used instead for it.', 'woocommerce' ), '<a href="https://php.net/manual/en/mbstring.installation.php">mbstring</a>' );
					echo '</mark>';
				}
				?>
			</td>
		...
		<tr>
			...
			<td>
				...
				if ( $environment['remote_post_successful'] ) {
					...
					echo '<span class="hide-from-export">' . esc_html__( 'No', 'woocommerce' ) . ' <span aria-hidden="true">-</span> </span>';
					/* Translators: %s classname and link. */
					echo sprintf( esc_html__( '%s failed. Contact your hosting provider.', 'woocommerce' ), 'wp_remote_post()' ) . ' ' . esc_html( $environment['remote_post_response'] );
					echo '</mark>';
				}
				?>
			</td>
		...
		<tr>
			...
			<td>
				...
				if ( $environment['remote_get_successful'] ) {
					...
					echo '<span class="hide-from-export">' . esc_html__( 'No', 'woocommerce' ) . ' <span aria-hidden="true">-</span> </span>';
					/* Translators: %s classname and link. */
					echo sprintf( esc_html__( '%s failed. Contact your hosting provider.', 'woocommerce' ), 'wp_remote_get()' ) . ' ' . esc_html( $environment['remote_get_response'] );
					echo '</mark>';
				}
				?>
			</td>
		...
		foreach ( $rows as $row ) {
			if ( ! empty( $row['success'] ) ) {
				$css_class = 'yes';
				$icon      = '<span class="dashicons dashicons-yes" aria-hidden="true"></span>';
			} else {
				$css_class = 'error';
				$icon      = '<span class="dashicons dashicons-no-alt" aria-hidden="true"></span>';
			}
			?>
			<tr>
				...
				<td>
					<mark class="<?php echo esc_attr( $css_class ); ?>">
						<?php echo wp_kses_post( $icon ); ?> <span class="hide-from-export"><?php echo ( 'yes' === $css_class ? esc_html__( 'Yes', 'woocommerce' ) : esc_html__( 'No', 'woocommerce' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?> <span aria-hidden="true">-</span> </span><?php echo wp_kses_data( ! empty( $row['note'] ) ? $row['note'] : '' ); ?>
					</mark>
				</td>
			</tr>
			<?php
		...
		<tr>
			...
			<td>
				<?php if ( $security['secure_connection'] ) : ?>
					<mark class="yes"><span class="dashicons dashicons-yes" aria-hidden="true"></span> <span class="hide-from-export"><?php esc_html_e( 'Yes', 'woocommerce' ); ?></span></mark>
				<?php else : ?>
					<mark class="error"><span class="dashicons dashicons-warning" aria-hidden="true"></span> <span class="hide-from-export"><?php esc_html_e( 'No', 'woocommerce' ); ?> <span aria-hidden="true">-</span> </span>
					<?php
		...
		<tr>
			...
			<td>
				<?php if ( $security['hide_errors'] ) : ?>
					<mark class="yes"><span class="dashicons dashicons-yes" aria-hidden="true"></span> <span class="hide-from-export"><?php esc_html_e( 'Yes', 'woocommerce' ); ?></span></mark>
				<?php else : ?>
					<mark class="error"><span class="dashicons dashicons-warning" aria-hidden="true"></span> <span class="hide-from-export"><?php esc_html_e( 'No', 'woocommerce' ); ?> <span aria-hidden="true">-</span> </span><?php echo esc_html__( 'Error messages should not be shown to visitors.', 'woocommerce' ); ?></mark>
				<?php endif; ?>
			</td>
		</tr>
	</tbody>
...
<table class="wc_status_table widefat" cellspacing="0">
	...
	<tbody>
		...
		foreach ( $wp_pages as $_page ) {
			...
			// Page ID check.
			if ( ! $_page['page_set'] ) {
				echo '<mark class="error"><span class="dashicons dashicons-warning" aria-hidden="true"></span> ' . esc_html__( 'Page not set', 'woocommerce' ) . '</mark>';
				$found_error = true;
			} elseif ( ! $_page['page_exists'] ) {
				echo '<mark class="error"><span class="dashicons dashicons-warning" aria-hidden="true"></span> ' . esc_html__( 'Page ID is set, but the page does not exist', 'woocommerce' ) . '</mark>';
				$found_error = true;
			} elseif ( ! $_page['page_visible'] ) {
				/* Translators: %s: docs link. */
				echo '<mark class="error"><span class="dashicons dashicons-warning" aria-hidden="true"></span> ' . wp_kses_post( sprintf( __( 'Page visibility should be <a href="%s" target="_blank">public</a>', 'woocommerce' ), 'https://wordpress.org/support/article/content-visibility/' ) ) . '</mark>';
				$found_error = true;
			} else {
		...
		<tr>
			...
			<td>
				...
				if ( $theme['is_child_theme'] ) {
					...
					echo '<span class="hide-from-export">' . esc_html__( 'No', 'woocommerce' ) . ' <span aria-hidden="true">-</span> </span>';
					/* Translators: %s docs link. */
					echo wp_kses_post( sprintf( __( 'If you are modifying WooCommerce on a parent theme that you did not build personally we recommend using a child theme. See: <a href="%s" target="_blank">How to create a child theme</a>', 'woocommerce' ), 'https://developer.wordpress.org/themes/advanced-topics/child-themes/' ) );
				}
				?>
				</td>
		</tr>
		...
		<tr>
			...
			<td>
				...
				if ( ! $theme['has_woocommerce_support'] ) {
					echo '<mark class="error"><span class="dashicons dashicons-warning" aria-hidden="true"></span> ' . esc_html__( 'Not declared', 'woocommerce' ) . '</mark>';
				} else {
					echo '<mark class="yes"><span class="dashicons dashicons-yes" aria-hidden="true"></span> ';
					echo '<span class="hide-from-export">' . esc_html__( 'Yes', 'woocommerce' ) . '</span></mark>';
				}
