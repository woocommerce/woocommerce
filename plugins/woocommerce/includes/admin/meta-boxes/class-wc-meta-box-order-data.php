...
class WC_Meta_Box_Order_Data {
	...
	public static function init_address_fields() {
		...
		/**
		 * Filters the Billing fields used in this meta box.
		 *
		 * @since Unknown
		 *
		...
		/**
		 * Filters the shipping fields used in this meta box.
		 *
		 * @since Unknown
		 *
	...
	public static function output( $post ) {
		...
		<div class="panel-wrap woocommerce">
			<input name="post_title" type="hidden" value="<?php echo esc_attr( empty( $post->post_title ) ? __( 'Order', 'woocommerce' ) : $post->post_title ); ?>" />
			...
			<div id="order_data" class="panel woocommerce-order-data">
				...
				<p class="woocommerce-order-data__meta order_number">
					...
					if ( $payment_method && 'other' !== $payment_method ) {
						$payment_method_string = wp_kses_post(
							sprintf(
								/* translators: %s: payment method */
								__( 'Payment via %s', 'woocommerce' ),
								isset( $payment_gateways[ $payment_method ] ) ? $payment_gateways[ $payment_method ]->get_title() : $payment_method
							)
						);

						$transaction_id = $order->get_transaction_id();
						if ( $transaction_id ) {
							if ( isset( $payment_gateways[ $payment_method ] ) && ! empty( $payment_gateways[ $payment_method ]->get_transaction_url( $order ) ) ) {
				...
				<div class="order_data_column_container">
					<div class="order_data_column">
						...
						<p class="form-field form-field-wide">
							...
							<?php
							/**
							 * Filters the date input HTML pattern.
							 *
							 * @since Unknown
							 *
							...
							?>
							<input type="text" class="date-picker" name="order_date" maxlength="10" value="<?php echo esc_attr( date_i18n( 'Y-m-d', strtotime( $post->post_date ) ) ); ?>" pattern="<?php echo esc_attr( $pattern ); ?>" />@
							&lrm;
					...
					<div class="order_data_column">
						...
						<div class="address">
							...
							/** Filter documented elsewhere in this file. */
							if ( apply_filters( 'woocommerce_enable_order_notes_field', 'yes' == get_option( 'woocommerce_enable_order_comments', 'yes' ) ) && $post->post_excerpt ) { //PHPCS:ignore WooCommerce.Commenting.CommentHooks
						...
						<div class="edit_address">
							...
							/**
							 * Filters if the notes field should be enabled.
							 *
							 * @since Unknown
							 *
							 * @param bool $bool Whether the notes field should be enabled.
							 */
	...
	public static function save( $order_id ) {
		...
		if ( ! empty( $_POST['_payment_method'] ) && $order->get_payment_method() !== wc_clean( wp_unslash( $_POST['_payment_method'] ) ) ) {
		...
		if ( empty( $_POST['order_date'] ) ) {
			$date = time();
		} elseif ( ! empty( $_POST['order_date_hour'] ) && ! empty( $_POST['order_date_minute'] ) && ! empty( $_POST['order_date_second'] ) ) {
			$date = gmdate( 'Y-m-d H:i:s', strtotime( wc_clean( wp_unslash( $_POST['order_date'] ) ) . ' ' . (int) $_POST['order_date_hour'] . ':' . (int) $_POST['order_date_minute'] . ':' . (int) $_POST['order_date_second'] ) );
		}
		...
		$order_status = isset( $_POST['order_status'] ) ? wc_clean( wp_unslash( $_POST['order_status'] ) ) : 'on-hold';
		$order->set_status( $order_status, '', true );
		$order->save();
	}
}
