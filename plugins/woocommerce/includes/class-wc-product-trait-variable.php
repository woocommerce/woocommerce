...
class WC_Product_Trait_Variable extends WC_Product_Trait {
	...
	*/
	 ...
	 * @return string
	 */
	public function _add_to_cart_aria_describedby() {
		/**
		 * This filter is documented in includes/abstracts/abstract-wc-product.php.
		 *
		 * @since 7.8.0
		 */
		return apply_filters( 'woocommerce_product_add_to_cart_aria_describedby', $this->product->is_purchasable() ? __( 'This product has multiple variants. The options may be chosen on the product page', 'woocommerce' ) : '', $this );
	}
	 ...
	 * @return string
	 */
	public function _add_to_cart_text() {
		return apply_filters( 'woocommerce_product_add_to_cart_text', $this->product->is_purchasable() ? __( 'Select options', 'woocommerce' ) : __( 'Read more', 'woocommerce' ), $this );
	}
	 ...
	 * @return string
	 */
	public function _add_to_cart_description() {
		/* translators: %s: Product title */
		return apply_filters( 'woocommerce_product_add_to_cart_description', sprintf( __( 'Select options for &ldquo;%s&rdquo;', 'woocommerce' ), $this->product->get_name() ), $this );
	}
	 ...
	 */
	public function _get_variation_prices( $for_display = false ) {
		$prices = $this->product->data_store->read_price_data( $this, $for_display );

		foreach ( $prices as $price_key => $variation_prices ) {
			$prices[ $price_key ] = $this->product->_( $variation_prices );
		}
	 ...
	 * @return string
	 */
	public function _get_variation_regular_price( $min_or_max = 'min', $for_display = false ) {
		$prices = $this->product->get_variation_prices( $for_display );
	 ...
	 * @return string
	 */
	public function _get_variation_sale_price( $min_or_max = 'min', $for_display = false ) {
		$prices = $this->product->get_variation_prices( $for_display );
	 ...
	 * @return string
	 */
	public function _get_variation_price( $min_or_max = 'min', $for_display = false ) {
		$prices = $this->product->get_variation_prices( $for_display );
	 ...
	 * @return string
	 */
	public function _get_price_html( $price = '' ) {
		...
		if ( empty( $prices['price'] ) ) {
			...
			} elseif ( $this->product->is_on_sale() && $min_reg_price === $max_reg_price ) {
				$price = wc_format_sale_price( wc_price( $max_reg_price ), wc_price( $min_price ) );
			} else {
				$price = wc_price( $min_price );
			}

			$price = apply_filters( 'woocommerce_variable_price_html', $price . $this->product->get_price_suffix(), $this );
		}
	 ...
	 * @return string
	 */
	public function _get_price_suffix( $price = '', $qty = 1 ) {
	 ...
	 */
	public function _get_children( $visible_only = '' ) {
		if ( is_bool( $visible_only ) ) {
			wc_deprecated_argument( 'visible_only', '3.0', 'WC_Product_Variable::_' );

			return $visible_only ? $this->product->_() : $this->product->get_children();
		}

		if ( null === $this->product->children ) {
			$children = $this->product->data_store->read_children( $this );
			$this->product->set_children( $children['all'] );
			$this->product->set_visible_children( $children['visible'] );
		}

		return apply_filters( 'woocommerce_get_children', $this->product->children, $this, false );
	}
	 ...
	 * @return array Children ids
	 */
	public function _get_visible_children() {
		if ( null === $this->product->visible_children ) {
			$children = $this->product->data_store->read_children( $this );
			$this->product->set_children( $children['all'] );
			$this->product->set_visible_children( $children['visible'] );
		}
		return apply_filters( 'woocommerce_get_children', $this->product->visible_children, $this, true );
	}
	 ...
	 */
	public function _get_variation_attributes() {
		if ( null === $this->product->variation_attributes ) {
			$this->product->variation_attributes = $this->product->data_store->read_variation_attributes( $this );
		}
	 ...
	 * @return string
	 */
	public function _get_variation_default_attribute( $attribute_name ) {
		$defaults       = $this->product->get_default_attributes();
	 ...
	 * @return bool
	 */
	public function _get_downloadable( $context = 'view' ) {
		return false;
	}
	 ...
	 * @return bool
	 */
	public function _get_virtual( $context = 'view' ) {
		return false;
	}
	 ...
	 */
	public function _get_available_variations( $return = 'array' ) {
		...
		foreach ( $variation_ids as $variation_id ) {
			...
			if ( apply_filters( 'woocommerce_hide_invisible_variations', true, $this->product->get_id(), $variation ) && ! $variation->variation_is_visible() ) {
				continue;
			}

			if ( 'array' === $return ) {
				$available_variations[] = $this->product->get_available_variation( $variation );
			} else {
	 ...
	 */
	private function _variation_is_available( WC_Product_Variation $variation ) {
		// Hide out of stock variations if 'Hide out of stock items from the catalog' is checked.
		if ( ! $variation || ! $variation->exists() || ( 'yes' === get_option( 'woocommerce_hide_out_of_stock_items' ) && ! $variation->is_in_stock() ) ) {
			return false;
		}
	 ...
	 * @return array|bool
	 */
	public function _get_available_variation( $variation ) {
		...
		$show_variation_price = apply_filters( 'woocommerce_show_variation_price', $variation->get_price() === '' || $this->product->get_variation_sale_price( 'min' ) !== $this->product->get_variation_sale_price( 'max' ) || $this->product->get_variation_regular_price( 'min' ) !== $this->product->get_variation_regular_price( 'max' ), $this, $variation );
	...
	*/
	 ...
	 */
	public function _set_variation_attributes( $variation_attributes ) {
		$this->product->variation_attributes = $variation_attributes;
	}
	 ...
	 */
	public function _set_children( $children ) {
		$this->product->children = array_filter( wp_parse_id_list( (array) $children ) );
	}
	 ...
	 */
	public function _set_visible_children( $visible_children ) {
		$this->product->visible_children = array_filter( wp_parse_id_list( (array) $visible_children ) );
	}
	...
	*/
	 ...
	 * @since 3.0.0
	 */
	public function _validate_props() {
		parent::validate_props();

		if ( ! $this->product->get_manage_stock() ) {
			$this->product->data_store->sync_stock_status( $this );
		}
	}
	 ...
	 */
	protected function _before_data_store_save_or_update() {
		// Get names before save.
		$previous_name = $this->product->data['name'];
		$new_name      = $this->product->get_name( 'edit' );
	 ...
	 */
	protected function _after_data_store_save_or_update( $state ) {
		$this->product->data_store->sync_variation_names( $this, $state['previous_name'], $state['new_name'] );
		$this->product->data_store->sync_managed_variation_stock_status( $this );
	}
	...
	*/
	 ...
	 * @return bool
	 */
	public function _is_on_sale( $context = 'view' ) {
	 ...
	 * @return boolean
	 */
	public function _child_is_in_stock() {
		return $this->product->data_store->child_is_in_stock( $this );
	}
	 ...
	 * @return boolean
	 */
	public function _child_is_on_backorder() {
		return $this->product->data_store->child_has_stock_status( $this, 'onbackorder' );
	}
	 ...
	 * @return boolean
	 */
	public function _child_has_weight() {
		$transient_name = 'wc_child_has_weight_' . $this->product->get_id();
		$has_weight     = get_transient( $transient_name );

		if ( false === $has_weight ) {
			$has_weight = $this->product->data_store->child_has_weight( $this );
	 ...
	 * @return boolean
	 */
	public function _child_has_dimensions() {
		$transient_name = 'wc_child_has_dimensions_' . $this->product->get_id();
		$has_dimension  = get_transient( $transient_name );

		if ( false === $has_dimension ) {
			$has_dimension = $this->product->data_store->child_has_dimensions( $this );
	 ...
	 * @return bool
	 */
	public function _has_dimensions() {
		return parent::has_dimensions() || $this->product->child_has_dimensions();
	}
	 ...
	 * @return bool
	 */
	public function _has_weight() {
		return parent::has_weight() || $this->product->child_has_weight();
	}
	...
	*/
	 ...
	 */
	public static function _sync( $product, $save = true ) {
	 ...
	 * @return WC_Product Synced product object.
	 */
	public static function _sync_stock_status( $product, $save = true ) {
		if ( ! is_a( $product, 'WC_Product' ) ) {
			$product = wc_get_product( $product );
		}
		if ( is_a( $product, 'WC_Product_Variable' ) ) {
	 ...
	 * @return array
	 */
	protected function _sort_variation_prices( $prices ) {
		asort( $prices );
