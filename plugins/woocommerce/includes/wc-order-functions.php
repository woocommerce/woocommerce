...
function wc_delete_shop_order_transients( $order = 0 ) {
	...
	if ( is_a( $order, 'WC_Order' ) ) {
		...
		Users::delete_site_user_meta( $order->get_customer_id(), '_money_spent' );
		Users::delete_site_user_meta( $order->get_customer_id(), '_order_count' );
		Users::delete_site_user_meta( $order->get_customer_id(), '_last_order' );
	} else {
		$order_id = 0;
	}
