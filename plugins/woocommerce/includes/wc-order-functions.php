...
function wc_restock_refunded_items( $order, $refunded_line_items ) {
	...
	foreach ( $line_items as $item_id => $item ) {
		...
		$order->add_order_note( $restock_note, false, false, array( 'note_type' => OrderNoteType::PRODUCT_STOCK ) );
...
function wc_order_fully_refunded( $order_id ) {
	...
	$order->add_order_note( __( 'Order status set to refunded. To return funds to the customer you will need to issue a refund through your payment gateway.', 'woocommerce' ), false, false, array( 'note_type' => OrderNoteType::ORDER_UPDATE ) );
}
