...
Components must follow WAI-ARIA guidelines. Prefer semantic HTML over ARIA attributes. `@wordpress/components` (`Button`, `Modal`, `Notice`, `Popover`) already include proper ARIA — use them when possible.

**Testing query priority:** `getByRole` > `getByLabelText` > `getByText` > `getByTitle`
