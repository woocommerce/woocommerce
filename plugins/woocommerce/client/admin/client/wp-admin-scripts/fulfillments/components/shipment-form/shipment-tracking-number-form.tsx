...
export default function ShipmentTrackingNumberForm() {
	...
	const handleTrackingNumberLookup = async () => {
		...
		try {
			...
			if ( ! parsingResponse.tracking_number ) {
				setError(
					__(
			...
			setTrackingNumber( parsingResponse.tracking_number );
			setTrackingUrl( parsingResponse.tracking_url );
			setShipmentProvider( parsingResponse.provider );
			setProviderName( '' );
			setEditMode( false );
		} catch ( err ) {
			setError(
