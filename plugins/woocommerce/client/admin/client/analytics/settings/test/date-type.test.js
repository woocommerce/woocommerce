...
describe( 'Analytics settings - date type', () => {
	...
	it( 'updates, saves, and resets analytics settings through real controls', async () => {
		...
		// Strict for consistency with the two filter suites, and because the six
		// report keys are cleared by being set to undefined rather than deleted,
		// which a loose comparison would treat as absent.
		expect( navigationQuerySnapshots ).toStrictEqual( [
			{
				period: undefined,
				compare: undefined,
				before: undefined,
		...
		// Spelled out rather than mapped out of the same config module the
		// component reads: a derived expectation moves with the config and so
		// cannot catch a changed default. The key comparison below keeps this
		// honest if a setting is ever added to the page.
