...
describe( 'buildEmailEditorReviewUrl', () => {
	...
	it( 'encodes via URLSearchParams (no hand-rolled concat)', () => {
		// Spaces or symbols would break a hand-rolled URL; URLSearchParams escapes them.
		const url = buildEmailEditorReviewUrl( 9999 );
		// Locks the contract: post comes before action which comes before the drawer param.
		expect( url ).toMatch(
