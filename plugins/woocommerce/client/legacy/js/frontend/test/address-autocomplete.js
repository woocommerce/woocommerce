...
describe("Address Autocomplete Provider Registration", () => {
  ...
  test("should freeze provider after successful registration", () => {
    ...
    expect(
      Object.isFrozen(window.wc.addressAutocomplete.providers["test-provider"])
    ).toBe(true);
  ...
  test("should not allow duplicate provider registration", () => {
    ...
    expect(
      window.wc.addressAutocomplete.providers["test-provider"].canSearch()
    ).toBe(false);
    expect(
  ...
  test("should allow multiple providers with different IDs", () => {
    ...
    expect(window.wc.addressAutocomplete.providers["provider-1"]).toBeDefined();
    expect(window.wc.addressAutocomplete.providers["provider-2"]).toBeDefined();
...
describe("Address Suggestions Component", () => {
  ...
  describe("DOM Initialization", () => {
    ...
    test("should add autocomplete-available class when provider is active", () => {
      ...
      expect(billingWrapper.classList.contains("autocomplete-available")).toBe(
        true
      );
      expect(shippingWrapper.classList.contains("autocomplete-available")).toBe(
        true
      );
    });
  });
  ...
  describe("Address Suggestions Display", () => {
    ...
    test("should display suggestions for input with 3 or more characters", async () => {
      ...
      expect(mockProvider.search).toHaveBeenCalledWith("123", "US", "billing");
  ...
  describe("Browser Autofill Management", () => {
    test("should disable browser autofill when suggestions are shown", async () => {
      ...
      expect(billingAddressInput.getAttribute("data-lpignore")).toBe("true");
      expect(billingAddressInput.getAttribute("data-op-ignore")).toBe("true");
      expect(billingAddressInput.getAttribute("data-1p-ignore")).toBe("true");
    });
