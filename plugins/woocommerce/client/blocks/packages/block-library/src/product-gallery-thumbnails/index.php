...
/**
 * Renders the `woocommerce/product-gallery-thumbnails` block on the server.
 *
 * @since 11.0.0
 *
 ...
 * @return string Rendered product gallery thumbnails block.
 */
function render_block_woocommerce_product_gallery_thumbnails( $attributes, $content, $block ) {
	unset( $content );
...
/**
 * Registers the `woocommerce/product-gallery-thumbnails` block on the server.
 *
 * @since 11.0.0
 */
function register_block_woocommerce_product_gallery_thumbnails(): void {
	if ( WP_Block_Type_Registry::get_instance()->is_registered( 'woocommerce/product-gallery-thumbnails' ) ) {
		return;
	}
	...
	register_block_type_from_metadata(
		__DIR__,
		array(
			'render_callback' => 'render_block_woocommerce_product_gallery_thumbnails',
		)
	);
}

add_action( 'init', 'register_block_woocommerce_product_gallery_thumbnails' );
