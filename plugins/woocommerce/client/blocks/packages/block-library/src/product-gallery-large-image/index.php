...
/**
 * Renders the `woocommerce/product-gallery-large-image` block on the server.
 *
 * @since 11.0.0
 *
 ...
 * @return string Rendered product gallery large image block.
 */
function render_block_woocommerce_product_gallery_large_image( $attributes, $content, $block ) {
...
/**
 * Registers the `woocommerce/product-gallery-large-image` block on the server.
 *
 * @since 11.0.0
 */
function register_block_woocommerce_product_gallery_large_image(): void {
	if ( WP_Block_Type_Registry::get_instance()->is_registered( 'woocommerce/product-gallery-large-image' ) ) {
		return;
	}
	...
	register_block_type_from_metadata(
		__DIR__,
		array(
			'render_callback'   => 'render_block_woocommerce_product_gallery_large_image',
			'skip_inner_blocks' => true,
		)
	);
}

add_action( 'init', 'register_block_woocommerce_product_gallery_large_image' );
