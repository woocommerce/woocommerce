<?php
/**
 * Server-side rendering of the `woocommerce/product-gallery-large-image-next-previous` block.
 *
...
/**
 * Renders the `woocommerce/product-gallery-large-image-next-previous` block on the server.
 *
 * @since 11.0.0
 *
 ...
 * @return string Rendered product gallery next/previous buttons block.
 */
function render_block_woocommerce_product_gallery_large_image_next_previous( $attributes, $content, $block ) {
	unset( $content );
...
/**
 * Registers the `woocommerce/product-gallery-large-image-next-previous` block on the server.
 *
 * @since 11.0.0
 */
function register_block_woocommerce_product_gallery_large_image_next_previous(): void {
	if ( WP_Block_Type_Registry::get_instance()->is_registered( 'woocommerce/product-gallery-large-image-next-previous' ) ) {
		return;
	}
	...
	register_block_type_from_metadata(
		__DIR__,
		array(
			'render_callback' => 'render_block_woocommerce_product_gallery_large_image_next_previous',
		)
	);
}

add_action( 'init', 'register_block_woocommerce_product_gallery_large_image_next_previous' );
