<?php
/**
 * Server-side rendering of the `woocommerce/product-categories` block.
 *
...
/**
 * Server renderer for the `woocommerce/product-categories` block.
 *
 * @since 11.0.0
 */
final class WooCommerce_Block_Library_Product_Categories {
	/**
	 * @var array
	 */
	...
	/**
	 * Category children keyed by parent term ID.
	 *
	 * @var array<int, WP_Term[]>
	 */
	private array $category_children = array();
	...
	protected function get_categories( $attributes ) {
		...
		if ( ! empty( $attributes['hasImage'] ) ) {
			$attachment_ids = array_filter( array_map( static fn( WP_Term $category ): int => (int) get_term_meta( $category->term_id, 'thumbnail_id', true ), $categories ) );
	 ...
	 *
	 * @param WP_Term[] $categories List of terms.
	...
	protected function build_category_tree( $categories, $children_only ) {
		...
		foreach ( $tree as $category ) {
			if ( ! empty( $categories_by_parent[ 'cat-' . $category->term_id ] ) ) {
				$this->category_children[ $category->term_id ] = $this->fill_category_children( $categories_by_parent[ 'cat-' . $category->term_id ], $categories_by_parent );
			}
		}
	 ...
	 *
	 * @param WP_Term[]             $categories List of terms.
	 * @param array<string, array>  $categories_by_parent List of terms grouped by parent.
	 * @return WP_Term[]
	 */
	 ...
	 * @param WP_Term[] $categories List of terms.
	 * @param array     $attributes Block attributes. Default empty array.
	 * @param string    $uid Unique ID for the rendered block, used for HTML IDs.
	...
	protected function renderDropdownOptions( $categories, $attributes, $uid, $depth = 0 ) {
		...
		foreach ( $categories as $category ) {
			...
			$output .= '
				<option value="' . esc_attr( $this->get_category_link( $category ) ) . '">
					' . str_repeat( '&minus;', $depth ) . '
					' . esc_html( $category->name ) . '
					' . $this->getCount( $category, $attributes ) . '
				</option>
				' . ( ! empty( $children ) ? $this->renderDropdownOptions( $children, $attributes, $uid, $depth + 1 ) : '' ) . '
			';
		}
		...
		foreach ( $categories as $category ) {
			...
			$output .= '
				<li class="wc-block-product-categories-list-item">
					<a style="' . esc_attr( $link_color_style ) . '" href="' . esc_url( $this->get_category_link( $category ) ) . '">'
					...
					. '</a>'
					. $this->getCount( $category, $attributes )
					. ( ! empty( $children ) ? $this->renderList( $children, $attributes, $uid, $depth + 1 ) : '' ) . '
				</li>
			';
		}
	 ...
	 * @return string
	 */
	public function get_image_html( WP_Term $category, $attributes, $size = 'woocommerce_thumbnail' ) {
		if ( empty( $attributes['hasImage'] ) ) {
			return '';
		}
	 ...
	 * @return string
	 */
	protected function getCount( WP_Term $category, $attributes ) {
	 ...
	 *
	 * @param WP_Term $category Term object.
	 * @return string Category permalink, or empty string when unavailable.
	 */
	private function get_category_link( WP_Term $category ): string {
	...
	/**
	 * Get children assigned while building the category tree.
	 *
	 * @param WP_Term $category Term object.
	 * @return WP_Term[]
	 */
	private function get_category_children( WP_Term $category ): array {
		return $this->category_children[ $category->term_id ] ?? array();
	}
}

/**
 * Registers the `woocommerce/product-categories` block on the server.
 *
 * @since 11.0.0
 */
function register_block_woocommerce_product_categories(): void {
	if ( WP_Block_Type_Registry::get_instance()->is_registered( 'woocommerce/product-categories' ) ) {
		return;
	}

	$renderer = new WooCommerce_Block_Library_Product_Categories();
...
add_action( 'init', 'register_block_woocommerce_product_categories' );
