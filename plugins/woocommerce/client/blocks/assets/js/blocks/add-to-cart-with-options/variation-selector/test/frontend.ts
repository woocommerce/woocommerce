...
// The `woocommerce` context `getContext( 'woocommerce' )` returns —
// `undefined` simulates no per-element override (falls back to updating
// `mockWooState.products.variationId` directly).
let mockWooContext: { variationId?: number | null } | undefined;

// The unified `woocommerce` store's state this module reads and writes
// through: `itemInContext.baseProduct`/`.draftItem`/`.variation`,
// `findItem`, and `products.variations`/`.variationId`. Setting
// `itemInContext.draftItem` simulates a draft view already resolved for the
// in-context product/variation — whether it answers a live draft's values,
// another surface's values, or a not-yet-materialized seed; an absent
// `draftItem` simulates no product in context. Because this file's tests
// never exercise the real draft view, `draftItem` is a directly-settable
// plain object standing in for it.
let mockWooState: {
	itemInContext: Partial< Envelope >;
	products: {
...
function createVariationWriteSpy(
	initial: Partial< Envelope[ 'draftItem' ] > = {}
) {
	...
	const draftItem = {
		id: 1,
		...initial,
	} as NonNullable< Envelope[ 'draftItem' ] >;
	Object.defineProperty( draftItem, 'variation', {
		enumerable: true,
		configurable: true,
		get: () => current,
		set: ( value: unknown ) => {
...
describe( 'Variation selector frontend store', () => {
	beforeEach( () => {
		...
		mockWooContext = undefined;
		mockWooState = {
			itemInContext: {},
			products: { variations: {} },
			findItem: jest.fn( () => ( { product: null } ) ),
		};
		mockElementRef = null;
	...
	describe( 'actions.toggle', () => {
		it( 'does nothing, and writes no draft, when the item is hidden or disabled', () => {
			const { draftItem, setter } = createVariationWriteSpy();
			mockWooState.itemInContext = { draftItem };
		...
		it( 'records exactly one write, replacing the shared selection wholesale, when deselecting the currently selected value', () => {
			...
			draftItem.variation = [ { attribute: 'Color', value: 'blue' } ];
			setter.mockClear();
			mockWooState.itemInContext = { draftItem };
			mockContext.name = 'Color';
			mockContext.selectedAttributes = [
	...
	describe( 'callbacks.resolveVariationId', () => {
		it( 'does nothing when the base product has no variations', () => {
			...
			expect( mockWooState.findItem ).not.toHaveBeenCalled();
			expect( mockWooState.products.variationId ).toBeUndefined();
		} );
		...
		it( "resolves this surface's local selection into the global variationId when no shared draft exists yet", () => {
			...
			mockWooState.findItem = jest.fn( () => ( {
				product: { id: 2 } as ProductResponseItem,
			} ) );
			...
			expect( mockWooState.products.variationId ).toBe( 2 );
		} );

		it( 'writes the resolved variation id onto a per-element `woocommerce` context when present', () => {
			mockContext.selectedAttributes = [
				{ attribute: 'Color', value: 'blue' },
			];
			mockWooContext = { variationId: null };
			mockWooState.itemInContext.baseProduct = {
				id: 1,
				variations: [ { id: 2 } ],
			} as ProductResponseItem;
			...
			expect( mockWooContext.variationId ).toBe( 2 );
			expect( mockWooState.products.variationId ).toBeUndefined();
		} );
		...
		it( 'resolves to null when the attributes do not fully resolve a variation', () => {
			...
			// The variation-existence probe resolves the parent itself when
			// no variation matches.
			mockWooState.findItem = jest.fn( () => ( {
				product: { id: 1 } as ProductResponseItem,
			} ) );

			const { callbacks } = loadStore();
			callbacks.resolveVariationId();

			expect( mockWooState.products.variationId ).toBeNull();
		} );
	...
	describe( 'callbacks.validateVariation', () => {
		...
		it( "validates the resolved collection's draft-resolved attribute selection rather than this instance's stale local context, so a sibling surface displaying a complete configuration can also submit it", () => {
			...
			mockWooState.products.variations = {
	...
	describe( 'state.selectedAttributes', () => {
		...
		it( "falls back to the local selection — never an empty array — when the resolved draft's `variation` is present but empty", () => {
			...
			mockWooState.itemInContext = {
				draftItem: { id: 1, quantity: 1, variation: [] },
			};
