 ...
 * catalog layer, this module's own `productScopes` records, and `cart.ts`'s
 * `findItemInCart`. `findItemInCart` is optional: `cart.ts` is a separate
 * script module that may not have registered yet when this state is bound.
 */
type SharedState = CatalogState &
	Pick< ScopeState, 'productScopes' > &
	Partial< Pick< CartStore[ 'state' ], 'findItemInCart' > >;

// Bound once, by `index.ts`, right after its `store()` registration. This
// module never calls `store()` itself, to avoid registering `woocommerce` a
// second time.
...
/**
 * Returns the scope's record, creating it first when missing. Resolves the
 * identity before publishing it to `state.productScopes`, then reads the
 * record back from there, so later writes land on the tracked proxy rather
 * than on a raw object the state proxy is no longer watching.
 *
 * @param name    The scope's name.
...
/**
 * Compares two variation selections for equality, order-independent and with
 * attribute names normalized, for matching an unnamed ref's `variation`
 * against a record's draft.
 *
 * @param a The first selection.
 * @param b The second selection.
 * @return `true` when both selections carry the same attributes and values.
 */
function sameVariation(
	a: TemplateVariationAttribute[],
	b: TemplateVariationAttribute[]
): boolean {
	...
	return a.every( ( entry ) =>
		b.some(
			( other ) =>
				attributeNamesMatch( entry.attribute, other.attribute ) &&
...
/**
 * Finds the one name under `state.productScopes` whose record's draft id
 * equals `ref.productId` (and whose variation equals `ref.variation`, when
 * given), for an unnamed `findProductScope( ref )`. Returns `null` when
 * `ref` gives no `productId` at all, so a `cartItemKey`-only ref never
 * falls back to a record matched by `state.template`'s product id.
 *
 * @param ref The ref (or declared context) to match records against.
 * @return The matching name, or `null` when zero, several, or no `productId`.
 */
function findSingleMatchingRecordName(
	ref: ProductScopeContext | null
): string | null {
	if ( ref?.productId === undefined ) {
		return null;
	}
	const matches = Object.keys( state.productScopes ).filter( ( name ) => {
		const draft = state.productScopes[ name ].draftCartItem;
		if ( draft?.id !== ref.productId ) {
			return false;
		}
		return (
			ref.variation === undefined ||
			sameVariation( draft?.variation ?? [], ref.variation )
		);
	} );
 ...
 * Enumerates as a plain object — `id`, `variation`, `quantity` and any
 * extension prop — so `{ ...draftCartItem }`, `JSON.stringify` and `'key' in
 * draftCartItem` behave the way a payload built from it needs them to.
 * `delete` and `Object.defineProperty` with a data descriptor edit the
 * record like an assignment. Deleting `id`/`variation`, accessor or
 * non-configurable definitions, and `Object.freeze`/`seal`/
 * `preventExtensions` are refused, so the empty target never breaks a
 * Proxy invariant.
 *
 * @param name       The scope's name.
...
function createDraftCartItemProxy(
	...
	const identityKeys = [ 'id', 'variation', 'quantity' ];
	...
	const resolvedKeys = (): string[] => {
		const record = state.productScopes[ name ];
		const extensionKeys = Object.keys( record?.draftCartItem ?? {} ).filter(
			( key ) => ! identityKeys.includes( key )
		);
		return [ ...identityKeys, ...extensionKeys ];
	};
	...
	const writeValue = ( key: string, value: unknown ): void => {
		if ( key === 'id' || key === 'variation' ) {
			writeIdentity(
				...
				getLocator(),
				key,
				value as number | TemplateVariationAttribute[]
			);
			return;
		}
	...
	const handler: ProxyHandler< Record< string, unknown > > = {
		get( _target, key ) {
			return typeof key === 'string' ? getValue( key ) : undefined;
		},
		...
		has( _target, key ) {
			return typeof key === 'string' && resolvedKeys().includes( key );
		},
		ownKeys() {
			return resolvedKeys();
		},
		getOwnPropertyDescriptor( _target, key ) {
			if ( typeof key !== 'string' || ! resolvedKeys().includes( key ) ) {
				return undefined;
			}
			return {
				enumerable: true,
		...
		deleteProperty( _target, key ) {
			...
			delete state.productScopes[ name ]?.draftCartItem?.[ key ];
			return true;
		},
		defineProperty( _target, key, descriptor ) {
			...
			writeValue( key, descriptor.value );
			return true;
		},
		preventExtensions() {
			return false;
...
function createEnvelope(
	...
	// Shared by `scopeName`, `draftCartItem` and the identity setters below:
	// an unnamed ref falls back to the single record whose draft id (and
	// variation, when the ref gives one) matches the ref itself — not the
	// already-resolved identity, which may have fallen back to
	// `state.template` and would then match unrelated records.
	const resolveScopeName = (): string | null =>
		resolveName() ?? findSingleMatchingRecordName( getLocator() );
