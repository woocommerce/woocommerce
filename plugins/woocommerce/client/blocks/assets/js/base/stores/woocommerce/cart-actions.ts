...
/**
 * This module's own `state` and `actions` references, bound once by
 * `cart.ts` right after its `store()` registration returns. The generator
 * functions below were object-literal methods inside that `store()` call and
 * closed over its `state`/`actions` consts directly; moving them into this
 * module keeps their bodies unchanged but requires the same references to be
 * supplied from outside, since a closure can only capture bindings in its own
 * module's scope.
 */
...
/**
 * Binds this module's `state` and `actions` to the ones the `woocommerce`
 * store's own `store()` registration returns. Called once, by `cart.ts`,
 * immediately after its second `store()` call resolves.
 *
 * @param boundState   The store's own returned state reference.
 * @param boundActions The store's own returned, registered actions reference.
 */
export function bindCartActions(
	boundState: Store[ 'state' ],
	boundActions: Store[ 'actions' ]
): void {
	state = boundState;
...
export function* removeCartItem( key: string ): AsyncAction< void > {
...
export function* batchAddCartItems(
	...
	try {
		...
		const batchProductCaptures = new Map< string, BatchProductCapture >();
