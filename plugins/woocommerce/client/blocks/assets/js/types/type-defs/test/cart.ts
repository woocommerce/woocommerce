...
/**
 * Verifies that `CartItem` declares `is_canonical_product_line` as a required boolean
 ...
 * `is_canonical_product_line`; the field is deliberately absent so that optimistic
...
describe( 'CartItem TypeScript interface', () => {
	describe( 'is_canonical_product_line field on CartItem', () => {
		it( 'accepts is_canonical_product_line: true for a plain standalone line', () => {
			const item: CartItem = {
				...minimalCartItem,
				is_canonical_product_line: true,
			};
			expect( item.is_canonical_product_line ).toBe( true );
		} );

		it( 'accepts is_canonical_product_line: false for a meta-differentiated line', () => {
			const item: CartItem = {
				...minimalCartItem,
				is_canonical_product_line: false,
			};
			expect( item.is_canonical_product_line ).toBe( false );
		} );

		it( 'exposes is_canonical_product_line as a boolean property on CartItem', () => {
			expect( typeof minimalCartItem.is_canonical_product_line ).toBe(
				'boolean'
			);
		} );
	} );

	describe( 'OptimisticCartItem does not declare is_canonical_product_line', () => {
		it( 'does not carry is_canonical_product_line (field is absent)', () => {
		...
		it( 'accessing a missing is_canonical_product_line yields undefined (falsy-safe)', () => {
			...
			// is_canonical_product_line must yield undefined (falsy), not throw.
			const asUnion = minimalOptimisticCartItem as
				| OptimisticCartItem
				| CartItem;
			const value = ( asUnion as CartItem ).is_canonical_product_line;
