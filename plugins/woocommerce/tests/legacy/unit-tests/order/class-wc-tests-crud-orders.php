...
class WC_Tests_CRUD_Orders extends WC_Unit_Test_Case {
	...
	public function test_calculate_shipping_with_shipping_classes() {
		...
		$product_1 = WC_Helper_Product::create_simple_product( false, array( 'name' => 'Test Shipping Product 1', 'sku' => 'TSSKU1' ) );
		$product_1->set_shipping_class_id( $shipping_class_1['term_id'] );
		$product_1->save();

		$product_2 = WC_Helper_Product::create_simple_product( false, array( 'name' => 'Test Shipping Product 2', 'sku' => 'TSSKU2' ) );
		$product_2->set_shipping_class_id( $shipping_class_2['term_id'] );
		$product_2->save();

		$product_3 = WC_Helper_Product::create_simple_product( false, array( 'name' => 'Test Shipping Product 3', 'sku' => 'TSSKU3' ) );
		$product_3->set_shipping_class_id( $shipping_class_3['term_id'] );
		$product_3->save();

		$product_4 = WC_Helper_Product::create_simple_product( false, array( 'name' => 'Test Shipping Product 4', 'sku' => 'TSSKU4' ) );
		...
		$map = array(
			$shipping_class_1['term_id'] => $instance_id_1, // NY
			$shipping_class_2['term_id'] => $instance_id_2, // FL
			$shipping_class_3['term_id'] => $instance_id_3, // CA
			$shipping_class_4['term_id'] => $instance_id_4, // IL
		);
		...
		foreach ( $map as $shipping_class_id => $instance_id ) {
			$data = array(
			...
			$data['woocommerce_flat_rate_class_cost_' . $shipping_class_id ] = $shipping_class_id * 10;
		...
		// Customers address in in NY and product_1 is shippable to NY.
		$this->assertEquals( (string) ( 10 * $shipping_class_1['term_id'] ), WC()->cart->get_shipping_total() );

		WC()->cart->add_to_cart( $product_2->get_id(), 1 );

		WC()->cart->calculate_shipping();

		// product_2 is not shippable to NY.
		...
		$this->assertEquals( (string) ( 10 * $shipping_class_2['term_id'] ), WC()->cart->get_shipping_total() );

		WC()->cart->add_to_cart( $product_1->get_id(), 1 );

		WC()->cart->calculate_shipping();

		// product_1 is not shippable to FL.
		$this->assertEquals( '0', WC()->cart->get_shipping_total() );
