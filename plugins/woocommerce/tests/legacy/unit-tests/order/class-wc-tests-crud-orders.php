...
class WC_Tests_CRUD_Orders extends WC_Unit_Test_Case {
	...
	/**
	 * Test: get_prices_include_tax falls back to the store setting when the order has no value of
	 * its own, so an order created without one is not silently treated as tax exclusive.
	 *
	 * @param string $option_value The woocommerce_prices_include_tax option value.
	 * @param bool   $expected     The value the getter should report.
	 *
	 * @dataProvider provide_prices_include_tax_store_settings
	 */
	public function test_get_prices_include_tax_defaults_to_store_setting( $option_value, $expected ) {
		update_option( 'woocommerce_prices_include_tax', $option_value );

		$object = new WC_Order();

		$this->assertSame( $expected, $object->get_prices_include_tax() );
	}

	/**
	 * Test: an order keeps a value it was given explicitly, even when that value matches the
	 * property default and contradicts the store setting.
	 *
	 * @param string $option_value The woocommerce_prices_include_tax option value.
	 *
	 * @dataProvider provide_prices_include_tax_store_settings
	 */
	public function test_explicit_prices_include_tax_survives_save( $option_value ) {
		update_option( 'woocommerce_prices_include_tax', $option_value );

		foreach ( array( true, false ) as $explicit_value ) {
			$object = new WC_Order();
			$object->set_prices_include_tax( $explicit_value );
			$object->save();

			$this->assertSame(
				$explicit_value,
				$object->get_prices_include_tax(),
				'An explicitly set prices_include_tax value should not be replaced by the store setting.'
			);

			$this->assertSame(
				$explicit_value,
				wc_get_order( $object->get_id() )->get_prices_include_tax(),
				'An explicitly set prices_include_tax value should survive a round trip through the data store.'
			);
		}
	}

	/**
	 * Test: an order created without an explicit value records the store setting, so the value
	 * stays stable if the store later changes its tax entry mode.
	 *
	 * @param string $option_value The woocommerce_prices_include_tax option value.
	 * @param bool   $expected     The value the order should be created with.
	 *
	 * @dataProvider provide_prices_include_tax_store_settings
	 */
	public function test_prices_include_tax_is_recorded_on_create( $option_value, $expected ) {
		update_option( 'woocommerce_prices_include_tax', $option_value );

		$object = new WC_Order();
		$object->save();

		// Flip the store setting: the saved order should keep what it was created with.
		update_option( 'woocommerce_prices_include_tax', 'yes' === $option_value ? 'no' : 'yes' );

		$this->assertSame(
			$expected,
			wc_get_order( $object->get_id() )->get_prices_include_tax(),
			'An order should keep the tax entry mode it was created with when the store setting changes.'
		);
	}

	/**
	 * Data provider covering both store tax entry modes.
	 *
	 * @return array[]
	 */
	public function provide_prices_include_tax_store_settings() {
		return array(
