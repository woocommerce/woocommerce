...
- **Respect the selected base class.** Tests that extend `WP_UnitTestCase` directly or indirectly and call the parent lifecycle run each test in a `$wpdb` transaction. Rollback covers writes through that connection to transactional tables; it does not cover class fixtures, explicit commits or DDL, non-transactional tables, other connections, files, external services, or process state. Plain PHPUnit does not provide this rollback; inspect other custom bases for their own lifecycle. Never use `TRUNCATE TABLE`: its implicit commit breaks isolation; use `DELETE FROM` when deletion is required.
