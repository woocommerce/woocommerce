...
test.describe(
	...
	() => {
		...
		test(
		'cannot delete the default shipping zone "Locations not covered by your other zones"',
		async( {request} ) => {
	...
	test(
		'cannot update the default shipping zone',
		async( {request} ) => {
			...
			const response = await request.put(
				` / wp - json / wc / v3 / shipping / zones / 0`,
				{
					data:newZoneDetails
				}
			)
	...
	test(
		...
		async( {request} ) => {
			...
			const response = await request.put(
				` / wp - json / wc / v3 / shipping / zones / ${shippingZone.id}`,
				{
	...
	test(
		'can add a shipping region to a shipping zone',
		async( {request} ) => {
			...
			const putResponse2Countries = await request.put(
				` / wp - json / wc / v3 / shipping / zones / ${shippingZone.id} / locations`,
				{
	...
	test(
		'can clear/delete a shipping region on a shipping zone',
		async( {request} ) => {
			// GB and US locations exist initially
			// update the locations of the shipping zone regions to contain an individual state
			const putResponseStateOnly = await request.put(
