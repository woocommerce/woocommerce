...
test.describe(
    ...
    test.beforeAll(
		async( { request } ) => {
            ...
            // Have "John" order all products.
					Object.values( orderedProducts ).forEach(
						async( product ) => {
            ...
            // create child order by referencing a parent_id
					const order3 = await request.post(
						'/wp-json/wc/v3/orders',
						{
							data: {
								...orderBaseData,
            ...
            const coupon     = await request.post(
            ...
            const order4     = await request.post(
				'/wp-json/wc/v3/orders',
				{
					data: {
						...orderBaseData,
            ...
            const colorNamesObjectArray       = colorNames.map(
					( name ) => ( {
                    name,
                    } )
				);
            ...
            const sizeNamesObjectArray        = sizeNames.map(
					( name ) => ( {
                    name,
                    } )
				);
            ...
            const description                  =
					...
					'Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. ' +
					'Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo.</p>\n';
            const hoodie                       = await request.post(
                        '/wp-json/wc/v3/products',
                        {
						data: {
							name: 'Hoodie oxo',
            ...
            const vneck                            = await request.post(
					'/wp-json/wc/v3/products',
					{
						data: {
            ...
            const createSampleHierarchicalProducts = async() => {
					const parent                           = await request.post(
					'/wp-json/wc/v3/products',
					{
						data: {
	...
	test(
		'can add shipping and billing contacts to an order',
		async(
			{
					request,
			}
	...
	test(
		'can permanently delete an order',
		async( { request } ) => {
	...
	test.describe(
        ...
        test(
			...
			// Verify the last page only has 1 order as we expect.
				const lastPage = await request.get(
					'wp-json/wc/v3/orders',
					{
						params: {
							per_page: pageSize,
		...
		test(
			'inclusion / exclusion',
			async( { request } ) => {
            const allOrders = await request.get(
				'wp-json/wc/v3/orders',
				{
					params: {
						per_page: 10,
			...
			const included       = await request.get(
				'wp-json/wc/v3/orders',
				{
					params: {
						per_page: 20,
		...
		test(
			...
			const result4     = await request.get(
				'wp-json/wc/v3/orders',
				{
					params: {
						search: 'Doe',
			...
			const result5     = await request.get(
				'wp-json/wc/v3/orders',
				{
					params: {
						search: 'Pennant oxo',
