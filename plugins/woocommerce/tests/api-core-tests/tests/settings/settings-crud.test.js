...
test.describe('Settings API tests: CRUD', () => {
	...
	test.describe('List all settings groups', () => {
		...
		test('can retrieve all settings groups', async ({
			request
		}) => {
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						description: "",
						parent_id: "",
						"sub_groups": expect.arrayContaining(["email_new_order"]),
					})
				]));
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						label: "Failed order",
						description: "Failed order emails are sent to chosen recipient(s) when orders have been marked failed (if they were previously pending or on-hold).",
						parent_id: "email",
						"sub_groups": expect.arrayContaining([]),
					})
				]));
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						description: "Order refunded emails are sent to customers when their orders are refunded.",
						parent_id: "email",
						"sub_groups": expect.arrayContaining([]),
					})
				]));
	...
	test.describe('List all settings options', () => {
		...
		test('can retrieve all general settings', async ({
			...
			const response = await request.get('/wp-json/wc/v3/settings/general');
			const responseJSON = await response.json();
			expect(response.status()).toEqual(200);
			expect(Array.isArray(responseJSON)).toBe(true);
			expect(responseJSON.length).toBeGreaterThan(0);
	...
	test.describe('Retrieve a settings option', () => {
		...
		test('can retrieve a settings option', async ({
			request
		}) => {
			// call API to retrieve all settings options
			const response = await request.get('/wp-json/wc/v3/settings/general/woocommerce_allowed_countries');
			const responseJSON = await response.json();
			expect(response.status()).toEqual(200);
			expect(responseJSON).toEqual(
				expect.objectContaining({
	...
	test.describe('Update a settings option', () => {
		...
		test('can update a settings option', async ({
			request
		}) => {
			// call API to update settings options
			const response = await request.put('/wp-json/wc/v3/settings/general/woocommerce_allowed_countries', {
				data: {
					value: "all_except"
				}
			});
	...
	test.describe('Batch Update a settings option', () => {
		...
		test('can batch update settings options', async ({
			request
		}) => {

			// call API to update settings options
			const response = await request.post('/wp-json/wc/v3/settings/general/batch', {
				data: {
					update: [{
							id: "woocommerce_allowed_countries",
							value: "all_except"
			...
			const countriesUpdatedResponse = await request.get('/wp-json/wc/v3/settings/general/woocommerce_allowed_countries');
			const countriesUpdatedResponseJSON = await countriesUpdatedResponse.json();
			expect(countriesUpdatedResponseJSON.value).toEqual('all_except');

			const currencyUpdatedResponse = await request.get('/wp-json/wc/v3/settings/general/woocommerce_currency');
			const currencyUpdatedResponseJSON = await currencyUpdatedResponse.json();
	...
	test.describe('List all Products settings options', () => {
		...
		test('can retrieve all products settings', async ({
			request
		}) => {
			// call API to retrieve all settings options
			const response = await request.get('/wp-json/wc/v3/settings/products');
			const responseJSON = await response.json();
			expect(response.status()).toEqual(200);
			expect(Array.isArray(responseJSON)).toBe(true);
			expect(responseJSON.length).toBeGreaterThan(0);
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"description": "If you are using X-Accel-Redirect download method along with NGINX server, make sure that you have applied settings as described in <a href='https://docs.woocommerce.com/document/digital-downloadable-product-handling#nginx-setting'>Digital/Downloadable Product Handling</a> guide.",
						"type": "select",
						"default": "force",
						"options": {
	...
	test.describe('List all Tax settings options', () => {
		...
		test('can retrieve all tax settings', async ({
			request
		}) => {
			// call API to retrieve all settings options
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"description": "Optionally control which tax class shipping gets, or leave it so shipping tax is based on the cart items themselves.",
						"type": "select",
						"default": "inherit",
						"options": {
	...
	test.describe('List all Shipping settings options', () => {
		...
		test('can retrieve all shipping settings', async ({
			request
		}) => {
			// call API to retrieve all settings options
			const response = await request.get('/wp-json/wc/v3/settings/shipping');
			const responseJSON = await response.json();
			expect(response.status()).toEqual(200);
			expect(Array.isArray(responseJSON)).toBe(true);
			expect(responseJSON.length).toBeGreaterThan(0);
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"label": "Calculations",
						"description": "Enable the shipping calculator on the cart page",
						"type": "checkbox",
						"default": "yes",
						"value": "yes",
					})
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"label": "",
						"description": "Hide shipping costs until an address is entered",
						"type": "checkbox",
						"default": "no",
						"value": "no",
					})
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"description": "This controls which shipping address is used by default.",
						"type": "radio",
						"default": "billing",
						"options": {
							"shipping": "Default to customer shipping address",
							"billing": "Default to customer billing address",
							"billing_only": "Force shipping to the customer billing address"
						},
	...
	test.describe('List all Checkout settings options', () => {
		...
		test('can retrieve all checkout settings', async ({
			request
		}) => {
			// call API to retrieve all settings options
			const response = await request.get('/wp-json/wc/v3/settings/checkout');
			const responseJSON = await response.json();
			expect(response.status()).toEqual(200);
			expect(Array.isArray(responseJSON)).toBe(true);
			expect(responseJSON).toEqual(
	...
	test.describe('List all Account settings options', () => {
		...
		test('can retrieve all account settings', async ({
			request
		}) => {
			// call API to retrieve all settings options
			const response = await request.get('/wp-json/wc/v3/settings/account');
			const responseJSON = await response.json();
			expect(response.status()).toEqual(200);
			expect(Array.isArray(responseJSON)).toBe(true);
			expect(responseJSON).toEqual(
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"description": "Remove personal data from orders on request",
						"type": "checkbox",
						"default": "no",
						"tip": expect.stringContaining('When handling an <a href='),
						"tip": expect.stringContaining('wp-admin/erase-personal-data.php\">account erasure request</a>, should personal data within orders be retained or removed?'),
						"value": "no",
					})
				]));
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"default": "no",
						"tip": expect.stringContaining('When handling an <a href='),
						"tip": expect.stringContaining('wp-admin/erase-personal-data.php\">account erasure request</a>, should access to downloadable files be revoked and download logs cleared?'),
						"value": "no",
					})
				]));
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"description": "Allow personal data to be removed in bulk from orders",
						"type": "checkbox",
						"default": "no",
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"description": "",
						"type": "textarea",
						"default": "Your personal data will be used to support your experience throughout this website, to manage access to your account, and for other purposes described in our [privacy_policy].",
	...
	test.describe('List all Email settings options', () => {
		...
		test('can retrieve all email settings', async ({
			request
		}) => {
			// call API to retrieve all settings options
			const response = await request.get('/wp-json/wc/v3/settings/email');
			const responseJSON = await response.json();
			expect(response.status()).toEqual(200);
			expect(Array.isArray(responseJSON)).toBe(true);
			expect(responseJSON).toEqual(
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"label": "Footer text",
						"description": "The text to appear in the footer of all WooCommerce emails. Available placeholders: {site_title} {site_url}",
						"type": "textarea",
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"label": "Base color",
						"description": "The base color for WooCommerce email templates. Default <code>#7f54b3</code>.",
						"type": "color",
						"default": "#7f54b3",
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"description": "The background color for WooCommerce email templates. Default <code>#f7f7f7</code>.",
						"type": "color",
						"default": "#f7f7f7",
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"description": "The main body background color. Default <code>#ffffff</code>.",
						"type": "color",
						"default": "#ffffff",
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"description": "The main body text color. Default <code>#3c3c3c</code>.",
						"type": "color",
						"default": "#3c3c3c",
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"description": "Receive email notifications with additional guidance to complete the basic store setup and helpful insights",
						"type": "checkbox",
						"default": "no",
						"value": "no",
					})
	...
	test.describe('List all Advanced settings options', () => {
		...
		test('can retrieve all advanced settings', async ({
			...
			// call API to retrieve all settings options
			const response = await request.get('/wp-json/wc/v3/settings/advanced');
			const responseJSON = await response.json();
			expect(response.status()).toEqual(200);
			expect(Array.isArray(responseJSON)).toBe(true);
			expect(responseJSON).toEqual(
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"description": "Endpoint for the \"Checkout &rarr; Add payment method\" page.",
						"type": "text",
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"description": "Endpoint for the delete payment method page.",
						"type": "text",
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"label": "Orders",
						"description": "Endpoint for the \"My account &rarr; Orders\" page.",
						"type": "text",
						"default": "orders",
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"label": "View order",
						"description": "Endpoint for the \"My account &rarr; View order\" page.",
						"type": "text",
						"default": "view-order",
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"label": "Downloads",
						"description": "Endpoint for the \"My account &rarr; Downloads\" page.",
						"type": "text",
						"default": "downloads",
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"label": "Edit account",
						"description": "Endpoint for the \"My account &rarr; Edit account\" page.",
						"type": "text",
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"label": "Addresses",
						"description": "Endpoint for the \"My account &rarr; Addresses\" page.",
						"type": "text",
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"description": "Endpoint for the \"My account &rarr; Payment methods\" page.",
						"type": "text",
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"label": "Lost password",
						"description": "Endpoint for the \"My account &rarr; Lost password\" page.",
						"type": "text",
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"label": "Logout",
						"description": "Endpoint for the triggering logout. You can add this to your menus via a custom link: yoursite.com/?customer-logout=true",
						"type": "text",
	...
	test.describe('List all Email New Order settings', () => {
		...
		test('can retrieve all email new order settings', async ({
			...
			// call API to retrieve all settings options
			const response = await request.get('/wp-json/wc/v3/settings/email_new_order');
			const responseJSON = await response.json();
			expect(response.status()).toEqual(200);
			expect(Array.isArray(responseJSON)).toBe(true);
			expect(responseJSON).toEqual(
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"description": "Text to appear below the main email content. Available placeholders: <code>{site_title}</code>, <code>{site_address}</code>, <code>{site_url}</code>, <code>{order_date}</code>, <code>{order_number}</code>",
						"type": "textarea",
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"description": "Text to appear below the main email content. Available placeholders: <code>{site_title}&lt;/code&gt;, &lt;code&gt;{site_address}&lt;/code&gt;, &lt;code&gt;{site_url}&lt;/code&gt;, &lt;code&gt;{order_date}&lt;/code&gt;, &lt;code&gt;{order_number}</code>",
						"type": "textarea",
						"default": "Hopefully they’ll be back. Read more about <a href=\"https://docs.woocommerce.com/document/managing-orders/\">troubleshooting failed payments</a>.",
						"tip": "Text to appear below the main email content. Available placeholders: <code>{site_title}&lt;/code&gt;, &lt;code&gt;{site_address}&lt;/code&gt;, &lt;code&gt;{site_url}&lt;/code&gt;, &lt;code&gt;{order_date}&lt;/code&gt;, &lt;code&gt;{order_number}</code>",
	...
	test.describe('List all Email Customer On Hold Order settings', () => {
		...
		test('can retrieve all email customer on hold order settings', async ({
			...
			// call API to retrieve all settings options
			const response = await request.get('/wp-json/wc/v3/settings/email_customer_on_hold_order');
			const responseJSON = await response.json();
			expect(response.status()).toEqual(200);
			expect(Array.isArray(responseJSON)).toBe(true);
			expect(responseJSON).toEqual(
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						...
						"description": "Text to appear below the main email content. Available placeholders: <code>{site_title}&lt;/code&gt;, &lt;code&gt;{site_address}&lt;/code&gt;, &lt;code&gt;{site_url}&lt;/code&gt;, &lt;code&gt;{order_date}&lt;/code&gt;, &lt;code&gt;{order_number}</code>",
						"type": "textarea",
						"default": "We look forward to fulfilling your order soon.",
						"tip": "Text to appear below the main email content. Available placeholders: <code>{site_title}&lt;/code&gt;, &lt;code&gt;{site_address}&lt;/code&gt;, &lt;code&gt;{site_url}&lt;/code&gt;, &lt;code&gt;{order_date}&lt;/code&gt;, &lt;code&gt;{order_number}</code>",
	...
	test.describe('List all Email Customer Completed Order settings', () => {
		...
		test('can retrieve all email customer completed order settings', async ({
			...
			// call API to retrieve all settings options
			const response = await request.get('/wp-json/wc/v3/settings/email_customer_completed_order');
			const responseJSON = await response.json();
			expect(response.status()).toEqual(200);
			expect(Array.isArray(responseJSON)).toBe(true);
			expect(responseJSON).toEqual(
	...
	test.describe('List all Email Customer Refunded Order settings', () => {
		...
		test('can retrieve all email customer refunded order settings', async ({
			...
			// call API to retrieve all settings options
			const response = await request.get('/wp-json/wc/v3/settings/email_customer_refunded_order');
			const responseJSON = await response.json();
			expect(response.status()).toEqual(200);
			expect(Array.isArray(responseJSON)).toBe(true);
			expect(responseJSON).toEqual(
	...
	test.describe('List all Email Customer Invoice settings', () => {
		...
		test('can retrieve all email customer invoice settings', async ({
			...
			// call API to retrieve all settings options
			const response = await request.get('/wp-json/wc/v3/settings/email_customer_invoice');
			const responseJSON = await response.json();
			expect(response.status()).toEqual(200);
			expect(Array.isArray(responseJSON)).toBe(true);
			expect(responseJSON).toEqual(
	...
	test.describe('List all Email Customer Note settings', () => {
		...
		test('can retrieve all email customer note settings', async ({
			...
			// call API to retrieve all settings options
			const response = await request.get('/wp-json/wc/v3/settings/email_customer_note');
			const responseJSON = await response.json();
			expect(response.status()).toEqual(200);
			expect(Array.isArray(responseJSON)).toBe(true);
			expect(responseJSON).toEqual(
	...
	test.describe('List all Email Customer Reset Password settings', () => {
		...
		test('can retrieve all email customer reset password settings', async ({
			...
			// call API to retrieve all settings options
			const response = await request.get('/wp-json/wc/v3/settings/email_customer_reset_password');
			const responseJSON = await response.json();
			expect(response.status()).toEqual(200);
			expect(Array.isArray(responseJSON)).toBe(true);
			expect(responseJSON).toEqual(
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						"id": "subject",
						"label": "Subject",
						"description": "Available placeholders: <code>{site_title}&lt;/code&gt;, &lt;code&gt;{site_address}&lt;/code&gt;, &lt;code&gt;{site_url}</code>",
						"type": "text",
						"default": "",
			...
			expect(responseJSON).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						"id": "additional_content",
						"label": "Additional content",
						"description": "Text to appear below the main email content. Available placeholders: <code>{site_title}&lt;/code&gt;, &lt;code&gt;{site_address}&lt;/code&gt;, &lt;code&gt;{site_url}</code>",
						"type": "textarea",
						"default": "Thanks for reading.",
	...
	test.describe('List all Email Customer New Account settings', () => {
		...
		test('can retrieve all email customer new account settings', async ({
			...
			// call API to retrieve all settings options
			const response = await request.get('/wp-json/wc/v3/settings/email_customer_new_account');
			const responseJSON = await response.json();
			expect(response.status()).toEqual(200);
			expect(Array.isArray(responseJSON)).toBe(true);
			expect(responseJSON).toEqual(
