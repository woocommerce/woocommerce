...
describe( 'PaymentMethodPlaceOrderButtonContainer', () => {
	...
	it( 'does not render container when no custom payment method button is provided', () => {
		...
		const container = screen.queryBySelector( '.wc-block-checkout__payment-method-button' );
	...
	it( 'renders container with multiple children', () => {
		const CustomPlaceOrderButton = ( props ) => (
			<div data-testid="custom-place-order-button">
				<button { ...props }>Custom Place Order</button>
	...
	it( 'maintains container structure when payment method button changes', () => {
		const CustomPlaceOrderButton1 = ( props ) => (
			<button data-testid="custom-place-order-button-1" { ...props }>
				Custom Place Order 1
			</button>
		);

		const CustomPlaceOrderButton2 = ( props ) => (
			<button data-testid="custom-place-order-button-2" { ...props }>
	...
	it( 'handles container with complex nested structure', () => {
		const ComplexCustomButton = ( props ) => (
			<div data-testid="complex-custom-button">
				<div className="button-wrapper">
					<button { ...props }>Complex Custom Button</button>
				</div>
