...
import { useCheckoutSubmit } from '../../client/blocks/assets/js/base/context/hooks/use-checkout-submit';
...
describe( 'useCheckoutSubmit', () => {
	...
	beforeEach( () => {
		...
		checkoutStore.select.mockReturnValue( mockCheckoutStore );
		paymentStore.select.mockReturnValue( mockPaymentStore );
	...
	it( 'handles payment method without placeOrderButton', () => {
		...
		expect( result.current.paymentMethodButtonLabel ).toBe( 'Pay with Test' );
		expect( result.current.paymentMethodPlaceOrderButton ).toBeUndefined();
	} );
} );
