 ...
 * (once with distinct child products, once with the same product in both
 * slots), add the bundle, and confirm — by reading the Store API cart
 * response directly — that the bundle lands as a single line whose
 * `extensions['wc-bundle-demo'].children` carries exactly what was
 * configured, with no collision between the slots, a plain standalone form
 * for one of the same child products elsewhere on the page, or (in the
 * same-product case) the two same-product slots themselves.
 */
