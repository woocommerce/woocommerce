...
test.describe( 'Merchant → Local Pickup Settings', () => {
	...
	test( 'user can configure price and tax settings', async ( {
		page,
		localPickupUtils,
	} ) => {
		await test.step( 'user can toggle the price field state', async () => {
		...
		await test.step( 'user can edit costs and tax status', async () => {
			await localPickupUtils.enableLocalPickupCosts();
	...
	test( 'user can manage pickup locations', async ( {
		page,
		localPickupUtils,
	} ) => {
		await test.step( 'user can add a new location', async () => {
		...
		await test.step( 'user can edit a location', async () => {
			await expect(
				page.getByRole( 'cell', {
					name: 'Automattic, Inc.60 29th Street, Suite 343, San Francisco, California, 94110, United States (US)',
				} )
			...
			await Promise.all( [
				page.waitForResponse( ( response ) => {
					if (
						...
						! response
							.url()
							.includes( '/wp-json/wc/v3/pickup-locations' ) ||
						! response.ok()
					) {
						return false;
					}

					const { pickup_locations: pickupLocations } = response
						.request()
						.postDataJSON();
				...
				localPickupUtils.deletePickupLocation(),
			] );
		...
		await test.step( 'user can delete a location', async () => {
			await localPickupUtils.addPickupLocation( {
				location: {
