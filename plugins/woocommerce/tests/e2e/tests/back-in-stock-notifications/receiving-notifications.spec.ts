...
test.describe( 'Back in Stock Notifications — receiving back-in-stock emails', () => {
	...
	test( 'restocking a product dispatches the back-in-stock email with UTM params', async ( {
		...
		await expect( bisEmailBody( page ).locator( 'body' ) ).toContainText(
			BIS_EMAIL_FOOTER.guest
		);
	} );
	...
	test( "a logged-in customer's back-in-stock email offers account management instead of unsubscribing", async ( {
		...
		await expect(
			bisEmailBody( page ).locator( `a${ BIS_EMAIL_LINKS.unsubscribe }` )
