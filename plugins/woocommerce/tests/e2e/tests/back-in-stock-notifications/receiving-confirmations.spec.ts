...
test.describe( 'Back in Stock Notifications — receiving confirmations', () => {
	...
	test( 'double-opt-in signup dispatches verify email with UTM params', async ( {
		...
		expect( verifyUrl.searchParams.get( 'utm_medium' ) ).toBe( 'email' );
	} );
	...
	test( 'clicking the verify link dispatches the confirmation email with UTM on unsubscribe link', async ( {
		...
		expect( unsubscribeUrl.searchParams.get( 'email_link_action' ) ).toBe(
			'unsubscribe'
		);
		expect(
