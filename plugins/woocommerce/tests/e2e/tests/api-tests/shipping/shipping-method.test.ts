...
import { getShippingMethodExample } from '../../../data';
...
test.describe( 'Shipping methods API tests', () => {
	...
	for ( const shippingMethodRow of shippingMethods ) {
		test( `can add a ${ shippingMethodRow[ methodTitleIndex ] } shipping method`, async ( {
			request,
		} ) => {
			//create the shipping method
			const shippingMethod = getShippingMethodExample(
				shippingMethodRow[ methodIdIndex ],
				shippingMethodRow[ methodCostIndex ]
			);
			...
			expect( responseJSON.method_id ).toEqual(
				shippingMethodRow[ methodIdIndex ]
			);
			expect( responseJSON.method_title ).toEqual(
				shippingMethodRow[ methodTitleIndex ]
			);
			...
			if (
				[ 'flat_rate', 'local_pickup' ].includes(
					shippingMethodRow[ methodIdIndex ]
				)
			) {
				expect( responseJSON.settings.cost.value ).toEqual(
					shippingMethodRow[ methodCostIndex ]
				);
			}
