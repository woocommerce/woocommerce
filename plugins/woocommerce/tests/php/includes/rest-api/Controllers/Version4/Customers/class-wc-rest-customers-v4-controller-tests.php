...
class WC_REST_Customers_V4_Controller_Tests extends WC_REST_Unit_Test_Case {
	...
	/**
	 * Test orderBy with include sorting.
	 */
	public function test_orderby_include(): void {
		// Create customers with different names.
		$customer1 = $this->create_test_customer(
			array(
		...
		// Test ordering by include - specify IDs in reverse order: 3, 2, 1.
		$request = new WP_REST_Request( 'GET', '/wc/v4/customers' );
		$request->set_param( 'orderby', 'include' );
		$request->set_param( 'include', array( $customer3->get_id(), $customer2->get_id(), $customer1->get_id() ) );
		$response = $this->server->dispatch( $request );
		...
		// Should be ordered by include array: Third, Second, First.
		$this->assertEquals( $customer3->get_id(), $response_data[0]['id'] );
		$this->assertEquals( $customer2->get_id(), $response_data[1]['id'] );
		$this->assertEquals( $customer1->get_id(), $response_data[2]['id'] );

		// Test ordering by include with different order: 2, 1, 3.
		$request->set_param( 'include', array( $customer2->get_id(), $customer1->get_id(), $customer3->get_id() ) );
		$response = $this->server->dispatch( $request );
		...
		$this->assertCount( 3, $response_data );
		// Should be ordered by include array: Second, First, Third.
		$this->assertEquals( $customer2->get_id(), $response_data[0]['id'] );
		$this->assertEquals( $customer1->get_id(), $response_data[1]['id'] );
		$this->assertEquals( $customer3->get_id(), $response_data[2]['id'] );

		// Test ordering by include with order=desc should reverse the include array.
		$request->set_param( 'include', array( $customer1->get_id(), $customer2->get_id(), $customer3->get_id() ) );
		$request->set_param( 'order', 'desc' );
		$response = $this->server->dispatch( $request );
		...
		$this->assertCount( 3, $response_data );
		// Should be ordered reversed: Third, Second, First.
		$this->assertEquals( $customer3->get_id(), $response_data[0]['id'] );
		$this->assertEquals( $customer2->get_id(), $response_data[1]['id'] );
		$this->assertEquals( $customer1->get_id(), $response_data[2]['id'] );
	}
