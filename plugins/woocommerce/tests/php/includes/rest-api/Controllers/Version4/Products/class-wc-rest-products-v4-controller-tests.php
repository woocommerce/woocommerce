...
class WC_REST_Products_V4_Controller_Test extends WC_REST_Unit_Test_Case {
	...
	public function test_duplicate_simple_product() {
		...
		$request  = new WP_REST_Request( 'POST', '/wc/v4/products/' . $product_id . '/duplicate' );
		$response = $this->server->dispatch( $request );
		$this->assertEquals( 200, $response->get_status() );
	...
	public function test_cogs_values_received_for_variation_product( bool $set_additive_flag ) {
		...
		$response = $this->server->dispatch( new WP_REST_Request( 'GET', '/wc/v4/products/' . $variation->get_id() ) );
		$this->assertEquals( 200, $response->get_status() );
	...
	public function test_create_product_with_duplicate_sku_trashed_original_cleans_up_images() {
		...
		$_SERVER['REQUEST_URI'] = '/wp-json/wc/v4/products';
		...
		$create_request_for_failure = new WP_REST_Request( 'POST', '/wc/v4/products' );
	...
	/**
	 * Test that simple product return correct min_price and max_price values.
	 *
	 * @return void
	 */
	public function test_simple_product_min_max_price() {
		$simple_product = array_filter(
			self::$products,
			function ( $product ) {
				return $product->get_type() === ProductType::SIMPLE;
			}
		);
		$simple_product = reset( $simple_product );

		$response = $this->server->dispatch( new WP_REST_Request( 'GET', '/wc/v4/products/' . $simple_product->get_id() ) );
		$this->assertEquals( 200, $response->get_status() );
