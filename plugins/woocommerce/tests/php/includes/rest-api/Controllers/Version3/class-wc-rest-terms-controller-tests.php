...
class WC_REST_Terms_Controller_Tests extends WC_REST_Unit_Test_Case {
	...
	public function test_product_term_crud_lifecycle( $taxonomy, $route ): void {
		...
		$name             = 'Slice 021 ' . str_replace( '_', ' ', $taxonomy ) . ' CRUD';
		...
		try {
			...
			$this->delete_product_terms( $created_term_ids, $taxonomy );
		}
	}
	...
	public function test_product_term_batch_lifecycle( $taxonomy, $route ): void {
		$created_term_ids   = array();
		$name_prefix        = 'Slice 021 ' . str_replace( '_', ' ', $taxonomy ) . ' batch';
		...
		try {
			...
			$created_term_ids[] = $first_id;
			$created_term_ids[] = $second_id;
			$created_term_ids[] = $third_id;
	...
	/**
	 * Delete product terms created by a lifecycle test.
	 *
	 * @param int[]  $term_ids Product term IDs.
	 * @param string $taxonomy Product term taxonomy.
	 */
	private function delete_product_terms( $term_ids, $taxonomy ): void {
