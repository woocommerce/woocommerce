...
class WC_REST_Products_V2_Controller_Test extends WC_REST_Unit_Test_Case {
	...
	/**
	 * @testdox Creating a product (v2) with a custom attribute using a reserved structural key name is rejected.
	 */
	public function test_create_product_blocks_reserved_custom_attribute_name() {
		$request = new WP_REST_Request( 'POST', '/wc/v2/products' );
		$request->set_body_params(
			array(
				'name'       => 'V2 Reserved Attribute Product',
				'type'       => 'variable',
				'attributes' => array(
					array(
						'name'      => 'variation',
	...
	/**
	 * @testdox Updating a v2 product that already has a reserved custom attribute name is allowed (grandfathered).
	 */
	public function test_update_product_grandfathers_existing_reserved_custom_attribute_name() {
