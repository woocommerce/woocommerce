...
class WC_Query_Test extends \WC_Unit_Test_Case {
	...
	/**
	 * @testdox woocommerce_shop_page_queried_object filter allows customization.
	 */
	public function test_shop_page_queried_object_filter() {
		switch_theme( 'twentytwentyfour' );
		...
		$filter_called = false;
		$test_instance = $this;
		$filter_callback = function ( $queried_object, $query, $shop_page ) use ( &$filter_called, $test_instance ) {
			$filter_called = true;
			$test_instance->assertInstanceOf( 'WP_Post', $queried_object );
			$test_instance->assertInstanceOf( 'WP_Query', $query );
			$test_instance->assertInstanceOf( 'WP_Post', $shop_page );
			return $queried_object;
		};

		add_filter( 'woocommerce_shop_page_queried_object', $filter_callback, 10, 3 );
		...
		$this->assertTrue( $filter_called, 'woocommerce_shop_page_queried_object filter was not called' );

		// Cleanup.
		remove_filter( 'woocommerce_shop_page_queried_object', $filter_callback );
		$wp_the_query = $previous_wp_the_query; // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
		update_option( 'woocommerce_shop_page_id', $default_woocommerce_shop_page_id );
		wp_delete_post( $shop_page_id, true );
	}
