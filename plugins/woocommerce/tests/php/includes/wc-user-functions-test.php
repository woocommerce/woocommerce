...
class WC_User_Functions_Tests extends WC_Unit_Test_Case {
	...
	/**
	 * @testdox Should only register wc_translate_user_roles on the default-domain contextual gettext hook.
	 */
	public function test_wc_translate_user_roles_uses_default_domain_contextual_gettext_hook(): void {
		$this->assertFalse(
			has_filter( 'gettext_with_context', 'wc_translate_user_roles' ),
			'wc_translate_user_roles should not be registered on gettext_with_context.'
		);
		$this->assertSame(
			10,
			has_filter( 'gettext_with_context_default', 'wc_translate_user_roles' ),
			'wc_translate_user_roles should be registered on gettext_with_context_default with priority 10.'
		);
	}

	/**
	 * @testdox Should translate WooCommerce user roles on the default-domain contextual gettext hook.
	 */
	public function test_wc_translate_user_roles_translates_customer_and_shop_manager(): void {
		$this->assertSame(
			translate_user_role( 'Customer', 'woocommerce' ),
			apply_filters( 'gettext_with_context_default', 'Customer', 'Customer', 'User role', 'default' ),
			'Customer should be translated with the WooCommerce text domain.'
		);
		$this->assertSame(
			translate_user_role( 'Shop manager', 'woocommerce' ),
			apply_filters( 'gettext_with_context_default', 'Shop manager', 'Shop manager', 'User role', 'default' ),
			'Shop manager should be translated with the WooCommerce text domain.'
		);
	}

	/**
	 * @testdox Should leave unrelated wc_translate_user_roles inputs unchanged.
	 */
	public function test_wc_translate_user_roles_leaves_unrelated_values_unchanged(): void {
		$this->assertSame(
			'Original translation',
			apply_filters( 'gettext_with_context_default', 'Original translation', 'Customer', 'Plugin name', 'default' ),
			'Translations outside the User role context should remain unchanged.'
		);
		$this->assertSame(
			'Original translation',
			wc_translate_user_roles( 'Original translation', 'Customer', 'User role', 'woocommerce' ),
			'Translations outside the default domain should remain unchanged.'
		);
		$this->assertSame(
			'Original translation',
			apply_filters( 'gettext_with_context_default', 'Original translation', 'Administrator', 'User role', 'default' ),
			'User roles outside the WooCommerce remap list should remain unchanged.'
		);
	}
}
