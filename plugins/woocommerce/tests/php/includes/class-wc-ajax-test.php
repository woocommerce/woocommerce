...
class WC_AJAX_Test extends \WP_Ajax_UnitTestCase {
	...
	/**
	 * @testdox Reordering attribute terms invalidates cached product terms.
	 */
	public function test_term_ordering_invalidates_cached_product_terms(): void {
		$attribute_id = null;
		$taxonomy     = null;
		$product      = null;
		$term_ids     = array();
		...
		try {
			$attribute_id = wc_create_attribute(
				array(
					'name'     => 'Term ordering AJAX ' . $suffix,
					'slug'     => 'term-ordering-ajax-' . $suffix,
					'order_by' => 'menu_order',
				)
			);

			$this->assertIsInt( $attribute_id, 'The product attribute should be created.' );

			$taxonomy = $this->register_attribute_taxonomy_for_test( $attribute_id );

			foreach ( array( 'Small', 'Medium', 'Large' ) as $index => $term_name ) {
				$term = wp_insert_term( $term_name, $taxonomy );

				$this->assertIsArray( $term, "The {$term_name} term should be created." );
			...
			$product = WC_Helper_Product::create_simple_product();
			wp_set_object_terms( $product->get_id(), array_values( $term_ids ), $taxonomy );

			$cached_terms = wc_get_product_terms( $product->get_id(), $taxonomy, array( 'fields' => 'all' ) );
			$this->assertSame(
			...
			$_POST['security']    = wp_create_nonce( 'term-ordering' );
			$_POST['id']          = $term_ids['Large'];
			$_POST['nextid']      = $term_ids['Small'];
			$_POST['thetaxonomy'] = $taxonomy;

			$this->do_ajax( 'woocommerce_term_ordering' );

			$reordered_terms = wc_get_product_terms( $product->get_id(), $taxonomy, array( 'fields' => 'all' ) );
			$this->assertSame(
				array( 'Large', 'Small', 'Medium' ),
				wp_list_pluck( $reordered_terms, 'name' ),
				'Product terms should use the new order without saving the product.'
			);
		} finally {
			unset( $_POST['security'], $_POST['id'], $_POST['nextid'], $_POST['thetaxonomy'] );
