...
class WC_AJAX_Test extends \WP_Ajax_UnitTestCase {
	...
	public function test_add_new_attribute_saves_color_and_image_only_for_visual_attributes(): void {
		...
		try {
			...
			$this->assertSame( (string) $image_id, get_term_meta( $image_term_id, 'image', true ), 'Visual attribute terms should store the posted image.' );
			$this->assertSame( '', get_term_meta( $image_term_id, 'color', true ), 'Visual attribute terms should not store color meta when only image is posted.' );
			...
			unset( $_POST['security'], $_POST['taxonomy'], $_POST['term'], $_POST['term_color'], $_POST['term_image'] );
