...
/**
 * Mock payment gateway with custom place order button support.
 */
class WC_Mock_Gateway_With_Custom_Button extends WC_Payment_Gateway {
	/**
	 * Constructor.
	 */
...
/**
 * Mock payment gateway without custom place order button.
 */
class WC_Mock_Gateway_Without_Custom_Button extends WC_Payment_Gateway {
	/**
	 * Constructor.
	 */
	public function __construct() {
		...
		// has_custom_place_order_button defaults to false.
	}
}

/**
 * Mock payment gateway with truthy but non-boolean value.
 */
class WC_Mock_Gateway_With_Truthy_Value extends WC_Payment_Gateway {
	/**
	 * Constructor.
	 */
	public function __construct() {
...
class WC_Frontend_Scripts_Test extends WC_Unit_Test_Case {
	...
	/**
	 * Helper to register test gateways.
	 *
	 * @param array $gateway_classes Array of gateway class names to register.
	 */
	private function register_test_gateways( array $gateway_classes ): void {
		$this->gateways_filter_callback = function ( $gateways ) use ( $gateway_classes ) {
			return array_merge( $gateways, $gateway_classes );
		};
	...
	public function test_gateway_with_custom_button_is_included(): void {
		$this->register_test_gateways( array( 'WC_Mock_Gateway_With_Custom_Button' ) );
	...
	public function test_gateway_without_custom_button_is_not_included(): void {
		$this->register_test_gateways( array( 'WC_Mock_Gateway_Without_Custom_Button' ) );
	...
	public function test_gateway_with_truthy_non_boolean_value_is_not_included(): void {
		$this->register_test_gateways( array( 'WC_Mock_Gateway_With_Truthy_Value' ) );
	...
	public function test_multiple_gateways_with_custom_buttons_are_included(): void {
		$this->register_test_gateways(
			array(
				'WC_Mock_Gateway_With_Custom_Button',
				'WC_Mock_Gateway_Without_Custom_Button',
				'WC_Mock_Gateway_With_Truthy_Value',
			)
		);
