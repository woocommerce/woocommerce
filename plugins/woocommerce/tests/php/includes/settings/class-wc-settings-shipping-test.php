...
class WC_Settings_Shipping_Test extends WC_Settings_Unit_Test_Case {
	...
	/**
	 * @testDox 'get_region_options' keeps the edited zone's persisted legacy state locations selectable so merchants can remove them.
	 */
	public function test_get_region_options_includes_the_zones_persisted_legacy_state_locations() {
		...
		try {
			$options = $this->get_region_options_values( $zone );

			$this->assertContains( 'state:NP:BAG', $options, 'The zone\'s persisted legacy location must remain selectable.' );
			$this->assertNotContains( 'state:NP:MEC', $options, 'Legacy locations that are not persisted should not be offered.' );

			$other_zone_options = $this->get_region_options_values( new WC_Shipping_Zone() );

			$this->assertNotContains( 'state:NP:BAG', $other_zone_options, 'Other zones must not offer another zone\'s legacy location.' );
		} finally {
			$zone->delete();
		}
	}
