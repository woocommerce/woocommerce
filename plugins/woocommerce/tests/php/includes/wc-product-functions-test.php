...
class WC_Product_Functions_Tests extends \WC_Unit_Test_Case {
	...
	public function test_wc_scheduled_sales_releases_the_product_objects_cache_by_id(): void {
		...
		try {
			...
			$this->assertFalse(
				$product_cache->is_cached( $product->get_id() ),
				'The processed product must be released from product_objects.'
			);
			$this->assertTrue(
				$product_cache->is_cached( $unrelated_product->get_id() ),
				'Per-ID cleanup must leave unrelated product_objects entries intact.'
			);
		} finally {
	 ...
	 */
	public function test_wc_scheduled_sales_settles_supported_data_store_result_shapes(): void {
		$ids = array();
	...
	public function test_wc_scheduled_sales_survives_malformed_ids_from_a_replaced_data_store(): void {
		...
		$store = new class( $product->get_id(), $malformed ) extends WC_Product_Data_Store_CPT {
			...
			/**
			 * Return one real id and one row that violates the int[] contract.
			 *
			 * @return array
			 */
			public function get_ending_sales() {
				return array( $this->real_id, $this->malformed );
			}
		};
