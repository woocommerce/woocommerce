...
class WC_Order_Item_Product_Test extends WC_Unit_Test_Case {
	...
	public function test_set_variation_logs_collision_for_reserved_structural_key() {
		...
		$item->set_variation(
			array(
				'attribute_variation' => 'some-value',
		...
		$this->assertEquals( 'some-value', $item->get_meta( 'variation' ), 'The colliding attribute value should still be stored as meta.' );
		$this->assertEquals( 'blue', $item->get_meta( 'color' ), 'Non-colliding attributes should be unaffected.' );
	}
