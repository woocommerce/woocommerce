...
class CheckoutFieldsTest extends WP_UnitTestCase {
	...
	/**
	 * @testdox Bulk reads can skip defaults without losing saved empty or unchecked values.
	 * @testWith [null]
	 *           [""]
	 *           ["0"]
	 *           ["Saved answer"]
	 ...
	 * @param string|null $saved_value Saved answer, or null for a missing value.
	 */
	public function test_bulk_reads_can_skip_defaults( ?string $saved_value ): void {
		$key           = 'plugin-namespace/gov-id';
		$order         = new \WC_Order();
		$default_calls = 0;
		add_filter(
			"woocommerce_get_default_value_for_{$key}",
		...
		$values = $this->controller->get_all_fields_from_object( $order, 'billing', false, false );
		if ( null === $saved_value ) {
			$this->assertArrayNotHasKey( $key, $values );
		} else {
			$this->assertArrayHasKey( $key, $values );
			$this->assertSame( $saved_value, $values[ $key ] );
		}
		$this->assertSame( 0, $default_calls, 'Disabling defaults must skip the filter.' );

		$values = $this->controller->get_all_fields_from_object( $order, 'billing' );
		$this->assertArrayHasKey( $key, $values );
		$this->assertSame( $saved_value ?? 'Default answer', $values[ $key ], 'Existing callers must still receive defaults for missing fields.' );
	}

	/**
	 * @testdox Order-to-customer sync copies saved values without creating defaults.
	 * @testWith [null]
	 *           [""]
	 *           ["0"]
	 *           ["Saved answer"]
	 ...
	 * @param string|null $saved_value Saved answer, or null for a missing value.
	 */
	public function test_sync_uses_saved_values_only( ?string $saved_value ): void {
		$key      = 'plugin-namespace/gov-id';
		$meta_key = CheckoutFields::get_group_key( 'billing' ) . $key;
		$order    = new \WC_Order();
		$customer = new \WC_Customer();
		$customer->update_meta_data( $meta_key, 'Existing answer' );
		if ( null !== $saved_value ) {
			$order->update_meta_data( $meta_key, $saved_value );
		}
		add_filter(
			"woocommerce_get_default_value_for_{$key}",
			static function () {
		...
		$this->controller->sync_customer_additional_fields_with_order( $order, $customer );

		$this->assertSame( $saved_value ?? 'Existing answer', $customer->get_meta( $meta_key ), 'Sync must not replace saved data with a default.' );
		$this->assertFalse( $customer->meta_exists( CheckoutFields::get_group_key( 'shipping' ) . $key ), 'Sync must not create values for missing fields.' );
	}

	/**
	 * @testdox Customer defaults remain available when prefilling a checkout order.
	 */
	public function test_customer_defaults_prefill_order(): void {
		$key = 'plugin-namespace/gov-id';
		add_filter(
			"woocommerce_get_default_value_for_{$key}",
			static function () {
		...
		$order = new \WC_Order();
		$this->controller->sync_order_additional_fields_with_customer( $order, new \WC_Customer() );

		$this->assertSame( 'Default answer', $order->get_meta( CheckoutFields::get_group_key( 'billing' ) . $key ), 'Checkout prefilling must retain customer defaults.' );
	}
