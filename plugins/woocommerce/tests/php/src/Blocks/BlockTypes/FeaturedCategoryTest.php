...
class FeaturedCategoryTest extends WC_Unit_Test_Case {
	 ...
	 * @testWith [false]
	 *           [true]
	 * @param bool $inherited Whether the category comes from Terms Query context.
	 */
	public function test_renders_category_thumbnail_in_managed_cover( bool $inherited ): void {
...
HTML;
	...
	/**
	 * @testdox Should remove the saved image when a category has no thumbnail.
	 */
	public function test_removes_image_without_category_thumbnail(): void {
		$category_id = $this->create_category( 'No image category' );
		$markup      = <<<'HTML'
<!-- wp:woocommerce/featured-category {"layout":"cover"} -->
...
HTML;
		...
		$this->assertStringNotContainsString( 'wp-block-cover__image-background', $output );
		$this->assertStringNotContainsString( 'woocommerce-placeholder', $output );
	...
	/**
	 * @testdox Should follow thumbnail additions and removals without resaving an image-less Cover or changing nested content.
	 * @testWith ["regular"]
	 *           ["fixed"]
	 *           ["repeated"]
	 *           ["natural"]
	 * @param string $mode Cover image mode.
	 */
	public function test_image_less_cover_follows_category_thumbnail( string $mode ): void {
		$category_id = $this->create_category( 'Changing thumbnail' );
		$attributes  = array(
			'url'         => '',
			'hasParallax' => 'fixed' === $mode,
			'isRepeated'  => 'repeated' === $mode,
			'className'   => 'natural' === $mode ? 'wc-block-featured-category__natural-image' : '',
			'focalPoint'  => array(
				'x' => 0.25,
				'y' => 0.75,
			),
		...
		$children    = '<div class="wp-block-cover__inner-container"><p>Keep this content</p><div class="wp-block-cover"><img class="wp-block-cover__image-background" src="https://example.com/nested.jpg"/></div></div>';
		$overlay     = '<span class="wp-block-cover__background has-background-dim-30" style="background-color:#004455"></span>';
		$markup      = '<!-- wp:woocommerce/featured-category {"layout":"cover"} --><!-- wp:cover ' . wp_json_encode( $attributes ) . ' --><div class="wp-block-cover" style="min-height:620px">' . $overlay . $children . '</div><!-- /wp:cover --><!-- /wp:woocommerce/featured-category -->';
		$blank       = $this->render_selected_category( $markup, $category_id );
		$this->assertSame( 1, substr_count( $blank, 'wp-block-cover__image-background' ), 'Only the nested image should exist initially.' );

		$attachment_id = $this->create_attachment();
		update_term_meta( $category_id, 'thumbnail_id', $attachment_id );
		$output = $this->render_selected_category( $markup, $category_id );
		$this->assertStringContainsString( $overlay . $children, $output, 'The overlay and nested content must remain intact.' );
		$output = wp_filter_content_tags( $output );
		$this->assertStringContainsString( 'min-height:620px', $output );
		$this->assertSame( 2, substr_count( $output, 'wp-block-cover__image-background' ) );
		$processor = new \WP_HTML_Tag_Processor( $output );
		$this->assertTrue( $processor->next_tag( array( 'class_name' => 'wp-block-cover__image-background' ) ) );
		$this->assertStringContainsString( '25% 75%', $processor->get_attribute( 'style' ) );
		if ( 'fixed' === $mode || 'repeated' === $mode ) {
			$this->assertSame( 'DIV', $processor->get_tag() );
			$this->assertTrue( $processor->has_class( 'fixed' === $mode ? 'has-parallax' : 'is-repeated' ) );
			$this->assertSame( $attributes['alt'], $processor->get_attribute( 'aria-label' ) );
			$this->assertStringContainsString( wp_get_attachment_image_url( $attachment_id, 'full' ), $processor->get_attribute( 'style' ) );
		} else {
			$this->assertSame( 'IMG', $processor->get_tag() );
			$this->assertSame( wp_get_attachment_image_url( $attachment_id, 'full' ), $processor->get_attribute( 'src' ) );
			$this->assertSame( $attributes['alt'], $processor->get_attribute( 'alt' ) );
			$this->assertSame( 'regular' === $mode, $processor->has_class( 'wp-image-' . $attachment_id ) );
			if ( 'natural' === $mode ) {
				$this->assertNull( $processor->get_attribute( 'srcset' ) );
				$this->assertNull( $processor->get_attribute( 'sizes' ) );
			} else {
				$this->assertNotEmpty( $processor->get_attribute( 'srcset' ) );
			}
		}
		...
		$this->assertSame( $blank, $this->render_selected_category( $markup, $category_id ), 'Removing the thumbnail should restore the image-less Cover.' );
	}
