...
class AddToCartWithOptions extends \WP_UnitTestCase {
	...
	/**
	 * Tests that grouped product quantity inputs use Product Quantity global border radius styles.
	 *
	 * @covers \Automattic\WooCommerce\Blocks\BlockTypes\AddToCartWithOptions\GroupedProductItemSelector::render
	 */
	public function test_grouped_product_quantity_input_uses_product_quantity_global_border_radius() {
		$original_theme = get_stylesheet();
		switch_theme( 'twentytwentyfour' );

		$filter = function ( $theme_json ) {
			$data = $theme_json->get_data();
			$data['styles']['blocks']['woocommerce/add-to-cart-with-options-quantity-selector']['border']['radius'] = '5px';
		...
		$block_markup = '<!-- wp:woocommerce/add-to-cart-with-options-grouped-product-item-selector /-->';
		...
		try {
			...
			$quantity_markup = do_blocks( $block_markup );
			$this->assertStringContainsString(
				'wc-block-components-quantity-selector',
				$quantity_markup,
				'The grouped product item selector renders a quantity input for purchasable products.'
			);
			$this->assertStringContainsString(
				'border-radius:5px',
				$quantity_markup,
				'The grouped quantity selector uses the Product Quantity block global border radius.'
			);
			...
			$checkbox_markup = do_blocks( $block_markup );
			$this->assertStringContainsString(
				'type="checkbox"',
				$checkbox_markup,
				'The grouped product item selector renders a checkbox for sold individually products.'
			);
			$this->assertStringNotContainsString(
				'border-radius:5px',
				$checkbox_markup,
				'The checkbox CTA does not receive Product Quantity border radius styles.'
			);
			...
			$button_markup = do_blocks( $block_markup );
			$this->assertStringContainsString(
				'Read more',
				$button_markup,
				'The grouped product item selector renders a button for unpurchasable products.'
			);
			$this->assertStringNotContainsString(
				'border-radius:5px',
				$button_markup,
				'The button CTA does not receive Product Quantity border radius styles.'
			);
		} finally {
