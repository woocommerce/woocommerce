...
/**
 * @covers \Automattic\WooCommerce\StoreApi\Routes\V1\CartAddFee
...
class CartFeesIntegrationTest extends ControllerTestCase {
	...
	protected function setUp(): void {
		...
		// The cart/add-fee route lives in the shared Store API namespace but is
		// only registered when the point_of_sale feature is on, so enable it
		// before register_all_routes runs below.
		...
		// Cart-Token header so add-fee resumes the cart built by add-item.
	...
	/**
	 * @testdox The cart/add-fee route is present in the REST server's index.
	 */
	public function test_routes_are_registered(): void {
		$routes = rest_get_server()->get_routes();
		$this->assertArrayHasKey( '/wc/store/v1/cart/add-fee', $routes );
	}
	...
	private function add_fee( string $name, float $amount, array $extra = array() ): \WP_REST_Response {
		$request = new \WP_REST_Request( 'POST', '/wc/store/v1/cart/add-fee' );
