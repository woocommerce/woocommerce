...
class CustomerAccountPolicyTest extends WC_Unit_Test_Case {
	...
	/**
	 * @testdox register() attaches the customer-account hook inside POS context.
	 */
	public function test_register_attaches_hook_in_pos_context(): void {
	...
	/**
	 * @testdox register() does not attach the hook outside POS context.
	 */
	public function test_register_skips_hook_outside_pos_context(): void {
		Context::set_test_override( false );

		$this->sut->register();

		$this->assertFalse( has_action( 'woocommerce_store_api_checkout_update_order_from_request', array( $this->sut, 'attach_customer_account' ) ) );
	}
