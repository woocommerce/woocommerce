...
class CheckoutIntegrationTest extends ControllerTestCase {
	...
	protected function setUp(): void {
		...
		// Bridges the `cart_token` query param onto the Cart-Token header so a
		// checkout resumes the cart built by the prior add-item call.
	...
	/**
	 * @testdox The /checkout route is registered under wc/store/v1.
	 */
	public function test_checkout_route_is_registered(): void {
		...
		$this->assertArrayHasKey(
			...
			$routes,
			'Shared /checkout route should be registered.'
		);
	}

	/**
	 * The shared /checkout route is public (the web storefront uses it), so POS
	 * gates *behaviour*, not access. Outside POS context the Store API's own nonce
	 * guard must still stand — the POS nonce opt-out is scoped to POS requests —
	 * so a nonceless write is rejected even for a manager. (The checkout schema
	 * does mark billing_address as required; POS satisfies that by injecting empty
	 * address objects in CheckoutAddressPolicy, exercised by the empty-body test.)
	 *
	 * @testdox A non-POS request to the shared /checkout route still requires a nonce (the POS opt-out is gated).
	 */
	...
	public function test_checkout_with_empty_body_creates_pending_order_from_cart(): void {
		...
		$add_request = new \WP_REST_Request( 'POST', '/wc/store/v1/cart/add-item' );
	...
	/**
	 * `customer_id` is no longer a declared schema parameter — the shared
	 * `wc/store/v1/checkout` schema deliberately doesn't advertise it, so there is
	 * no schema-level `minimum: 1` to reject a non-positive value. Instead
	 * CustomerAccountPolicy reads it off the request body and treats a
	 * non-positive id as a guest sale: the checkout succeeds and the order stays
	 * unattributed rather than failing validation.
	 *
	 * @testdox A POS checkout with a non-positive customer_id creates a guest order rather than failing.
	 */
	public function test_checkout_with_non_positive_customer_id_creates_guest_order(): void {
		wp_set_current_user( $this->admin_id );
		$cart_token = $this->seed_cart_token();
		wp_set_current_user( $this->admin_id );
		...
		$this->assertSame(
			0,
			$order->get_customer_id(),
			'A non-positive customer_id must leave the order as a guest order.'
		);
	}
	...
	public function test_default_behavior_still_requires_payment_method(): void {
		...
		// Build the cart as a POS request so add-item clears the nonce, and capture
		// its Cart-Token to carry the session into the checkout below.
		Context::set_test_override( true );
		$add_request = new \WP_REST_Request( 'POST', '/wc/store/v1/cart/add-item' );
		$add_request->set_body_params(
			array(
		...
		// Now exercise the checkout as a NON-POS caller. The Cart-Token waives the
		// nonce natively (so this isn't a nonce failure), and without POS context
		// the original Store API payment-method guard must still fire.
		Context::set_test_override( false );
		wp_set_current_user( $this->admin_id );

		$checkout_request = new \WP_REST_Request( 'POST', '/wc/store/v1/checkout' );
		// Pass the token as the native Cart-Token header: the POS cart_token param
		// bridge only runs for POS requests, and this checkout is deliberately
		// non-POS. The header waives the nonce the same way the web block client's
		// would, so the failure under test is the payment-method guard, not a nonce.
		$checkout_request->set_header( 'Cart-Token', $cart_token );
