...
/**
 * @covers \Automattic\WooCommerce\Internal\POS\StoreApi\Routes\CartApplyCoupon
...
class CartApplyCouponIntegrationTest extends ControllerTestCase {
	...
	/**
	 * @testdox The POS cart/apply-coupon route is present in the REST server's index.
	 */
	public function test_route_is_registered(): void {
		$routes = rest_get_server()->get_routes();
		$this->assertArrayHasKey( '/' . Controller::REST_NAMESPACE . '/cart/apply-coupon', $routes );
	}

	/**
	 * @testdox Anonymous request to cart/apply-coupon is forbidden.
	 */
	...
	private function apply_coupon( string $code, array $extra = array() ): \WP_REST_Response {
		$request = new \WP_REST_Request( 'POST', '/' . Controller::REST_NAMESPACE . '/cart/apply-coupon' );
		$request->set_body_params(
	...
	public function test_covered_classes_are_present(): void {
		$this->assertTrue( class_exists( CartApplyCoupon::class ) );
	}
}
