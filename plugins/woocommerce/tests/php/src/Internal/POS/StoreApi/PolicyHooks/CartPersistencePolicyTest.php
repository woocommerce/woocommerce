...
class CartPersistencePolicyTest extends WC_Unit_Test_Case {
	...
	/**
	 * @testdox register() attaches the persistent-cart opt-out inside POS context.
	 */
	public function test_register_attaches_filter_in_pos_context(): void {
		...
		$this->assertNotFalse( has_filter( 'woocommerce_persistent_cart_enabled', '__return_false' ) );
	}
	...
	public function test_register_skips_filter_outside_pos_context(): void {
		...
		$this->assertFalse( has_filter( 'woocommerce_persistent_cart_enabled', '__return_false' ) );
	}
}
