 ...
 *
 * Exercises the shared `wc/store/v1/cart/add-item` route end-to-end against the
 * real REST server, the real Store API cart pipeline, and the real session
 * handler swap, driven into POS behaviour by the policy hooks. These tests
 * would have caught:
 *
 *   - The shared route not being registered (404 / missing namespace).
...
class CartAddItemIntegrationTest extends ControllerTestCase {
	...
	protected function setUp(): void {
		...
		// Engage the POS policy hooks so the shared Store API routes behave as POS.
		// In production these are registered in class-woocommerce.php; in tests we
		// register them explicitly so the test is self-contained and exercises the
		// production wiring path.
		//
		// - CartPersistencePolicy: keeps the per-user-meta cart row from leaking
		//   into POS requests.
		// - StockPolicy: relaxes the in-stock check so oversell is permitted.
		// - CartTokenParamPolicy: bridges the `cart_token` request param onto the
		//   Cart-Token header so the session replay tests resume the same cart.
		( new CartPersistencePolicy() )->register();
		( new StockPolicy() )->register();
		( new NoncePolicy() )->register();
		( new CartTokenParamPolicy() )->register();

		// Register the shared Store API routes into our fresh REST server.
	...
	protected function tearDown(): void {
		...
		remove_all_filters( 'woocommerce_product_is_in_stock' );
		remove_all_filters( 'woocommerce_variation_is_in_stock' );
	...
	/**
	 * @testdox The shared cart/add-item route is present in the REST server's index.
	 */
	public function test_routes_are_registered(): void {
		$routes = rest_get_server()->get_routes();

		$this->assertArrayHasKey( '/wc/store/v1', $routes );
		$this->assertArrayHasKey( '/wc/store/v1/cart/add-item', $routes );
	}

	/**
	 * @testdox A non-POS request to the shared cart route still requires a nonce (the POS opt-out is gated).
	 */
	public function test_non_pos_request_still_requires_nonce(): void {
		// The shared cart route is public, so POS gates *behaviour*, not access.
		// Outside POS context the Store API's own nonce guard must still stand
		// (the POS nonce opt-out is scoped to POS requests), so a nonceless write
		// is rejected even for a manager.
	...
	/**
	 * Dispatch POST /wc/store/v1/cart/add-item with the given product id + quantity.
	 *
	...
	public function test_covered_classes_are_present(): void {
		$this->assertTrue( class_exists( StockPolicy::class ) );
