...
class OrdersTableDataStoreTests extends \HposTestCase {
	...
	public function test_cot_query_status_any_and_all() {
		...
		// Create orders with internal WordPress statuses.
		...
		// Test 'status' => 'any' - should return only valid WooCommerce statuses (excludes internal statuses).
		$query = new OrdersTableQuery( array( 'status' => 'any' ) );
		$this->assertEquals( 3, count( $query->orders ), "status='any' should return only valid WooCommerce statuses" );
		...
		$this->assertNotContains( $order_auto_draft->get_id(), $query->orders, "status='any' should exclude auto-draft orders" );
		$this->assertNotContains( $order_checkout_draft->get_id(), $query->orders, "status='any' should exclude checkout-draft orders" );
		...
		// Test empty status (should behave like 'any').
		$query = new OrdersTableQuery( array( 'status' => '' ) );
		$this->assertEquals( 3, count( $query->orders ), "Empty status should behave like 'any' and return only valid WooCommerce statuses" );
		...
		$this->assertNotContains( $order_auto_draft->get_id(), $query->orders, "Empty status should exclude auto-draft orders" );
		$this->assertNotContains( $order_checkout_draft->get_id(), $query->orders, "Empty status should exclude checkout-draft orders" );

		// Test omitted status (should behave like 'any').
		$query = new OrdersTableQuery( array() );
		$this->assertEquals( 3, count( $query->orders ), "Omitted status should behave like 'any' and return only valid WooCommerce statuses" );
		...
		$this->assertNotContains( $order_auto_draft->get_id(), $query->orders, "Omitted status should exclude auto-draft orders" );
		$this->assertNotContains( $order_checkout_draft->get_id(), $query->orders, "Omitted status should exclude checkout-draft orders" );
	}
