...
class AdminSettingsHandlerTest extends \WC_Unit_Test_Case {
	...
	/**
	 * Test that handle_enqueue_admin_scripts only enqueues on features section.
	 */
	public function test_handle_enqueue_admin_scripts_only_on_features_section(): void {
		$this->sut->register();

		// Set up the query vars to simulate features section.
		...
		// Register jquery (required dependency).
		wp_register_script( 'jquery', '', array(), '1.0.0' );

		// Call the method with the correct hook.
		$this->sut->handle_enqueue_admin_scripts( 'woocommerce_page_wc-settings' );

		// Check if inline script was added to jquery.
		$inline_scripts = $wp_scripts->get_data( 'jquery', 'after' );
		$this->assertNotEmpty( $inline_scripts );

		$script_content = implode( '', $inline_scripts );
		$this->assertStringContainsString( 'jetpack_connection_button', $script_content );
	...
	/**
	 * Test that handle_enqueue_admin_scripts doesn't enqueue on wrong hook.
	 */
	public function test_handle_enqueue_admin_scripts_doesnt_enqueue_on_wrong_hook(): void {
		$this->sut->register();
		...
		// Check that no inline script was added.
		$inline_scripts = $wp_scripts->get_data( 'jquery', 'after' );
	...
	/**
	 * Test that handle_enqueue_admin_scripts doesn't enqueue on wrong section.
	 */
	public function test_handle_enqueue_admin_scripts_doesnt_enqueue_on_wrong_section(): void {
		$this->sut->register();
