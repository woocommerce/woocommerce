...
class CallbackUtilTest extends \WC_Unit_Test_Case {
	...
	/**
	 * @testdox `get_hook_callback_signatures` should handle corrupted $wp_filter structure gracefully.
	 */
	public function test_get_hook_callback_signatures_with_corrupted_wp_filter() {
		global $wp_filter;

		$hook_name = 'test_hook_corrupted_' . wp_rand();

		// phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
		$wp_filter[ $hook_name ] = 'invalid_structure';

		$signatures = CallbackUtil::get_hook_callback_signatures( $hook_name );

		$this->assertIsArray( $signatures );
		$this->assertArrayHasKey( 0, $signatures );
		$this->assertIsArray( $signatures[0] );
		$this->assertCount( 1, $signatures[0] );
		// Should fall back to serialization.
		$this->assertStringStartsWith( 's:', $signatures[0][0] );

		unset( $wp_filter[ $hook_name ] );
	}

	/**
	 * @testdox `get_hook_callback_signatures` should handle corrupted priority entry gracefully.
	 */
	public function test_get_hook_callback_signatures_with_corrupted_priority_entry() {
		global $wp_filter;

		$hook_name = 'test_hook_corrupted_priority_' . wp_rand();

		add_action( $hook_name, '__return_true', 10 );

		// phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
		$wp_filter[ $hook_name ]->callbacks[20] = 'invalid_priority_data';
		...
		// Priority 10 should be valid and 20 should fall back to serialization.
		$this->assertSame( '__return_true', $signatures[10][0] );

		$this->assertCount( 1, $signatures[20] );
		$this->assertStringStartsWith( 's:', $signatures[20][0] );

		remove_all_actions( $hook_name );
	}

	/**
	 * @testdox `get_hook_callback_signatures` should handle corrupted callback entry gracefully.
	 */
	public function test_get_hook_callback_signatures_with_corrupted_callback_entry() {
		global $wp_filter;

		$hook_name = 'test_hook_corrupted_callback_' . wp_rand();

		add_action( $hook_name, '__return_true', 10 );

		// Corrupt one callback entry by removing the 'function' key.
		$callbacks = $wp_filter[ $hook_name ]->callbacks[10];
		$keys      = array_keys( $callbacks );
		$first_key = $keys[0];

		// phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
		$wp_filter[ $hook_name ]->callbacks[10][ $first_key ] = array( 'invalid' => 'data' );
		...
		// Should fall back to serialization for the corrupted entry.
		$this->assertStringStartsWith( 'a:', $signatures[10][0] );
