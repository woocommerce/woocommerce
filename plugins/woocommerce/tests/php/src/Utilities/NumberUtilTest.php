...
class NumberUtilTest extends \WC_Unit_Test_Case {
	...
	/**
	 * @testdox `floor` should work as the built-in function of the same name when passing a number.
	 */
	public function test_floor_when_passing_a_number() {
		$actual   = NumberUtil::floor( 1234.9 );
		$expected = 1234;
		$this->assertEquals( $expected, $actual );
	}

	/**
	 * @testdox `floor` should work as the built-in function of the same name when passing a number-like string.
	 */
	public function test_floor_when_passing_a_number_like_string() {
		$actual   = NumberUtil::floor( '1234.5678' );
		$expected = 1234;
		$this->assertEquals( $expected, $actual );
	}

	/**
	 * @testdox `floor` should work as the built-in function of the same name when passing a number-like string with spaces.
	 */
	public function test_floor_when_passing_a_number_like_string_with_spaces() {
		$actual   = NumberUtil::floor( '  1234.5678  ' );
		$expected = 1234;
		$this->assertEquals( $expected, $actual );
	}
	...
	/**
	 * @testdox `floor` should return false when passing a non-numeric value except 'true'.
	 *
	 * @dataProvider data_provider_for_test_round_when_passing_a_non_number_like_string
	 *
	 * @param mixed $value Value to test.
	 */
	public function test_floor_when_passing_a_non_number_like_string( $value ) {
		$actual = NumberUtil::floor( $value );
		$this->assertFalse( $actual );
	}

	/**
	 * @testdox `floor` should return 1 when passing the boolean 'true'.
	 */
	public function test_floor_when_passing_the_boolean_true() {
		$actual   = NumberUtil::floor( true );
		$expected = 1;
		$this->assertEquals( $expected, $actual );
	}
}
