...
async function selectVariation(
	...
	await expect(
		page.getByRole( 'alert' ).filter( { hasText: price.toString() } )
	).toBeVisible();
	...
	() => {
		...
		test.beforeAll( async ( { restApi } ) => {
			...
			await restApi
				...
				.then( async ( response: ProductResponse ) => {
	...
	() => {
		...
		test(
			...
			async ( { page } ) => {
				...
				let totalPrice: string | number | null = await page
