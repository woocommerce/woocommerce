...
test.describe(
	...
	() => {
		async function navigate(
			...
			await baseTest.step( 'Navigate to product edit page', async () => {
				await page.goto(
					`wp-admin/post.php?post=${ productId }&action=edit`
				);
			} );

			await baseTest.step( 'go to Linked Products', async () => {
				await expect( async () => {
					await page
		...
		async function updateProduct( page: Page ): Promise< void > {
			await baseTest.step( 'update the product', async () => {
