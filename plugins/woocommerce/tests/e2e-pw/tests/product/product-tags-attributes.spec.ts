...
test.describe(
	...
	() => {
		...
		test.afterAll( async ( { restApi } ) => {
			...
			const pages: PagesResponse = await restApi.get(
				`${ WP_API_PATH }/pages`
			);

			for ( const wpPage of pages.data ) {
				if ( wpPage.title.rendered === pageTitle ) {
					await restApi.delete(
						`${ WP_API_PATH }/pages/${ wpPage.id }`,
						{
							data: {
								force: true,
							},
		...
		test( 'should see shop catalog with all its products', async ( {
			...
			for ( let i = 0; i < ( await productPrice.count() ); ++i )
