...
test.describe(
	...
	() => {
		...
		test( 'can manage cross-sell products and maximum item quantity', async ( {
			...
			await expect(
				page.getByLabel(
					`Remove ${ productName } cross-sell 1 from cart`
				)
			).toBeVisible();
			await page
			...
			await expect(
				page.getByLabel(
					`Remove ${ productName } cross-sell 2 from cart`
				)
			).toBeVisible();
			...
			await expect(
				page.locator( '.order-total .amount' )
			).toContainText( `$${ fourProductPrice }` );
			...
			await expect(
				page.getByLabel(
					`Add to cart: “${ productName } cross-sell 1”`
				)
			).toBeVisible();
			await expect(
				page.getByLabel(
					`Add to cart: “${ productName } cross-sell 2”`
				)
			).toBeVisible();
		} );
	}
