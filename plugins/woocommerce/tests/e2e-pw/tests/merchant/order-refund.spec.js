...
test.describe.serial('WooCommerce Orders > Refund an order', () => {
	...
	test('can issue a refund by quantity', async ({
		page
	}) => {
		...
		await expect(page.locator('.do-manual-refund')).toContainText(
			`${getTextForLanguage()['Refund']} ${ currencySymbol }9.99 ${getTextForLanguage()['manually']}`
		);
		...
		await expect(page.locator('.system-note >> nth=0')).toContainText(
			`${getTextForLanguage()['OrderstatuschangedfromCompletedtoRefunded']}`
		);
	});

	// this test relies on the previous test, so should refactor
	test('can delete an issued refund', async ({
		page
	}) => {
		await page.goto(`wp-admin/post.php?post=${ orderId }&action=edit`);
...
test.describe('WooCommerce Orders > Refund and restock an order item', () => {
	...
	test('can update order after refunding item without automatic stock adjustment', async ({
		...
		await expect(page.locator('.system-note >> nth=1')).toContainText(
			getTextForLanguage()['StocklevelsreducedProductwithstock10to8Regex']
		);
		...
		await expect(page.locator('div.updated.notice-success')).toContainText(
			`${getTextForLanguage()['Orderupdated']}`
		);

		// Verify that inventory wasn't modified.
		expect(
			await page.$$eval('.note', (notes, pattern) =>
				notes.every((note) => !note.textContent.match(pattern)),
				getTextForLanguage()['AdjustedstockProductwithStock10to8Regex'])
		).toEqual(true);
	});
});
