...
test.describe.serial( 'Import Products from a CSV file', () => {
	...
	test( 'should show error message if you go without providing CSV file', async ( {
		...
		await page.click( `button[value=${getTranslationFor('"Continue"')}]` );
	...
	test( 'can upload the CSV file and import products', async ( { page } ) => {
		...
		await page.click( `button[value=${getTranslationFor('"Run the importer"')}]` );
		...
		// View the products
		await page.click( `text=${getTranslationFor('View products')}` );
