...
test.describe( 'Store owner can complete the core profiler', () => {
	...
	test( 'Can complete the core profiler skipping extension install', async( { page } ) => {
		await page.goto( 'wp-admin/admin.php?page=wc-admin&path=%2Fsetup-wizard' );

		await test.step( 'Intro page and opt in to data sharing', async() => {
			await expect( page.getByRole( 'heading', { name: 'Welcome to Woo!' } ) ).toBeVisible();
			await page.getByRole( 'checkbox', { name: 'I agree to share my data' } ).check();
			await page.getByRole( 'button', { name: 'Set up my store' } ).click();
		} );

		await test.step( 'User profile information', async() => {
			await expect( page.getByRole( 'heading', { name: 'Which one of these best describes you?' } ) ).toBeVisible();
			await page.getByRole( 'radio', { name: 'I\'m just starting my business' } ).first().click();
			await page.getByRole( 'button', { name: 'Continue' } ).click();
		} );

		await test.step( 'Business Information', async() => {
			await expect( page.getByRole( 'heading', { name: 'Tell us a bit about your store' } ) ).toBeVisible();
			await expect( page.getByPlaceholder( 'Ex. My awesome store' ) ).toHaveValue( 'WooCommerce Core E2E Test Suite' );
			await page.locator( 'form.woocommerce-profiler-business-information-form > div > div > div > div > input' ).first().click();
			...
			await page.locator( 'form.woocommerce-profiler-business-information-form > div > div > div > div > input' ).last().click();
			await page.getByRole( 'option', { name: 'Australia — Northern Territory' } ).click();

			await page.getByPlaceholder( 'wordpress@example.com' ).fill( 'merchant@example.com' );
			await page.getByLabel( 'Opt-in to receive tips,' ).uncheck();
			await page.getByRole( 'button', { name: 'Continue' } ).click();
		} );

		await test.step( 'Extensions -- do not install any', async() => {
			await expect( page.getByRole( 'heading', { name: 'Get a boost with our free features' } ) ).toBeVisible();
			// check that WooPayments is displayed because Australia is a supported country
			await expect( page.getByRole( 'heading', { name: 'Get paid with WooPayments' } ) ).toBeVisible();
		...
		await test.step( 'Confirm that core profiler was completed and no extensions installed', async() => {
			...
			await expect( page.getByRole( 'heading', { name: 'Turning on the lights' } ) ).toBeVisible();
			await expect( page.locator( '.woocommerce-onboarding-progress-bar__filler' ) ).toBeVisible();
			// dashboard shown
			await expect( page.getByRole( 'heading', { name: 'Welcome to WooCommerce Core E2E Test Suite' } ) ).toBeVisible();
			await expect( page.getByText( 'Add products to sell' ) ).toBeVisible();
	...
	test( 'Can complete the core profiler installing default extensions', async( { page } ) => {
		...
		await test.step( 'User profile information', async() => {
			await expect( page.getByRole( 'heading', { name: 'Which one of these best describes you?' } ) ).toBeVisible();
			await page.getByRole( 'radio', { name: 'I\'m already selling' } ).first().click();
			await page.getByLabel( 'Select an option' ).click();
			await page.getByRole( 'option', { name: 'No, I\'m selling offline' } ).click();
			await page.getByRole( 'button', { name: 'Continue' } ).click();
		} );
		...
		await test.step( 'Business Information', async() => {
			...
			await page.getByRole( 'option', { name: 'Food and drink' } ).click();
		...
		await test.step( 'Extensions -- install some suggested extensions', async() => {
			...
			await expect( page.getByRole( 'heading', { name: 'Get paid with WooPayments' } ) ).not.toBeAttached();
		...
		await test.step( 'Confirm that core profiler was completed and a couple of default extensions installed', async() => {
			...
			await expect( page.getByText( 'Pinterest for WooCommerce', { exact: true } ) ).toBeVisible();
...
test.describe( 'Store owner can skip the core profiler', () => {
	...
	test( 'Can click skip guided setup', async( { page } ) => {
		...
		await expect( page.getByRole( 'heading', { name: 'Where is your business located?' } ) ).toBeVisible();
		await page.getByLabel( 'Select country/region' ).click();
		await page.getByRole( 'option', { name: 'United States (US) — California' } ).click();
