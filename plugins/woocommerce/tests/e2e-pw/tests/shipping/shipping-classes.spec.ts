...
interface ShippingClassTestCase {
	name: string;
	slug: string;
	expectedSlug: string;
	description: string;
...
const testCases: ShippingClassTestCase[] = [
	{
		name: `Small Items ${ uq }`,
...
testCases.forEach( ( { testTitle, name, slug, expectedSlug, description } ) => {
	const test = baseTest.extend( {
		...
		page: async ( { restApi, page }, use ) => {
			...
			for ( const shippingClass of allShippingClasses.data as ShippingClass[] ) {
