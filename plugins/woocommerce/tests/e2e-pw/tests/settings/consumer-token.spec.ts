...
/**
 * Response interface for product creation.
 */
...
/**
 * Response interface for product read.
 */
...
interface ApiClient {
	...
	get: ( endpoint: string ) => Promise< ProductReadResponse >;
}
...
test( 'admin can manage consumer keys', async ( { page } ) => {
	...
	let key = '';
	let secret = '';
	const testProduct: TestProduct = getFakeProduct();
	...
	const apiClient: ApiClient = createClient( playwrightConfig.use.baseURL, {
		type: 'oauth1',
		consumerKey: key,
	...
	await test.step( 'can use the consumer key', async () => {
		...
		expect( readResponse.data.id ).toBe( testProduct.id );
		expect( readResponse.data.name ).toBe( testProduct.name );
	} );
