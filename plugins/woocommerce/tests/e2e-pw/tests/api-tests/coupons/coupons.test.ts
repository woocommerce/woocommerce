...
interface BatchResponseJSON {
	create?: { id: number; code: string }[];
	update?: { id: number; free_shipping?: boolean; amount?: string }[];
	delete?: { id: number }[];
}
...
test.describe( 'Batch update coupons', () => {
	 ...
	 */
	const expectedCoupons: CouponData[] = [
		{
	...
	test( 'can batch create coupons', async ( { request } ) => {
		...
		const batchCreatePayload: BatchCreatePayload = {
		...
		const batchCreateResponseJSON: BatchResponseJSON =
			await batchCreateResponse.json();
	...
	test( 'can batch update coupons', async ( { request } ) => {
		...
		const batchUpdatePayload: BatchUpdatePayload = {
			update: [
				{
		...
		const batchUpdateResponseJSON: BatchResponseJSON =
			await batchUpdateResponse.json();
	...
	test( 'can batch delete coupons', async ( { request } ) => {
		...
		const batchDeletePayload: BatchDeletePayload = {
		...
		const batchDeletePayloadJSON: BatchResponseJSON =
		...
		for ( const deletedCouponId of couponIdsToDelete ) {
			//Call the API to attempte to retrieve the coupons
			const response = await request.get(
				`wp-json/wc/v3/coupons/${ deletedCouponId }`
			);
...
test.describe( 'List coupons', () => {
	...
	test.beforeAll( async ( { request } ) => {
		...
		const responseJSON: BatchResponseJSON = await response.json();
		...
		for ( const thisCoupon of allCoupons ) {
			...
			const foundCoupon = actualCreatedCoupons!.find(
				( { code } ) => thisCoupon.code === code
			);
			//Set the id on the current coupon
			thisCoupon.id = foundCoupon!.id;
		}
	} );
	...
	test( 'can list all coupons by default', async ( { request } ) => {
		...
		const actualCouponIdsList = listedCoupons.map(
			( { id }: { id: number } ) => id
		);
