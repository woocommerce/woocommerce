...
test.describe( 'Refunds API tests', () => {
	...
	test( 'can create a refund', async ( { request } ) => {
		...
		const responseJSON: RefundResponse = await response.json();
		...
		const getOrderResponseJSON: OrderResponse =
	...
	test( 'can retrieve a refund', async ( { request } ) => {
		...
		const refundJSON: RefundResponse = await refundCreateResponse.json();
		const refundGetResponse = await request.get(
			`./wp-json/wc/v3/orders/${ orderId }/refunds/${ refundJSON.id }`
		);
		const responseJSON: RefundResponse = await refundGetResponse.json();
	...
	test( 'can retrieve refund info from refund endpoint', async ( {
		...
		const responseJSON: RefundResponse[] = await refundsListResponse.json();
	...
	test( 'can delete a refund', async ( { request } ) => {
		...
		const responseJSON: RefundResponse = await refundDeleteResponse.json();
		...
		const retrieveOrderResponseJSON: OrderResponse =
			await orderRetrieveResponse.json();
