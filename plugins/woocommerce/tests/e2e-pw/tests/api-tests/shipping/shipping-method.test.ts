...
/**
 * Shipping method ID type.
 */
type MethodId = 'free_shipping' | 'flat_rate' | 'local_pickup';

/**
 * Data table for shipping methods.
 */
const shippingMethods: Array< [ string, MethodId, string | undefined ] > = [
...
test.describe( 'Shipping methods API tests', () => {
	...
	for ( const shippingMethodRow of shippingMethods ) {
		test( `can add a ${ shippingMethodRow[ methodTitleIndex ] } shipping method`, async ( {
			...
			const cost = shippingMethodRow[ methodCostIndex ];
			const shippingMethod = getShippingMethodExample(
				shippingMethodRow[ methodIdIndex ],
				cost !== undefined ? parseInt( cost, 10 ) : undefined
			);
