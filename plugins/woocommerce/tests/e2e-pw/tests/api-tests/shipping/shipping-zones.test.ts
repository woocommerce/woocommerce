...
import type { ShippingZone } from '../../../data';

const { BASE_URL } = process.env;
const shouldSkip = BASE_URL !== undefined;

/**
 * Extended shipping zone type with optional id property.
 */
interface ShippingZoneWithId extends ShippingZone {
	id?: number;
}
...
test.describe.serial( 'Shipping zones API tests', () => {
	...
	const shippingZone: ShippingZoneWithId = getShippingZoneExample();
	...
	test( 'cannot delete the default shipping zone "Locations not covered by your other zones"', async ( {
		...
		const ids = responseJSON.map( ( x: { id: number } ): number => x.id );

		// for each id, delete the related shipping zone
		for ( const id of ids ) {
			// call API to delete each shipping zone
			await request.delete( `./wp-json/wc/v3/shipping/zones/${ id }`, {
				data: { force: true },
			} );
		}
