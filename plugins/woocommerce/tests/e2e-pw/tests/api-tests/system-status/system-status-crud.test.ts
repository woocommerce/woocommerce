...
const expect = baseExpect.extend( {
	 ...
	 */
	anyOf( this: any, received: unknown, expectedTypes: string[] ): any {
		...
		try {
			...
			const error = e as { matcherResult?: { actual?: unknown } };
...
const anyOf = ( expectedTypes: string[] ): any =>
	( expect as any ).anyOf( expectedTypes );
const { any, anything } = expect;

/* eslint-enable @typescript-eslint/no-explicit-any */

/**
 * Schema field definition type.
 */
...
/**
 * Schemas for validating system status API responses.
 */
...
const otherTableSuffixes: string[] = [
...
/**
 * Returns an array of expected WooCommerce table names with the given database prefix.
 *
 * @param dbPrefix - The database prefix
 * @return Array of expected WooCommerce table names
 */
const getExpectedWooCommerceTables = ( dbPrefix: string ): string[] => {
	return [
...
/* eslint-disable playwright/no-nested-step, @typescript-eslint/no-use-before-define */
test.describe( 'System Status API tests', () => {
	test( 'can view all system status items', async ( { request } ) => {
		let responseJSON: Record< string, unknown >;
		let databasePrefix: string;
		let databaseSize: { data: number; index: number };
		let databaseTables: {
			woocommerce: Record<
				string,
				{ data: string; index: string; engine: string }
			>;
			other: Record<
				string,
				{ data: string; index: string; engine: string }
		...
		let woocommerceTables: Record<
			string,
			{ data: string; index: string; engine: string }
		>;
		let otherTables: Record<
			string,
			{ data: string; index: string; engine: string }
		>;
		let activePlugins: Array< Record< string, unknown > >;
		let dropinsMuPlugins: {
			dropins: Array< { name: string; plugin: string } >;
			mu_plugins: Array< unknown >;
		};
		let taxonomiesJSON: Record< string, string >;
		let productVisibilityTerms: Record< string, string >;
		...
		await test.step( 'Verify "environment" fields', async () => {
			const { environment } = responseJSON as {
			...
			await test.step( 'Verify "environment.external_object_cache"', () => {
				const { external_object_cache } = environment as {
		...
		await test.step( 'Verify "database" fields', async () => {
			const { database } = responseJSON as {
			...
			databaseSize = database.database_size as {
				data: number;
				index: number;
			};
			databasePrefix = database.database_prefix as string;
			databaseTables = database.database_tables as typeof databaseTables;
		} );
		...
		await test.step( 'Verify "database.database_tables.woocommerce" fields', async () => {
			...
			for ( const tableName of wooTableNames ) {
				...
				for ( const {
					...
					expect(
						thisTable[ field as keyof typeof thisTable ],
		...
		await test.step( 'Verify "database.database_tables.other" fields', async () => {
			...
			for ( const suffix of otherTableSuffixes ) {
				...
				const thisTable = otherTables[ tableName as string ];
				expect(
					thisTable,
		...
		await test.step( 'Verify "active_plugins"', async () => {
			const { active_plugins } = responseJSON as {
				active_plugins: Array< Record< string, unknown > >;
			};
			expect(
				active_plugins,
		...
		await test.step( 'Verify "dropins_mu_plugins"', async () => {
			const { dropins_mu_plugins } = responseJSON as {
				dropins_mu_plugins: typeof dropinsMuPlugins;
			};
		...
		await test.step( 'Verify "settings"', async () => {
			const { settings } = responseJSON as {
			...
			taxonomiesJSON = settings.taxonomies as Record< string, string >;
			productVisibilityTerms =
				settings.product_visibility_terms as Record< string, string >;
		} );
		...
		await test.step( 'Verify "security" fields', async () => {
			const { security } = responseJSON as {
		...
		await test.step( 'Verify "post_type_counts" array', async () => {
			const { post_type_counts } = responseJSON as {
				post_type_counts: Array< { type: string; count: string } >;
			};
