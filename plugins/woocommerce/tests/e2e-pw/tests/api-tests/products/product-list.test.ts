...
interface ShippingClasses {
	freightJSON: ShippingClass;
}

interface TaxClasses {
	reducedRateJSON?: TaxClass;
}

interface VariableProducts {
	hoodieJSON: Product;
	hoodieVariations: Product[];
	vneckJSON: Product;
	vneckVariations: Product[];
}
...
test.describe( 'Products API tests: List All Products', () => {
	...
	test.beforeAll( async ( { request } ) => {
		const createSampleCategories = async (): Promise< Categories > => {
			...
			const clothingJSON: Category = await clothing.json();
			...
			const accessoriesJSON: Category = await accessories.json();
			...
			const hoodiesJSON: Category = await hoodies.json();
			...
			const tshirtsJSON: Category = await tshirts.json();
			...
			const decorJSON: Category = await decor.json();
			...
			const musicJSON: Category = await music.json();
		...
		const createSampleAttributes = async (): Promise< Attributes > => {
			...
			const colorJSON: Attribute = await color.json();
			...
			const sizeJSON: Attribute = await size.json();
			...
			return {
				colorJSON,
				colors: colorsJSON.create as AttributeTerm[],
				sizeJSON,
				sizes: sizesJSON.create as AttributeTerm[],
			};
		};

		const createSampleTags = async (): Promise< Tags > => {
			const cool = await request.post( './wp-json/wc/v3/products/tags', {
				data: {
					name: `Cool${ RAND_NUM }`,
				},
			} );
			const coolJSON: Tag = await cool.json();
		...
		const createSampleShippingClasses =
			async (): Promise< ShippingClasses > => {
		...
		const createSampleTaxClasses = async (): Promise< TaxClasses > => {
			...
			if ( reducedRateStatus === 404 ) {
				...
				const reducedRateJSON: TaxClass = await reducedRate.json();
		...
		const createSampleSimpleProducts = async (
			...
			return simpleProductsJSON.create as Product[];
		};
		...
		const createSampleExternalProducts = async (
			...
			return externalProductsJSON.create as Product[];
		};
		...
		const createSampleGroupedProduct = async (
			...
			const logoProductsJSON: { id: number }[] =
			...
			return groupedProductsJSON.create as Product[];
		};

		const createSampleVariableProducts = async (
			categories: Categories,
			attributes: Attributes
		): Promise< VariableProducts > => {
			const description =
				'<p>Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. ' +
				'Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. ' +
				'Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo.</p>\n';
			...
			const hoodieJSON: Product = await hoodie.json();
			...
			const vneckJSON: Product = await vneck.json();
			...
			return {
				hoodieJSON,
				hoodieVariations: hoodieVariationsJSON.create as Product[],
				vneckJSON,
				vneckVariations: vneckVariationsJSON.create as Product[],
			};
		};
		...
		const createSampleHierarchicalProducts =
			async (): Promise< HierarchicalProducts > => {
				...
				const parentJSON: Product = await parent.json();
				...
				const childJSON: Product = await child.json();
		...
		const createSampleProductReviews = async (
			...
			return [
				review1JSON.id as number,
				review2JSON.id as number,
				review3JSON.id as number,
			];
		};
		...
		const createSampleProductOrders = async (
			...
			const orderJSON: Order = await order.json();
			return [ orderJSON ];
		};
	...
	test.afterAll( async ( { request } ) => {
		const deleteSampleData = async (
			...
			const productIds: number[] = []
				.concat( simpleProducts.map( ( p ) => p.id ) as never[] )
				.concat( externalProducts.map( ( p ) => p.id ) as never[] )
				.concat( groupedProducts.map( ( p ) => p.id ) as never[] )
				.concat( [
	...
	test.describe( 'List all products', () => {
		...
		test( 'pagination', async ( { request } ) => {
			...
			const page1JSON: Product[] = await page1.json();
			...
			const page2JSON: Product[] = await page2.json();
			...
			const page2OffsetJSON: Product[] = await page2Offset.json();
			...
			const lastPageJSON: Product[] = await lastPage.json();
			...
			const page6JSON: Product[] = await page6.json();
		...
		test( 'search', async ( { request } ) => {
			...
			const result1JSON: Product[] = await result1.json();
			...
			const result2JSON: Product[] = await result2.json();
		...
		test( 'inclusion / exclusion', async ( { request } ) => {
			...
			const allProductsJSON: Product[] = await allProducts.json();
			...
			const includedJSON: Product[] = await included.json();
			...
			const excludedJSON: Product[] = await excluded.json();
		...
		test( 'type', async ( { request } ) => {
			...
			const result3JSON: Product[] = await result3.json();
			...
			const result4JSON: Product[] = await result4.json();
