...
interface VariableProduct {
	hoodieJSON: SimpleProduct;
	hoodieVariations: Variation[];
	vneckJSON: SimpleProduct;
	vneckVariations: Variation[];
}

/**
 * Hierarchical products interface.
 */
interface HierarchicalProducts {
	parentJSON: SimpleProduct;
	childJSON: SimpleProduct;
}

/**
 * Test product data interface.
 */
interface TestProductData {
	categories: {
		clothingJSON: Category;
		accessoriesJSON: Category;
		hoodiesJSON: Category;
		tshirtsJSON: Category;
		decorJSON: Category;
		musicJSON: Category;
	};
	attributes: {
	...
	shippingClasses: {
		freightJSON: ShippingClass;
	};
	simpleProducts: SimpleProduct[];
	externalProducts: SimpleProduct[];
	groupedProducts: SimpleProduct[];
...
interface SampleData {
	...
	orders: OrderResponse[];
	precisionOrder: OrderResponse;
	hierarchicalOrders: {
		parent: OrderResponse;
		child: OrderResponse;
	};
	guestOrderJSON: OrderResponse;
	testProductData: TestProductData;
	couponJSON: Coupon;
}

/**
 * Mutable order interface for test modifications.
 */
interface MutableOrder extends Order {
	billing: CustomerBilling;
	shipping: CustomerShipping;
}
 ...
 */
const updatedCustomerBilling: CustomerBilling = {
	first_name: 'Jane',
	last_name: 'Doe',
	company: 'Automattic',
	country: 'US',
 ...
 */
const updatedCustomerShipping: CustomerShipping = {
	first_name: 'Mike',
	last_name: 'Anderson',
	company: 'Automattic',
	country: 'US',
...
test.describe.serial( 'Orders API tests', () => {
	...
	test.beforeAll( async ( { request } ) => {
		const createSampleCategories = async (): Promise<
		...
		> => {
			...
			const categoriesJSON: { create: Category[] } =
			...
			const subCategoriesJSON: { create: Category[] } =
				await subCategories.json();
		...
		const createSampleAttributes = async (): Promise<
			TestProductData[ 'attributes' ]
		> => {
			// Create attributes
			const color = {
			...
			const attributesJSON: { create: Attribute[] } =
			...
			const colorsJSON: { create: AttributeTerm[] } = await colors.json();
			...
			const sizesJSON: { create: AttributeTerm[] } = await sizes.json();
		...
		const createSampleTags = async (): Promise<
			TestProductData[ 'productTags' ]
		> => {
			...
			const coolJSON: ProductTag = await cool.json();
		...
		const createSampleShippingClasses = async (): Promise<
			TestProductData[ 'shippingClasses' ]
		> => {
			...
			const freightJSON: ShippingClass = await freight.json();
		...
		const createSampleSimpleProducts = async (
			...
			attributes: TestProductData[ 'attributes' ],
			productTags: TestProductData[ 'productTags' ]
			...
			const simpleProductsJSON: { create: SimpleProduct[] } =
				await simpleProducts.json();

			return simpleProductsJSON.create;
		};

		const createSampleExternalProducts = async (
			categories: TestProductData[ 'categories' ]
		): Promise< SimpleProduct[] > => {
			...
			const externalProductsJSON: { create: SimpleProduct[] } =
				await externalProducts.json();

			return externalProductsJSON.create;
		};

		const createSampleGroupedProduct = async (
			categories: TestProductData[ 'categories' ]
		): Promise< SimpleProduct[] > => {
			...
			const logoProductsJSON: Array< { id: number } > =
				await logoProducts.json();
			...
			const groupedProductsJSON: { create: SimpleProduct[] } =
				await groupedProducts.json();
		...
		const createSampleVariableProducts = async (
			...
			const hoodieJSON: SimpleProduct = await hoodie.json();
			...
			const hoodieVariationsJSON: { create: Variation[] } =
				await hoodieVariations.json();
			...
			const vneckJSON: SimpleProduct = await vneck.json();
			...
			const vneckVariationsJSON: { create: Variation[] } =
				await vneckVariations.json();
		...
		const createSampleHierarchicalProducts =
			async (): Promise< HierarchicalProducts > => {
				...
				const parentJSON: SimpleProduct = await parent.json();
				...
				const childJSON: SimpleProduct = await child.json();
		...
		const createSampleProductReviews = async (
			simpleProducts: SimpleProduct[]
		): Promise< number[] > => {
			...
			const review1JSON: { id: number } = await review1.json();
			...
			const review2JSON: { id: number } = await review2.json();
			...
			const review3JSON: { id: number } = await review3.json();
		...
		const createSampleProductOrders = async (
			simpleProducts: SimpleProduct[]
			...
			const orderJSON: OrderResponse = await order1.json();

			return [ orderJSON ];
		};

		const productsTestSetupCreateSampleData =
			async (): Promise< TestProductData > => {
		...
		const createSampleData = async (): Promise< SampleData > => {
			...
			const johnJSON: Customer = await john.json();
			...
			const tinaJSON: Customer = await tina.json();
			...
			for ( const product of Object.values( orderedProducts ) ) {
				...
				const orderJSON: OrderResponse = await order2.json();
			...
			const order2JSON: OrderResponse = await order2.json();
			...
			const order3JSON: OrderResponse = await order3.json();
			...
			const guestOrderJSON: OrderResponse = await guestOrder.json();
			...
			const couponJSON: Coupon = await coupon.json();
			...
			const order4JSON: OrderResponse = await order4.json();
	...
	test.afterAll( async ( { request } ) => {
		const productsTestSetupDeleteSampleData = async (
	...
	test( 'can add shipping and billing contacts to an order', async ( {
		...
		const mutableOrder = order as unknown as MutableOrder;
		mutableOrder.billing = updatedCustomerBilling;
		mutableOrder.shipping = updatedCustomerShipping;
	...
	test.describe( 'List all orders', () => {
		...
		test( 'pagination', async ( { request } ) => {
			...
			const page1JSON: OrderResponse[] = await page1.json();
			...
			const page2JSON: OrderResponse[] = await page2.json();
			...
			const page2OffsetJSON: OrderResponse[] = await page2Offset.json();
			...
			const lastPageJSON: OrderResponse[] = await lastPage.json();
			...
			const page6JSON: OrderResponse[] = await page6.json();
		...
		test( 'inclusion / exclusion', async ( { request } ) => {
			...
			const allOrdersJSON: OrderResponse[] = await allOrders.json();
			...
			const includedJSON: OrderResponse[] = await included.json();
			...
			const excludedJSON: OrderResponse[] = await excluded.json();
		...
		test( 'parent', async ( { request } ) => {
			...
			const result1JSON: OrderResponse[] = await result1.json();
			...
			const result2JSON: OrderResponse[] = await result2.json();
		...
		test( 'dp (precision)', async ( { request } ) => {
			...
			const result3JSON: OrderResponse = await result3.json();
		...
		test( 'search', async ( { request } ) => {
			...
			const result4JSON: OrderResponse[] = await result4.json();
			...
			const result5JSON: OrderResponse[] = await result5.json();
	...
	test.describe( 'orderby', () => {
		...
		test( 'default', async ( { request } ) => {
			...
			const resultJSON: OrderResponse[] = await result.json();
