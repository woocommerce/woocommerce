...
import type { OrderExample } from '../../../data';
import type { CustomerShipping } from '../../../data/shared/customer';

/**
 * Extended shipping interface with phone property.
 */
interface ExtendedShipping extends CustomerShipping {
	company: string;
	phone: string;
}

/**
 * Order with extended shipping type.
 */
interface OrderWithExtendedShipping
	extends Omit< OrderExample, 'shipping' | 'line_items' > {
 ...
 */
const order: OrderWithExtendedShipping = {
	...
	fee_lines: [],
	coupon_lines: [],
	line_items: [ { product_id: undefined } ],
};
...
test.describe( 'Order Search API tests', () => {
	test.beforeAll( async ( { request } ) => {
		...
		const productResponseJSON: { id: number } =
		...
		const responseJSON: OrderResponseJSON = await response.json();
