...
import type { USTaxRate } from '../../../data';

const { BASE_URL } = process.env;
const shouldSkip = BASE_URL !== undefined;

/**
 * Extended US tax rate type with optional id property.
 */
interface USTaxRateWithId extends USTaxRate {
	id?: number;
}
...
test.describe.serial( 'Tax Rates API tests: CRUD', () => {
	...
	// Cast to extended type to allow id assignment
	const taxRatesData = allUSTaxesExample as USTaxRateWithId[];
	...
	test.describe( 'Batch tax rate operations', () => {
		...
		test( 'can batch create tax rates', async ( { request } ) => {
			...
			expect( actualTaxRates ).toHaveLength( taxRatesData.length );

			for ( let i = 0; i < actualTaxRates.length; i++ ) {
				const { id, rate } = actualTaxRates[ i ];

				expect( id ).toBeDefined();
				expect( rate ).toEqual( taxRatesData[ i ].rate );

				// Save the tax rate id
				taxRatesData[ i ].id = id;
			}
		} );
		...
		test( 'can batch update tax rates', async ( { request } ) => {
			...
			const batchUpdatePayload = {
				update: [
					{
						id: taxRatesData[ 0 ].id,
						rate: '4.1111',
					},
					{
						id: taxRatesData[ 1 ].id,
						order: 49,
						rate: '5.6111',
					},
				],
		...
		test( 'can batch delete tax rates', async ( { request } ) => {
			...
			const taxRateIdsToDelete = taxRatesData.map(
