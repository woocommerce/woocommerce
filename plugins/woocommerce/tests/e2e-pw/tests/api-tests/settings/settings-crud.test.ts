...
const enableEmailImprovementsFeature = async (): Promise< void > => {
	await setOption(
		apiRequest,
		BASE_URL as string,
...
const disableEmailImprovementsFeature = async (): Promise< void > => {
	await setOption(
		apiRequest,
		BASE_URL as string,
		'woocommerce_feature_email_improvements_enabled',
...
test.describe( 'Settings API tests: CRUD', () => {
	...
	// NOTE: The remaining email settings tests are omitted for brevity
