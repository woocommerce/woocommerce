...
const test = baseTest.extend< UsersCreateFixtures >( {
...
for ( const userData of users ) {
	test( `can create a new ${ userData.role }`, async ( { page, user } ) => {
		...
		await test.step( 'create a new user', async () => {
			...
			( user as User ).password = await page
				.locator( '#pass1' )
				.inputValue();
			...
			( user as User ).id =
				new URLSearchParams( new URL( page.url() ).search ).get(
					'id'
				) ?? undefined;
		} );
