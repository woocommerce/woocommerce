...
interface UsersManageFixtures {
	user: WpUser;
	customer: WpUser;
	manager: WpUser;
}

interface UpdatedCustomerBilling {
	first_name: string;
	last_name: string;
	company: string;
	country: string;
...
interface UpdatedCustomer {
	...
	nick_name: string;
	email: string;
	billing: UpdatedCustomerBilling;
}

const test = baseTest.extend< UsersManageFixtures >( {
	storageState: ADMIN_STATE_PATH,
	user: async ( { restApi }, use ) => {
		let user: WpUser | undefined;
		await restApi
			.post( `${ WP_API_PATH }/users`, getFakeUser( 'subscriber' ) )
			.then( ( response: { data: WpUser } ) => {
				user = response.data;
			} );

		await use( user as WpUser );
	...
	customer: async ( { restApi, user }, use ) => {
		...
		await use( customer as WpUser );
	},
	manager: async ( { restApi, user }, use ) => {
		let manager: WpUser | undefined;
		await restApi
			.put( `${ WP_API_PATH }/users/${ user.id }`, {
				roles: [ 'shop_manager' ],
			} )
		...
		await use( manager as WpUser );
	},
} );

async function userDeletionTest(
	page: Page,
	username: string
): Promise< boolean > {
...
test( `can update customer data`, async ( { page, customer } ) => {
	...
	const now = Date.now();
	const updatedCustomer: UpdatedCustomer = {
