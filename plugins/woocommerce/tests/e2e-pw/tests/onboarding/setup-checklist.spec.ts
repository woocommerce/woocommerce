...
interface TaskListOptions {
	woocommerce_task_list_hidden: string;
}
...
const test = baseTest.extend< {
	nonSupportedWooPaymentsCountryPage: Page;
} >( {
	storageState: ADMIN_STATE_PATH,

	page: async ( { page, restApi }, use ) => {
		const initialTaskListState: { data: TaskListOptions } =
			await restApi.get(
	...
	nonSupportedWooPaymentsCountryPage: async ( { page, restApi }, use ) => {
		...
		const initialDefaultCountry: { data: SettingValue } = await restApi.get(
