...
async function runComingSoonTests( themeContext = '' ): Promise< void > {
...
test.describe( 'Launch Your Store front end - logged out', () => {
	...
	test.describe( 'Block Theme (Twenty Twenty Four)', async () => {
		test.beforeAll( async ( { baseURL } ) => {
			await activateTheme( baseURL as string, 'twentytwentyfour' );
		} );

		test.afterAll( async ( { baseURL } ) => {
			// Reset theme to the default.
			await activateTheme( baseURL as string, DEFAULT_THEME );
		} );
	...
	test.describe( 'Classic Theme (Storefront)', async () => {
		test.beforeAll( async ( { baseURL } ) => {
			await activateTheme( baseURL as string, 'storefront' );
		} );
