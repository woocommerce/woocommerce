...
/**
 * Install response from WC Beta Tester.
 */
...
/**
 * Deactivate WooCommerce using the WC Admin Test Helper.
 *
 * @param restApi - REST API client instance
 */
async function deactivateWooCommerce(
...
/**
 * Get the currently activated WooCommerce version.
 *
 * @param restApi - REST API client instance
 * @return The WooCommerce version or undefined if not found
 */
async function getActivatedWooCommerceVersion(
	restApi: RestApiClient
): Promise< string | undefined > {
	const response = await restApi.get( 'wp/v2/plugins', { status: 'active' } );
	const plugins = ( await response.data ) as PluginInfo[];
	return plugins.find( ( plugin ) => plugin.name === 'WooCommerce' )?.version;
}

setup(
	'Install WC using WC Beta Tester',
	async ( { restApi } ): Promise< void > => {
		setup.skip(
		...
		const activatedWcVersion = await getActivatedWooCommerceVersion(
			restApi
		);
		...
		if ( wcVersion === 'latest' ) {
			...
			if ( latestResponse.statusCode !== 200 ) {
				throw new Error(
					`Failed to install latest WC: ${ latestResponse.statusCode }`
				);
			}

			resolvedVersion =
				( ( await latestResponse.data ) as InstallResponse )?.version ||
				'';
			...
			try {
				...
				console.error(
					`Error installing WC version ${ wcVersion }:`,
					err
				);
			}
		}
