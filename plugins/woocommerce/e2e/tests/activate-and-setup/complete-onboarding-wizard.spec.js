...
test.describe('Store owner can complete onboarding wizard', () => {
	...
	test('can unselect all business features and continue', async ({ page }) => {
		...
		await page.click('button.woocommerce-admin__business-details__selective-extensions-bundle__expand');
		// Check to see if WC Payments is present
		const wcPay = await page.locator('a:has-text("WooCommerce Payments")');
	...
	test('can complete the theme selection section', async ({ page }) => {
		await page.goto('wp-admin/admin.php?page=wc-admin&path=%2Fsetup-wizard&step=theme');
		const pageHeading = await page.textContent('div.woocommerce-profile-wizard__step-header > h2');
...
test.describe('A japanese store can complete the selective bundle install but does not include WCPay.', () => {
	...
	test.beforeEach(async ({ page }) => {
		...
		await page.fill('#inspector-text-control-1', 'addr 2');
		await page.click('#woocommerce-select-control-0__control-input');
		await page.fill('#woocommerce-select-control-0__control-input', 'Japan — Hokkaido');
	...
	test('can choose not to install any extensions', async ({ page }) => {
		...
		await page.click('#tab-panel-0-business-features');
		const pageHeading = await page.textContent('div.woocommerce-profile-wizard__step-header > h2');
		await expect(pageHeading).toContain('Included business features');
		// Expand list of features
		await page.click('button.woocommerce-admin__business-details__selective-extensions-bundle__expand');
		// Make sure WC Payments is NOT present
		await expect(page.locator('a:has-text("WooCommerce Payments")')).toHaveCount(0);
		// Uncheck all business features
		if (page.isChecked('#inspector-checkbox-control-1')) {
			await page.click('#inspector-checkbox-control-1');
		};
	...
	test('should display the choose payments task, and not the WC Pay task', async ({ page }) => {
		...
		const modalHeading = await page.$('h2.woocommerce__welcome-modal__page-content__header');
		if (modalHeading) {
			await expect(modalHeading).toContain('Welcome to your WooCommerce store’s online HQ!');
			await page.click('[aria-label="Close dialog"]');
		};
		const listItem = await page.textContent(':nth-match(li[role=button], 3)');
