...
test.describe('Analytics pages', () => {
	...
	test('a user should see 3 sections by default - Performance, Charts, and Leaderboards', async ({ page }) => {
	...
	test.describe('moving sections', () => {
		...
		test('should not display move up for the top, or move down for the bottom section', async ({ page }) => {
			await page.goto(
				'wp-admin/admin.php?page=wc-admin&path=%2Fanalytics%2Foverview'
			);
		...
		test('should allow a user to move a section down', async ({ page }) => {
			await page.goto(
				'wp-admin/admin.php?page=wc-admin&path=%2Fanalytics%2Foverview'
			);
