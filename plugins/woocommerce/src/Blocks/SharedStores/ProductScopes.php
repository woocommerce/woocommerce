 ...
 * Assembling the right values for each kind of scope element is the
 * caller's responsibility.
 *
 * *Places* are the scope elements whose inner blocks establish where a form
 * renders — a Single Product block or a Product Collection tile. The
 * helper tracks them as a stack, so the current place is always the
 * innermost one still open.
 *
 * Nothing here is persisted across requests or stored in post content.
 *
...
class ProductScopes {
	 ...
	 * `render_block_context` filter it installs around `do_blocks()`, so
	 * its inner blocks, and `GroupedProductItem` forwarding into each
	 * child row's context, can read it back.
	 *
	 * @since 11.3.0
	 *
