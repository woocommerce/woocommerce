...
class CheckoutFields {
	...
	public function init() {
		...
		// Hooks which allow additional fields to display on the customer My Account pages.
		add_filter( 'woocommerce_save_account_details_required_fields', array( $this, 'myaccount_required_additional_contact_fields' ), 10, 1 );
		add_filter( 'woocommerce_edit_account_form', array( $this, 'myaccount_edit_additional_contact_fields' ), 10, 1 );
		add_action( 'woocommerce_save_account_details', array( $this, 'save_additional_contact_fields_to_account' ), 10, 1 );
		add_action( 'woocommerce_my_account_after_my_address', array( $this, 'myaccount_render_additional_address_fields' ), 10, 1 );
		add_filter( 'woocommerce_address_to_edit', array( $this, 'myaccount_edit_additional_address_fields' ), 10, 2 );
		add_action( 'woocommerce_after_save_address_validation', array( $this, 'save_additional_address_fields_to_account' ), 10, 4 );
	}
	 ...
	 * @return array
	 */
	public function myaccount_required_additional_contact_fields( $fields ) {
		$additional_fields = $this->get_fields_for_location( 'contact' );
	 ...
	 */
	public function myaccount_edit_additional_contact_fields() {
		$customer = new \WC_Customer( get_current_user_id() );
		$fields   = $this->get_fields_for_location( 'contact' );

		foreach ( $fields as $key => $field ) {
			$form_field          = $field;
			$form_field['value'] = $this->get_field_from_customer( $key, $customer, 'contact' );
	 ...
	 */
	public function save_additional_contact_fields_to_account( $user_id ) {
		...
		foreach ( $fields as $field_key => $field ) {
			...
			$this->persist_field_for_customer( $field_key, wc_clean( wp_unslash( $_POST[ $field_key ] ) ), $customer );
		}
	}
	 ...
	 */
	public function myaccount_render_additional_address_fields( $address_type ) {
		if ( ! in_array( $address_type, array( 'billing', 'shipping' ), true ) ) {
			return;
		}

		$customer = new \WC_Customer( get_current_user_id() );
		$fields   = $this->get_fields_for_location( 'address' );
		...
		foreach ( $fields as $key => $field ) {
			$value = $this->format_additional_field_value(
				$this->get_field_from_customer( $key, $customer, $address_type ),
				$field
			);
	 ...
	 */
	public function myaccount_edit_additional_address_fields( $address, $address_type ) {
		$customer = new \WC_Customer( get_current_user_id() );
		$fields   = $this->get_fields_for_location( 'address' );
		...
		foreach ( $fields as $key => $field ) {
			...
			$address[ $field_key ]['value'] = $this->get_field_from_customer( $key, $customer, $address_type );
	 ...
	 * @param integer      $user_id User ID.
	 * @param string       $address_type Type of address (billing or shipping).
	 * @param array        $address Address data from the core form.
	 * @param \WC_Customer $customer Customer object.
	 * @return void
	 */
	public function save_additional_address_fields_to_account( $user_id, $address_type, $address, $customer ) {
		$fields = $this->get_fields_for_location( 'address' );
		...
		foreach ( $fields as $key => $field ) {
			...
			$this->persist_field_for_customer( $field_key, $value, $customer );
		}
	}
