...
class CartCheckoutUtils {
	...
	public static function is_cart_block_default() {
		return self::is_block_default( 'cart' );
	}
	...
	public static function is_checkout_block_default() {
		return self::is_block_default( 'checkout' );
	}

	/**
	 * Checks if the default block is using the block.
	 *
	 * @param string $block The block name.
	 *
	 * @return bool true if the WC block is using the block.
	 */
	private static function is_block_default( $block ) {
		if ( wc_current_theme_is_fse_theme() ) {
			// Ignore the pages and check the templates.
			$templates_from_db = BlockTemplateUtils::get_block_templates_from_db( array( $block ) );
			foreach ( $templates_from_db as $template ) {
				if ( has_block( 'woocommerce/' . $block, $template->content ) ) {
					return true;
				}
			}
		}
		$page_id = wc_get_page_id( $block );

		return $page_id && has_block( 'woocommerce/' . $block, $page_id );
	}
