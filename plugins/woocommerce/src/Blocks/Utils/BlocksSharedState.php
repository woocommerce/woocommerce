...
class BlocksSharedState {
	...
	public static function load_cart_state( string $consent_statement ): void {
		...
		if ( null === self::$blocks_shared_cart_state ) {
			...
			//
			// Drafts are client-only — never seeded server-side. They are born
			// lazily on first interaction via the cart store's `upsertDraftItem`
			// action, so there is no `draftItems` slot to seed here.
