	...
	protected function enqueue_assets( $attributes, $content, $block ) {
		...
		if ( isset( $product_id ) ) {
			...
			if ( $rendered_product instanceof \WC_Product ) {
				$product_type       = $rendered_product->get_type();
				$template_part_path = $this->get_template_part_path(
					ProductType::VARIATION === $product_type
						? ProductType::SIMPLE
						: $product_type
				);
			...
			*/
			...
			if ( ProductType::SIMPLE === $product_type ) {
				...
				$hidden_input = '<div class="single_variation_wrap">
					...
					<input type="hidden"
						name="variation_id"
						data-wp-bind--value="woocommerce/product-context::state.selectedVariation.id"
					/>
				</div>';
			}
