...
class CustomerAccount extends AbstractBlock {
	...
	private function render_icon( $attributes ) {
		...
		if ( self::DISPLAY_LINE === $attributes['iconStyle'] ) {
			$icon_name = self::DISPLAY_LINE;
			$svg       = '<svg class="' . esc_attr( $attributes['iconClass'] ) . '" viewBox="1 1 29 29" fill="none" xmlns="http://www.w3.org/2000/svg">
				<circle
					cx="16"
					cy="10.5"
					r="3.5"
			...
			$icon_name = self::DISPLAY_ALT;
			$svg       = '<svg class="' . esc_attr( $attributes['iconClass'] ) . '" xmlns="http://www.w3.org/2000/svg" viewBox="-4 -4 25 25">
				<path
			...
			$icon_name = 'default';
			$svg       = '<svg class="' . esc_attr( $attributes['iconClass'] ) . '" xmlns="http://www.w3.org/2000/svg" viewBox="-5 -5 25 25">
			<path
				fill-rule="evenodd"
				clip-rule="evenodd"
		...
		return BlockIconUtils::filter_block_icon( $svg, $icon_name, $this->get_full_block_name(), $attributes );
	}
