...
/**
 * Product Filter: Active List Block.
 */
final class ProductFilterActiveList extends AbstractBlock {
	 ...
	 * @var string
	 */
	protected $block_name = 'product-filter-active-list';
	...
	protected function render( $attributes, $content, $block ) {
		...
		$namespace             = wp_json_encode( array( 'namespace' => 'woocommerce/product-filter-active-list' ), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP );
		$classes               = '';
		$style                 = '';

		$tags = new \WP_HTML_Tag_Processor( $content );
		if ( $tags->next_tag( array( 'class_name' => 'wc-block-product-filter-active-list' ) ) ) {
		...
		<div <?php echo get_block_wrapper_attributes( $wrapper_attributes ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
			<ul class="wc-block-product-filter-active-list__list" aria-label="<?php echo esc_attr__( 'Filter Options', 'woocommerce' ); ?>">
			<?php foreach ( $items as $item ) { ?>
					...
					<li
						...
						if ( ! $item['selected'] ) :
							if ( $count >= $remaining_initial_unchecked ) :
								?>
								class="wc-block-product-filter-active-list__item"
					...
					>
						<label
							class="wc-block-product-filter-active-list__label"
							for="<?php echo esc_attr( $item['id'] ); ?>"
						>
							<span class="wc-block-product-filter-active-list__input-wrapper">
								<input
									id="<?php echo esc_attr( $item['id'] ); ?>"
									class="wc-block-product-filter-active-list__input"
									type="checkbox"
									aria-invalid="false"
								...
								>
								<svg class="wc-block-product-filter-active-list__mark" viewBox="0 0 10 8" fill="none" xmlns="http://www.w3.org/2000/svg">
									<path d="M9.25 1.19922L3.75 6.69922L1 3.94922" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
								</svg>
							</span>
							<span class="wc-block-product-filter-active-list__text">
			...
			<?php if ( count( $items ) > $show_initially ) : ?>
				<button
					class="wc-block-product-filter-active-list__show-more"
					data-wc-bind--hidden="context.showAll"
