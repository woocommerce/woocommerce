...
class Status extends AbstractOrderConfirmationBlock {
	...
	protected function render_content( $order, $permission = false, $attributes = [], $content = '' ) {
		...
		error_log( 'Hook content: ' . $hook_content . ' | Content: ' . $content );

		// If content is empty, provide fallback structure with Block Bindings
		if ( empty( $content ) ) {
			$content = '<h1 data-wc-bind--inner-html="woocommerce/order-status-title">' . esc_html__( 'Order Status', 'woocommerce' ) . '</h1>';
			$content .= '<p data-wc-bind--inner-html="woocommerce/order-status-description">' . esc_html__( 'Loading order details...', 'woocommerce' ) . '</p>';
		}

		// Block Bindings handle dynamic content - return the content structure  
		return $hook_content . $content;
	}
