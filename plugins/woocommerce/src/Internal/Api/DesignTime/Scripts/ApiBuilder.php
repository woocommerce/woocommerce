...
class ApiBuilder {
	...
	private function generate_resolver( string $fqcn, \ReflectionClass $ref, string $kind ): void {
		...
		if ( $ref->hasMethod( 'authorize' ) ) {
			...
			if ( ! $authorize_ignored ) {
				$authorize_param_names = $this->validate_authorize_method( $fqcn, $execute_method, $authorize_method );
			}
		}

		// Capabilities (skipped when authorize() is present).
		$auth          = $authorize_param_names === null ? $this->resolve_authorization( $ref ) : array(
			'caps'   => array(),
			'public' => false,
		);
		$caps          = $auth['caps'];
	 ...
	 *
	 * @return string[] The authorize parameter names.
	 */
	private function validate_authorize_method(
		...
		foreach ( $authorize_method->getParameters() as $p ) {
			...
			$authorize_param_names[] = $name;
		}

		return $authorize_param_names;
	}
