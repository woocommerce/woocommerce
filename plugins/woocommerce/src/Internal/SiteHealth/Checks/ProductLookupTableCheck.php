...
class ProductLookupTableCheck {
	...
	public function get_label(): string {
		return __( 'Woo: product lookup table', 'woocommerce' );
	}
	...
	public function run(): array {
		...
		if ( ( $lookup === 0 && $products > 0 ) || $drift > $threshold ) {
			return $this->finish( array(
				'label'       => __( 'Woo: product lookup table is out of sync', 'woocommerce' ),
				'status'      => 'recommended',
		...
		return $this->finish( array(
			'label'       => __( 'Woo: product lookup table is in sync', 'woocommerce' ),
			'status'      => 'good',
