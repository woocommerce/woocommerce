...
class UPSShippingProvider extends AbstractShippingProvider {
	...
	public function try_parse_tracking_number( string $tracking_number, string $shipping_from, string $shipping_to ): ?array {
		...
		$is_1z_format = preg_match( '/^1Z[0-9A-Z]{16}$/', $tracking_number );
		...
		$is_possibly_ups      = preg_match( '/^9\d{21,34}$/', $tracking_number );
		...
		if ( $is_1z_format ) {
			$match = true;
		} elseif ( ( $is_t_format || $is_h_format ) && $is_domestic_shipping ) {
			$match = true;
