 ...
 *
 * The parameter is optional and declared on the checkout route by
 * {@see \Automattic\WooCommerce\Internal\POS\StoreApi\Routes\Controller}, so the
 * Store API enforces that it is a positive integer before this hook runs. With
 * no `customer_id` the order stays a guest order (the existing behaviour); an
 * unknown account fails the checkout loudly rather than silently attributing
 * the sale to the wrong account.
 *
...
class CustomerAccountPolicy implements RegisterHooksInterface {
	...
	/**
	 * Set the order's customer id from the request's `customer_id` parameter.
	 *
	...
	public function attach_customer_account( WC_Order $order, WP_REST_Request $request ): void {
		// The schema (declared in Controller) guarantees a positive integer when
		// present; absent or non-positive means a guest sale — leave it untouched.
