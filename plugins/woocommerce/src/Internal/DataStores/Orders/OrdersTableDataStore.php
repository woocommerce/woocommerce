...
class OrdersTableDataStore extends \Abstract_WC_Order_Data_Store_CPT implements \WC_Object_Data_Store_Interface, \WC_Order_Data_Store_Interface {
	...
	public function search_orders( $term ) {
		return wc_get_orders( array( 's' => $term ) );
		global $wpdb;

		$table       = self::get_orders_table_name();
		$search_term = '%' . $wpdb->esc_like( wc_clean( $term ) ) . '%';
		$order_ids   = array();
		...
		/**
		 * Controls the meta fields utilized in order searches.
		 *
		 ...
		 * @since TBD
		 *
		 * @param array $search_fields Meta keys to be queried during an order search.
		 */
		$search_fields = array_map(
			function ( $term ): string {
				return "'" . esc_sql( wc_clean( $term ) ) . "'";
			},
			apply_filters(
				'woocommerce_cot_shop_order_meta_search_fields',
				array(
		...
		if ( ! empty( $search_fields ) ) {
			...
			$address_field_matches = $wpdb->get_col(
				$wpdb->prepare(
					"
						SELECT DISTINCT orders.id
						FROM $table AS orders
						LEFT JOIN mywc_wc_orders_meta AS address_indicies ON address_indicies.id = orders.id
						WHERE (
							address_indicies.meta_key IN ( $search_fields )
							AND address_indicies.meta_value LIKE %s
						)
					",
					$search_term
				)
			...
			$address_field_matches = array();
		}

		$order_item_matches = $wpdb->get_col(
			$wpdb->prepare(
				"
					SELECT order_id
