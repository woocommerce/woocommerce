	...
	public function cleanup_post_data( int $order_id, bool $skip_checks = false ): void {
		...
		if ( ! $post_is_placeholder ) {
			...
			if ( ! $skip_checks && ! $this->is_order_newer_than_post( $order ) ) {
				...
				throw new \Exception( sprintf( __( 'Data in posts table appears to be more recent than in HPOS tables. Compare order data with `wp wc hpos diff %1$d` and use `wp wc hpos backfill %1$d --from=posts --to=hpos` to fix.', 'woocommerce' ), $order_id ) );
			}
		}
