...
class RestApiUtil {
	 ...
	 */
	public function lazy_load_namespace( string $namespace, callable $callback, string $rest_route = '', string $callback_filter_id = '' ) {
		if ( '' === $rest_route ) {
		...
		if ( '' !== $rest_route ) {
			...
			$namespace  = trailingslashit( $namespace );
			if ( '/' === $rest_route || str_starts_with( $rest_route, $namespace ) ) {
		...
		if ( '' === $callback_filter_id ) {
			$callback_filter    = function ( $filter_result, $server, $request ) use ( $namespace, $callback, &$callback_filter_id ) {
				if ( is_callable( array( $request, 'get_route' ) ) ) {
					$this->lazy_load_namespace( $namespace, $callback, $request->get_route(), $callback_filter_id );
				}
