...
/**
 * ProcessCheckoutTrait
 *
 * The shared checkout request lifecycle (GET/POST/PUT handlers, the nonce rule
 * and error responses) plus the place-order orchestration (draft order
 * create/update, customer processing, validation, stock reservation, payment
 * dispatch) for the Store API `/checkout` route.
 *
 * Extracted from {@see \Automattic\WooCommerce\StoreApi\Routes\V1\Checkout} so a
 * first-party reuser can run the *exact same* checkout pipeline by composing this
 * trait instead of subclassing the concrete web route or duplicating its body.
 * POS ({@see \Automattic\WooCommerce\Internal\POS\StoreApi\Routes\Checkout},
 * `wc/internal/pos/v1`) does exactly that. Each consumer keeps only its own
 * endpoint shape (`get_path()` + `get_args()`); everything behavioural lives here
 * and is shared, so a pipeline change reaches every consumer at once. For
 * anything a consumer should opt out of, prefer a seam/filter (e.g.
 * {@see \Automattic\WooCommerce\StoreApi\Routes\V1\AbstractCartRoute::is_cookie_authenticated()})
 * over a hard-coded branch.
 *
 * Complements {@see CheckoutTrait} (payment + additional-field persistence). This
 * trait expects the consuming class to provide the cart/order controllers and
 * schemas via {@see \Automattic\WooCommerce\StoreApi\Routes\V1\AbstractCartRoute},
 * plus {@see DraftOrderTrait} and {@see CheckoutTrait}.
 */
trait ProcessCheckoutTrait {
	 ...
	 * but still honours the {@see \Automattic\WooCommerce\StoreApi\Routes\V1\AbstractCartRoute::is_cookie_authenticated()}
	...
	private function build_draft_route_response( \WP_REST_Request $request ) {
		// The draft response is built from cart + customer; the request isn't consulted here.
		unset( $request );
	...
	private function process_order( \WP_REST_Request $request ) { // phpcs:ignore Squiz.Commenting.FunctionComment.IncorrectTypeHint
		...
		try {
			...
			throw new RouteException(
				esc_html( (string) $e->getErrorCode() ),
				esc_html( $e->getMessage() ),
				(int) $e->getCode()
			);
		}
	 ...
	 */
	private function add_data_to_error_object( $error, $data, $http_status_code, bool $include_cart = false ): \WP_Error {
	 ...
	 * @throws RouteException On error.
	 */
	private function create_or_update_draft_order( \WP_REST_Request $request ): void {
		// The order is derived from the cart/session, not the request, in this helper.
		unset( $request );
	 ...
	 */
	private function update_customer_address_field( $customer, $key, $value, $address_type ): void {
	 ...
	 * @phpstan-param \WP_REST_Request<array<string, mixed>> $request
	 */
	private function update_customer_from_request( \WP_REST_Request $request ): void {
		...
		foreach ( $additional_field_contexts as $context => $context_data ) {
			...
			if ( 'shipping_address' === $context_data['param'] ) {
				$field_values = (array) $request['shipping_address'];
	 ...
	 * @phpstan-param \WP_REST_Request<array<string, mixed>> $request
	 */
	private function process_customer( \WP_REST_Request $request ): void {
		...
		if ( $this->should_create_customer_account( $request ) ) {
			...
			if ( is_wp_error( $customer_id ) ) {
				throw new RouteException(
					esc_html( (string) $customer_id->get_error_code() ),
	 ...
	 */
	private function validate_user_can_place_order(): void {
		if (
