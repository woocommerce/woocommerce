 ...
 *
 * Encapsulates the Store API checkout place-order pipeline: request lifecycle (GET/POST/PUT),
 * draft-order creation, customer processing, pre-payment validation, stock/coupon holds and
 * payment. Extracted verbatim from the web `Checkout` route so an alternative checkout route can
 * reuse the same pipeline instead of subclassing the concrete web route. Strictly
 * behaviour-preserving for web checkout.
 *
 * Consuming classes must extend `AbstractCartRoute` and also compose `DraftOrderTrait` and
 * `CheckoutTrait`, which supply the helpers this trait calls (e.g. `get_draft_order()`,
 * `process_payment()`, `update_order_from_request()`) and the controllers/schema it relies on
 * (`$this->order_controller`, `$this->cart_controller`, `$this->additional_fields_controller`,
 * `$this->schema`, `$this->cart_schema`).
 *
 * @internal
 */
