...
const templatePath = `${ BLOCK_THEME_WITH_TEMPLATES_SLUG }//taxonomy-product_cat`;
const templateType = 'wp_template';

test.describe( 'Product by Category template', async () => {
	test( "theme template has priority over WooCommerce's and can be modified", async ( {
		...
		await expect(
			page
				.getByText( 'Products by Category template loaded from theme' )
				.first()
		).toBeVisible();
		await expect(
