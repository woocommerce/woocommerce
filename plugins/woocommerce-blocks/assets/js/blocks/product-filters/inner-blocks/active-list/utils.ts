...
export function getColorVars( attributes: BlockAttributes ) {
	...
	const vars: Record< string, string > = {
		'--wc-product-filter-active-list-option-element': getCSSVar(
			optionElement,
			customOptionElement
		),
		'--wc-product-filter-active-list-option-element-border': getCSSVar(
			optionElementBorder,
			customOptionElementBorder
		),
		'--wc-product-filter-active-list-option-element-selected': getCSSVar(
			optionElementSelected,
			customOptionElementSelected
		),
	};
