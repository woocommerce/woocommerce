...
const ProcessingStep: React.FC< ProcessingStepProps > = function ( props ) {
	...
	const [ hasActionSuccessfullyRun, setHasActionSuccessfullyRun ] = useState(
		false
	);
	...
	const action = useSelect(
		( select ) =>
			( select( ONBOARD_STORE ) as OnboardSelect ).getPendingAction(),
		[]
	);
	const progress = useSelect(
		( select ) =>
			( select( ONBOARD_STORE ) as OnboardSelect ).getProgress(),
		[]
	);
	const progressTitle = useSelect(
		( select ) =>
			( select( ONBOARD_STORE ) as OnboardSelect ).getProgressTitle(),
		[]
	);
	const stepProgress = useSelect(
		( select ) =>
			( select( ONBOARD_STORE ) as OnboardSelect ).getStepProgress(),
		[]
	);
	...
	const captureFlowException = useCaptureFlowException(
		props.flow,
		'ProcessingStep'
	);
	...
	const getSubtitle = () => {
		return (
			props.subtitle || loadingMessages[ currentMessageIndex ]?.subtitle
		);
	};
