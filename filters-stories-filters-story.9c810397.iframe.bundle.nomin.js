 ...
 */
const getCurrentDatesMemoized = (0,lodash__WEBPACK_IMPORTED_MODULE_3__.memoize)((period, compare, primaryStart, primaryEnd, secondaryStart, secondaryEnd, secondaryShift) => {
...
}, (period, compare, primaryStart, primaryEnd, secondaryStart, secondaryEnd, secondaryShift) => [period, compare, primaryStart && primaryStart.format(), primaryEnd && primaryEnd.format(), secondaryStart && secondaryStart.format(), secondaryEnd && secondaryEnd.format(), secondaryShift].join(':'));
...
const getCurrentDates = (query, defaultDateRange = 'period=month&compare=previous_year') => {
  ...
  return getCurrentDatesMemoized(period, compare, primaryStart, primaryEnd, secondaryStart, secondaryEnd, secondaryShift);
};
 ...
 */
const getPreviousDate = (date, date1, date2, compare = 'previous_year', interval, shift) => {
  ...
  const wallClock = value => moment.utc(moment(value).format(defaultDateTimeFormat));
  const difference = wallClock(date1).diff(wallClock(date2), interval);
  return moment(wallClock(date).subtract(difference, interval).format(defaultDateTimeFormat));
};
