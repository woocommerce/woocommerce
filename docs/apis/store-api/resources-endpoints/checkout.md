...
For post-purchase business logic such as fraud screening, fulfillment, tracking, or upsells, do not rely on template hooks. Use order lifecycle hooks instead, which fire regardless of how the order was placed:

- `woocommerce_store_api_checkout_order_processed` - fires when the Store API places the order, before payment is processed.
- `woocommerce_payment_complete` - fires when payment for an order completes.
- `woocommerce_order_status_*` transition hooks (e.g. `woocommerce_order_status_processing`) - fire when an order changes status.
