...
| `search_fields`   | array   | Fields to search when used with `search` parameter. All search tokens must be found across any of the specified fields. Takes precedence over other search parameters. If empty, uses default `search` behavior. Options: `name`, `sku`, `global_unique_id`, `mpn`, `description`, and `short_description`. |
...
| `mpn`             | string  | Limit results to exact MPN matches. Separate multiple values with commas.                                                                                                                                                                                                                            |
| `search_mpn` | string | Limit results to partial MPN matches within the first 100 characters. Takes precedence over `mpn`; combines with name and SKU search filters. `search` with `search_fields` takes precedence over `search_mpn`. |
