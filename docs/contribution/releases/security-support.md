...
When a security fix warrants it (CVSS score >=9), WooCommerce will provide security patches for the **last 21 major versions**. If the current stable WooCommerce version is 99.0, that means version **97.0 and newer**. Otherwise, we publish fixes for the latest version.
