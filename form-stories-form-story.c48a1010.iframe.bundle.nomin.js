...
// EXTERNAL MODULE: ../../node_modules/.pnpm/lodash@4.18.1/node_modules/lodash/isSymbol.js
...
const PLAIN_KEY = /^\w*$/;
const DEEP_PATH = /\.|\[(?:[^[\]]*|(["'])(?:(?!\1)[^\\]|\\.)*?\1)\]/;
function isKey(name, object) {
  if (Array.isArray(name)) {
    return false;
  }
  const type = typeof name;
  if (type === 'number' || type === 'symbol' || type === 'boolean' || name === null || name === undefined || isSymbol_default()(name)) {
    return true;
  }
  return PLAIN_KEY.test(name) || !DEEP_PATH.test(name) || name in object;
}
...
function FormComponent({
  ...
  (name, value) => {
    ...
    const segments = isKey(name, newValues) ? toPath_default()([name]) : toPath_default()(name);
    ...
    // inherited value.
    if (!segments.length || segments.some(segment => UNWRITABLE_KEYS.includes(segment))) {
      return;
    }
    setWith_default()(newValues, segments, value, (clone_default()));
