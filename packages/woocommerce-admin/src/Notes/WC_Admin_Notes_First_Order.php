...
class WC_Admin_Notes_First_Order {
	...
	public static function get_note() {
		$content = __( "Congratulations on getting your first order! Now it's a great time to learn how to manage your orders.", 'woocommerce' );

		$note = new WC_Admin_Note();
		$note->set_title( __( 'First order received', 'woocommerce' ) );
		$note->set_content( $content );
		$note->set_content_data( (object) array() );
		$note->set_type( WC_Admin_Note::E_WC_ADMIN_NOTE_INFORMATIONAL );
		$note->set_name( self::NOTE_NAME );
		$note->set_source( 'woocommerce-admin' );
		$note->add_action( 'learn-more', __( 'Learn more', 'woocommerce' ), 'https://docs.woocommerce.com/document/managing-orders/?utm_source=inbox' );
		return $note;
	}
}
