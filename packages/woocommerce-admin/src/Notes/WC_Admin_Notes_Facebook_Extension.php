...
class WC_Admin_Notes_Facebook_Extension {
	...
	public static function possibly_add_facebook_note() {
		...
		$content = __( 'Grow your business by targeting the right people and driving sales with Facebook. You can install this free extension now.', 'woocommerce' );

		$note = new WC_Admin_Note();
		$note->set_title( __( 'Market on Facebook', 'woocommerce' ) );
		...
		$note->add_action( 'learn-more', __( 'Learn more', 'woocommerce' ), 'https://woocommerce.com/products/facebook/', WC_Admin_Note::E_WC_ADMIN_NOTE_UNACTIONED );
		$note->add_action( 'install-now', __( 'Install now', 'woocommerce' ), false, WC_Admin_Note::E_WC_ADMIN_NOTE_UNACTIONED, true );
	...
	public function install_facebook_extension( $note ) {
		if ( self::NOTE_NAME === $note->get_name() ) {
			...
			$content = __( 'You\'re almost ready to start driving sales with Facebook. Complete the setup steps to control how WooCommerce integrates with your Facebook store.', 'woocommerce' );
			$note->set_title( __( 'Market on Facebook — Installed', 'woocommerce' ) );
			$note->set_content( $content );
