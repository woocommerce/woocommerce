...
describe( 'Products API tests: List All Products', () => {
	...
	describe( 'List all products', () => {
		...
		it( 'pagination', async () => {
			...
			const allProductIds = page1.body
				.concat( page2.body )
				.reduce( ( acc, product ) => {
					acc[ product.id ] = 1;
					return acc;
		...
		it( 'inclusion / exclusion', async () => {
			...
			const allProductIds = allProducts.body.map(
				( product ) => product.id
			);
			...
			expect( excluded.body ).toHaveLength(
				PRODUCTS_COUNT - productsToFilter.length
			);
		...
		it( 'categories', async () => {
			const accessory = [ expect.objectContaining( { name: 'Beanie' } ) ];
			const hoodies = [
				expect.objectContaining( { name: 'Hoodie with Zipper' } ),
				expect.objectContaining( { name: 'Hoodie with Pocket' } ),
		...
		describe( 'orderby', () => {
			...
			const productNamesByRatingAsc = [ 'Sunglasses', 'Cap', 'T-Shirt' ];
			...
			it( 'price', async () => {
				...
				expect( result1.statusCode ).toEqual( 200 );
				expect( result1.body ).toHaveLength(
				...
				expect( result2.statusCode ).toEqual( 200 );
				expect( result2.body ).toHaveLength(
