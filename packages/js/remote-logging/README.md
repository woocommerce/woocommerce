...
1. **Remote Logging Enabled**: The package checks `window.wcSettings.isRemoteLoggingEnabled` to determine if the feature should be enabled. The value is set via PHP and passed to JS as a boolean. It requires the Remote Logging feature to be enabled, a valid WooCommerce.com connection, and a supported WooCommerce version. Please see the [RemoteLogger.php](https://github.com/woocommerce/woocommerce/blob/trunk/plugins/woocommerce/src/Internal/Logging/RemoteLogger.php) for more details.
...
If either of these conditions are not met, no logs will be transmitted to the remote server.
