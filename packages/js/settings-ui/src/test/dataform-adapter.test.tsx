...
describe( 'dataform-adapter', () => {
	...
	describe( 'package control mapping', () => {
		...
		it( 'maps a disabled state to the package isDisabled callback', () => {
			const settingsField: SettingsUIField = {
				id: 'field',
				label: 'Field',
				type: 'text',
		...
		it( 'drops attributes without a rule equivalent and warns', () => {
			...
			const settingsField: SettingsUIField = {
				...
				label: 'Field',
				type: 'text',
				customAttributes: { 'data-tracking': 'yes' },
			};
			const field = buildDataFormField(
				settingsField,
				fieldOptions( settingsField )
			...
			expect( warnSpy ).toHaveBeenCalledWith(
				expect.stringContaining( 'data-tracking' ),
				expect.anything()
			);
			warnSpy.mockRestore();
		} );

		it( 'renders unknown field types through the package text control with a warning', () => {
			const settingsField: SettingsUIField = {
				id: 'field',
				label: 'Field',
				type: 'image_width',
