 ...
 *
 * @returns {boolean} True if we are in the email editor context, false otherwise
 */
export function useIsEmailEditor(): boolean {
	return useSelect( ( select ) => {
		...
		return (
			String(currentPostId) === String(emailPostId) && String(currentPostType) === String(emailPostType)
		);
	}, [] );
}
