 ...
 */
import { VariationOptionsControl } from '../variation-options-control';
...
describe( 'VariationOptionsControl', () => {
	beforeEach( () => {
		jest.clearAllMocks();
	} );
	...
	describe( 'empty state', () => {
		it( 'should show subtitle and "Add attributes" button', () => {
			const { queryByText } = render(
				<VariationOptionsControl value={ [] } onChange={ () => {} } />
			);
	...
	describe( 'deleting', () => {
		it( 'should show a window confirm when trash icon is clicked', async () => {
			...
			(
				await screen.findAllByLabelText( 'Remove variation option' )
			 )[ 0 ].click();
