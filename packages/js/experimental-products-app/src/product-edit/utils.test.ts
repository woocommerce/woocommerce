...
if ( isLatestWordPress() || isLatestMinusOneWordPress() ) {
	describe.skip( 'product edit utils', () => {
		...
		describe( 'getVisibleProductEditFields', () => {
			...
			const expectFieldOrder = (
				...
				orderedFieldIds.forEach( ( fieldId, index ) => {
					...
					if ( previousFieldId ) {
						expect(
							fieldIds.indexOf( previousFieldId )
						).toBeLessThan( fieldIds.indexOf( fieldId ) );
					}
				} );
			};
			...
			const bulkSellableInstanceFieldIds =
				sellableInstanceFieldIds.filter(
					( fieldId ) => fieldId !== 'sku'
				);
