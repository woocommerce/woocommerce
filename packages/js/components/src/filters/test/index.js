...
describe( 'ReportFilters date selection', () => {
	...
	it( 'maps the Last month preset to navigation and the selection callback', async () => {
		...
		// toHaveBeenCalledWith equates a key set to undefined with an absent
		// one, and clearing `after` and `before` is exactly what choosing a
		// preset does: dropping it would leave the assertion green.
		...
		expect( onDateSelect.mock.calls[ 0 ] ).toStrictEqual( [
			expectedSelection,
		] );
	} );
