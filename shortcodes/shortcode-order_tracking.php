	...
	if ($_POST) :
		...
		elseif ($order->id && $order_email && $order->get_order( $order->id )) :
			...
			if ($order->billing_email == $order_email) :
				...
				echo '<p>'.sprintf( __('Order #%s which was made %s has the status &ldquo;%s&rdquo;', 'woothemes'), $order->id, human_time_diff(strtotime($order->order_date), current_time('timestamp')).__(' ago', 'woothemes'), $order->status );
