	...
	function woocommerce_subcategory_thumbnail( $category ) {
		...
		if ( $thumbnail_id ) {
			...
			$image_srcset = function_exists( 'wp_get_attachment_image_srcset' )
							? wp_get_attachment_image_srcset( $thumbnail_id, $small_thumbnail_size )
							: false;
			$image_sizes  = function_exists( 'wp_get_attachment_image_sizes' )
							? wp_get_attachment_image_sizes( $thumbnail_id, $small_thumbnail_size )
							: false;
		}
		...
		if ( $image ) {
			...
			$html  = '<img src="'
						. esc_url( $image ) . '" alt="'
						. esc_attr( $category->name ) . '" width="'
						. esc_attr( $dimensions['width'] ) . '" height="';

			// Add responsive image markup if available.
			if ( $image_srcset && $image_sizes ) {
				$html .= esc_attr( $dimensions['height'] ) . '" srcset="'
							. esc_attr( $image_srcset ) . '" sizes="'
							. esc_attr( $image_sizes ) . '" />';
			} else {
				$html .= esc_attr( $dimensions['height'] ) . '" />';
			}
