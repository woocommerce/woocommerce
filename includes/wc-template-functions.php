...
if ( ! function_exists( 'woocommerce_subcategory_thumbnail' ) ) {
	...
	function woocommerce_subcategory_thumbnail( $category ) {
		...
		if ( $image ) {
			...
			$html  = '<img src="' . esc_url( $image ) . '" alt="' . esc_attr( $category->name ) . '" width="' . esc_attr( $dimensions['width'] ) . '" height="';

			// Add responsive image markup if available.
			if ( $image_srcset && $image_sizes ) {
				$html .= esc_attr( $dimensions['height'] ) . '" srcset="' . esc_attr( $image_srcset ) . '" sizes="' . esc_attr( $image_sizes );
			} else {
