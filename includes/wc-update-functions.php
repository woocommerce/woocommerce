...
/**
 * Add empty meta attribute values for variable products having a value of "Any..." for the attribute,
 * only for the variations missing them, in increments of 100 products.
 *
 * @return bool true if there are more products to process.
 */
function wc_update_450_insert_attribute_metadata_for_variations_not_having_it() {
	$state_option_name = 'woocommerce_' . __FUNCTION__ . '_state';
	...
	foreach ( $products as $product ) {
		$attributes_used_for_variations = array_keys( $product->get_variation_attributes() );
		$variation_ids                  = $product->get_children();
		foreach ( $variation_ids as $variation_id ) {
			foreach ( $attributes_used_for_variations as $attribute_name ) {
				$attribute_name = 'attribute_' . $attribute_name;
				if ( empty( get_post_meta( $variation_id, $attribute_name ) ) ) {
					add_post_meta( $variation_id, $attribute_name, '' );
				}
			}
		}
	}
