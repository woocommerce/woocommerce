...
class WC_Gateway_Paypal_IPN_Handler extends WC_Gateway_Paypal_Response {
	...
	public function valid_response( $posted ) {
		if ( ! empty( $posted['custom'] ) && ( $order = $this->get_paypal_order( $posted['custom'] ) ) ) {
			...
			WC_Gateway_Paypal::log( 'Found order #' . $order->get_order_id() );
	...
	protected function payment_status_completed( $order, $posted ) {
		if ( $order->has_status( 'completed' ) ) {
			WC_Gateway_Paypal::log( 'Aborting, Order #' . $order->get_order_id() . ' is already complete.' );
			exit;
		}
		...
		if ( 'completed' === $posted['payment_status'] ) {
			...
			if ( ! empty( $posted['mc_fee'] ) ) {
				...
				update_post_meta( $order->get_order_id(), 'PayPal Transaction Fee', wc_clean( $posted['mc_fee'] ) );
			}
	...
	protected function payment_status_refunded( $order, $posted ) {
		...
		if ( $order->get_total() == ( $posted['mc_gross'] * -1 ) ) {
			...
			$this->send_ipn_email_notification(
				sprintf( __( 'Payment for order %s refunded', 'woocommerce' ), '<a class="link" href="' . esc_url( admin_url( 'post.php?post=' . $order->get_order_id() . '&action=edit' ) ) . '">' . $order->get_order_number() . '</a>' ),
	...
	protected function payment_status_reversed( $order, $posted ) {
		...
		$this->send_ipn_email_notification(
			sprintf( __( 'Payment for order %s reversed', 'woocommerce' ), '<a class="link" href="' . esc_url( admin_url( 'post.php?post=' . $order->get_order_id() . '&action=edit' ) ) . '">' . $order->get_order_number() . '</a>' ),
	...
	protected function payment_status_canceled_reversal( $order, $posted ) {
		$this->send_ipn_email_notification(
			...
			sprintf( __( 'Order #%s has had a reversal cancelled. Please check the status of payment and update the order status accordingly here: %s', 'woocommerce' ), $order->get_order_number(), esc_url( admin_url( 'post.php?post=' . $order->get_order_id() . '&action=edit' ) ) )
		);
	}
	...
	protected function save_paypal_meta_data( $order, $posted ) {
		if ( ! empty( $posted['payer_email'] ) ) {
			update_post_meta( $order->get_order_id(), 'Payer PayPal address', wc_clean( $posted['payer_email'] ) );
		}
		if ( ! empty( $posted['first_name'] ) ) {
			update_post_meta( $order->get_order_id(), 'Payer first name', wc_clean( $posted['first_name'] ) );
		}
		if ( ! empty( $posted['last_name'] ) ) {
			update_post_meta( $order->get_order_id(), 'Payer last name', wc_clean( $posted['last_name'] ) );
		}
		if ( ! empty( $posted['payment_type'] ) ) {
			update_post_meta( $order->get_order_id(), 'Payment type', wc_clean( $posted['payment_type'] ) );
		}
	}
