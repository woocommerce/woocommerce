...
class WC_API_Products extends WC_API_Resource {
	...
	public function upload_product_image( $image_url ) {
		...
		if ( is_wp_error( $response ) ) {
			throw new WC_API_Exception( 'woocommerce_api_invalid_remote_product_image', sprintf( __( 'Error getting remote image %s: %s', 'woocommerce' ), $image_url, $response->get_error_message() ), 400 );
