			...
			foreach ( $this->meta as $meta_key => $meta_values ) {
				...
				foreach( $meta_values as $meta_value ) {
					...
					if ( $flat )
						...
						$meta_list[] = '<dt class="variation-' . esc_attr( wc_attribute_label( str_replace( 'attribute_', '', $meta_key ) ) ) . '">' . wp_kses_post( wc_attribute_label( str_replace( 'attribute_', '', $meta_key ) ) ) . ':</dt><dd class="variation-' . esc_attr( wc_attribute_label( str_replace( 'attribute_', '', $meta_key ) ) ) . '">' . wp_kses_post( wpautop( apply_filters( 'woocommerce_order_item_display_meta_value', $meta_value ) ) ) . '</dd>';
