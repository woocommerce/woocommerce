...
class WC_Order_Data_Store_CPT extends Abstract_WC_Order_Data_Store_CPT implements WC_Object_Data_Store_Interface, WC_Order_Data_Store_Interface {
	...
	protected function update_post_meta( &$order ) {
		...
		// @TODO: is this needed, we can add backward compatibility to get_status and set_status instead????
	...
	public function get_unpaid_orders( $date ) {
		...
		$unpaid_orders = $wpdb->get_col(
			$wpdb->prepare(
				...
				JOIN {$wpdb->prefix}wc_order_statuses AS statuses ON statuses.order_id = posts.ID
				WHERE   posts.post_type   IN ('" . implode( "','", wc_get_order_types() ) . "')
				AND     statuses.payment_status = 'wc-paid'
