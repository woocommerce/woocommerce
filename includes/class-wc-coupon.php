		...
		if ( sizeof( $this->product_categories ) > 0 ) {
			...
			if ( ! WC()->cart->is_empty() ) {
				foreach( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
					...
					if ( sizeof( wc_array_intersect( $product_cats, $this->product_categories ) ) > 0 ) {
						$valid_for_cart = true;
					}
				}
			}
		...
		if ( sizeof( $this->exclude_product_categories ) > 0 ) {
			...
			if ( ! WC()->cart->is_empty() ) {
				foreach( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
					...
					if ( sizeof( wc_array_intersect( $product_cats, $this->exclude_product_categories ) ) > 0 ) {
						$valid_for_cart = false;
					}
				}
			}
			...
			case self::E_WC_COUPON_EXCLUDED_CATEGORIES:
				...
				if ( ! WC()->cart->is_empty() ) {
					foreach( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
						...
						if ( sizeof( $intersect = wc_array_intersect( $product_cats, $this->exclude_product_categories ) ) > 0 ) {
