		...
		if ( $notes ) {
			foreach( $notes as $note ) {
				...
				<li rel="<?php echo absint( $note->comment_ID ) ; ?>" class="<?php echo implode( ' ', $note_classes ); ?>">
					...
					<p class="meta">
						<abbr class="exact-date" title="<?php echo $note->comment_date_gmt; ?> GMT"><?php printf( __( 'added on %1$s at %2$s', 'woocommerce' ), date_i18n( get_option( 'date_format' ), strtotime( $note->comment_date_gmt ) ), date_i18n( get_option( 'time_format' ), strtotime( $note->comment_date_gmt ) ) ); ?></abbr>
