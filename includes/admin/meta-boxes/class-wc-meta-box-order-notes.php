		...
		if ( $notes ) {
			...
			foreach( $notes as $note ) {
				...
				<li rel="<?php echo absint( $note->comment_ID ) ; ?>" class="<?php echo implode( ' ', $note_classes ); ?>">
					...
					<p class="meta">
						<abbr class="exact-date" title="<?php echo $note->comment_date; ?> GMT"><?php printf( __( 'added on %1$s at %2$s', 'woocommerce' ), date_i18n( wc_date_format(), strtotime( $note->comment_date ) ), date_i18n( wc_time_format(), strtotime( $note->comment_date ) ) ); ?></abbr>
