...
class WC_Report_Out_Of_Stock extends WC_Report_Stock {
	...
	public function get_items( $current_page, $per_page ) {
		...
		$query_from = apply_filters( 'woocommerce_report_out_of_stock_query_from', "FROM {$wpdb->posts} as posts
            ...
            LEFT JOIN wp_posts AS p ON p.post_parent = posts.id AND p.post_type = 'product_variation'
            WHERE 1=1
            AND ((posts.post_type = 'product' AND p.id IS NULL) OR (posts.post_type = 'product_variation'))
