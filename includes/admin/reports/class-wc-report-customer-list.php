		...
		switch ( $column_name ) {
			...
			case 'last_order' :
				...
				if ( $order_ids ) {
					...
					return '<a href="' . admin_url( 'post.php?post=' . $order->get_order_id() . '&action=edit' ) . '">' . _x( '#', 'hash before order number', 'woocommerce' ) . $order->get_order_number() . '</a> &ndash; ' . date_i18n( get_option( 'date_format' ), strtotime( $order->order_date ) );
				} else {
					return '-';
				}
