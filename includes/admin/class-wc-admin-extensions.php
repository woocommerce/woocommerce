...
/**
 * WC_Admin_Extensions Class.
 */
class WC_Admin_Extensions {

	/**
	 * Get featured for the extensions screen
	 *
	 * @return array of objects
	 */
	public static function get_featured() {
		if ( false === ( $featured = get_transient( 'wc_extensions_featured' ) ) ) {
			$raw_featured = wp_safe_remote_get( 'https://d3t0oesq8995hv.cloudfront.net/add-ons/featured.json', array( 'user-agent' => 'WooCommerce Extensions Page' ) );
			if ( ! is_wp_error( $raw_featured ) ) {
				$featured = json_decode( wp_remote_retrieve_body( $raw_featured ) );
				if ( $featured ) {
					set_transient( 'wc_extensions_featured', $featured, WEEK_IN_SECONDS );
				}
			}
		}
	...
	public static function get_sections() {
		if ( false === ( $sections = get_transient( 'wc_extensions_sections' ) ) ) {
			$raw_sections = wp_safe_remote_get( 'https://d3t0oesq8995hv.cloudfront.net/addon-sections.json', array( 'user-agent' => 'WooCommerce Extensions Page' ) );
			if ( ! is_wp_error( $raw_sections ) ) {
				$sections = json_decode( wp_remote_retrieve_body( $raw_sections ) );

				if ( $sections ) {
					set_transient( 'wc_extensions_sections', $sections, WEEK_IN_SECONDS );
				}
			}
		}
		...
		if ( $sections ) {
			foreach ( $sections as $sections_id => $section ) {
				...
				$extension_sections[ $sections_id ]->title    = wc_clean( $section->title );
				$extension_sections[ $sections_id ]->endpoint = wc_clean( $section->endpoint );
			}
		}

		return apply_filters( 'woocommerce_extensions_sections', $extension_sections );
	}
	...
	public static function get_section_data( $section_id ) {
		...
		if ( ! empty( $section->endpoint ) ) {
			if ( false === ( $section_data = get_transient( 'wc_extensions_section_' . $section_id ) ) ) {
				$raw_section = wp_safe_remote_get( esc_url_raw( $section->endpoint ), array( 'user-agent' => 'WooCommerce Extensions Page' ) );

				if ( ! is_wp_error( $raw_section ) ) {
					$section_data = json_decode( wp_remote_retrieve_body( $raw_section ) );

					if ( ! empty( $section_data->products ) ) {
						set_transient( 'wc_extensions_section_' . $section_id, $section_data, WEEK_IN_SECONDS );
					}
				}
			}
		}

		return apply_filters( 'woocommerce_extensions_section_data', $section_data->products, $section_id );
	}
	...
	public static function output_banner_block( $block ) {
		?>
		<div class="extensions-banner-block">
			<h1><?php echo esc_html( $block->title ); ?></h1>
			<p><?php echo esc_html( $block->description ); ?></p>
			<div class="extensions-banner-block-items">
				<?php foreach ( $block->items as $item ) : ?>
					<div class="extensions-banner-block-item">
						<div class="extensions-banner-block-item-icon">
							<img class="extensions-img" src="<?php echo esc_url( $item->image ); ?>" />
						</div>
						<div class="extensions-banner-block-item-content">
	...
	public static function output_column( $block ) {
		if ( isset( $block->container ) && 'column_container_start' === $block->container ) {
			?>
			<div class="extensions-column-section">
			<?php
		}
	...
	public static function output_column_block( $block ) {
		?>
		<div class="extensions-column-block">
			<h1><?php echo esc_html( $block->title ); ?></h1>
			<p><?php echo esc_html( $block->description ); ?></p>
			<?php foreach ( $block->items as $item ) : ?>
				<div class="extensions-column-block-item">
					<div class="extensions-column-block-item-icon">
						<img class="extensions-img" src="<?php echo esc_url( $item->image ); ?>" />
					</div>

					<div class="extensions-column-block-item-content">
	...
	public static function output_small_light_block( $block ) {
		?>
		<div class="extensions-small-light-block">
			<img class="extensions-img" src="<?php echo esc_url( $block->image ) ?>" />
			<div class="extensions-small-light-block-content">
				<h1><?php echo esc_html( $block->title ); ?></h1>
				<p><?php echo esc_html( $block->description ); ?></p>
				<div class="extensions-small-light-block-buttons">
	...
	public static function output_small_dark_block( $block ) {
		?>
		<div class="extensions-small-dark-block">
			<h1><?php echo esc_html( $block->title ); ?></h1>
			<p><?php echo esc_html( $block->description ); ?></p>
			<div class="extensions-small-dark-items">
				<?php foreach ( $block->items as $item ) : ?>
					<div class="extensions-small-dark-item">
						<?php if ( ! empty( $item->image ) ) : ?>
							<div class="extensions-small-dark-item-icon">
								<img class="extensions-img" src="<?php echo esc_url( $item->image ); ?>" />
							</div>
						<?php endif; ?>
						<?php
							self::output_button(
								$item->href,
								$item->button,
								'extensions-button-outline-white'
							);
						?>
					</div>
				<?php endforeach; ?>
	...
	public static function output_wcs_banner_block( $block = array() ) {
		...
		$button_url = wp_nonce_url(
			add_query_arg( array(
				'install-extension' => 'woocommerce-services',
			) ),
			'install-extension_woocommerce-services'
		);
		...
		?>
		<div class="extensions-wcs-banner-block">
			<div class="extensions-wcs-banner-block-image">
				<img
			...
			<div class="extensions-wcs-banner-block-content">
				...
				<?php
					self::output_button(
						$block_data['href'],
						$block_data['button'],
						'extensions-button-outline-green'
					);
				?>
			</div>
		</div>
	...
	public static function output_button( $url, $text, $theme, $plugin = '' ) {
		$theme = __( 'Free', 'woocommerce' ) === $text ? 'extensions-button-outline-green' : $theme;
		$theme = is_plugin_active( $plugin ) ? 'extensions-button-installed' : $theme;
		$text = is_plugin_active( $plugin ) ? __( 'Installed', 'woocommerce' ) : $text;
		?>
		<a
			class="extensions-button <?php echo esc_attr( $theme ); ?>"
	...
	public static function output() {
		...
		if ( isset( $_GET['install-extension'] ) && 'woocommerce-services' === $_GET['install-extension'] ) {
			self::install_woocommerce_services_extension();
		}
	 ...
	 */
	public static function install_woocommerce_services_extension() {
		check_admin_referer( 'install-extension_woocommerce-services' );
		...
		wp_safe_redirect( remove_query_arg( array( 'install-extension', '_wpnonce' ) ) );
		exit;
	}
}
