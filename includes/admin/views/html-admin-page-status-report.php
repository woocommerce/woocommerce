...
<table class="wc_status_table widefat" cellspacing="0">
	...
	<tbody>
		...
		<?php endif; ?>
		<?php if ( $wpdb->use_mysqli ) : ?>
			<?php $ver = mysqli_get_server_info( $wpdb->dbh ); ?>
		<?php else : ?>
			<?php $ver = mysql_get_server_info(); ?>
		<?php endif; ?>
		<?php if ( ! empty( $wpdb->is_mysql ) && ! stristr( $ver, 'MariaDB' ) ) : ?>
			<tr>
...
<table class="wc_status_table widefat" cellspacing="0">
	...
	<tbody>
		...
		<?php foreach ( $database['database_tables'] as $table => $table_exists ) : ?>
			<tr>
...
<table class="wc_status_table widefat" cellspacing="0">
	...
	<tbody>
		...
		<tr>
			...
			<td>
				<?php echo $theme['is_child_theme'] ? '<mark class="yes"><span class="dashicons dashicons-yes"></span></mark>' : '<span class="dashicons dashicons-no-alt"></span> &ndash; ' . sprintf( __( 'If you\'re modifying WooCommerce on a parent theme you didn\'t build personally, then we recommend using a child theme. See: <a href="%s" target="_blank">How to create a child theme</a>', 'woocommerce' ), 'https://codex.wordpress.org/Child_Themes' ); ?>
			</td>
		</tr>
		<?php
