...
class WC_Install {
	...
	public static function install() {
		...
		$table_creation_succeeded = self::create_tables();
		self::create_options();
		self::create_roles();
	...
	private static function create_tables() {
		 ...
		 */
		$product_permissions_table = "{$wpdb->prefix}woocommerce_downloadable_product_permissions";
		if ( $wpdb->get_var( "SHOW TABLES LIKE '$product_permissions_table';" ) ) {
			if ( ! $wpdb->get_var( "SHOW COLUMNS FROM `$product_permissions_table` LIKE 'permission_id';" ) ) {
				$schema_change_queries[ $product_permissions_table ][] =
					"ALTER TABLE $product_permissions_table DROP PRIMARY KEY, ADD `permission_id` BIGINT UNSIGNED NOT NULL PRIMARY KEY AUTO_INCREMENT;";
			}
		}
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '$sessions_table'" ) ) {
			if ( ! $wpdb->get_var( "SHOW KEYS FROM $sessions_table WHERE Key_name = 'PRIMARY' AND Column_name = 'session_id'" ) ) {
				$schema_change_queries[ $sessions_table ][] =
					"ALTER TABLE `$sessions_table` DROP PRIMARY KEY, DROP KEY `session_id`, ADD PRIMARY KEY(`session_id`), ADD UNIQUE KEY(`session_key`)";
			}
		}
		...
		if ( is_null( $index_exists ) ) {
			$schema_change_queries[ $wpdb->comments ][] = "ALTER TABLE {$wpdb->comments} ADD INDEX woo_idx_comment_type (comment_type)";
		}
		...
		if ( ! empty( $download_permissions_column_type ) && ! empty( $download_log_column_type ) && $download_permissions_column_type === $download_log_column_type ) {
			...
			if ( 0 === (int) $fk_result->fk_count ) {
				$download_log_table                             = "{$wpdb->prefix}wc_download_log";
				$schema_change_queries[ $download_log_table ][] =
					"ALTER TABLE `$download_log_table`
	...
	// TODO: Submit a patch for WordPress' dbDelta with so that it can return the queries instead of executing them, then get_dbdelta_queries method won't be necessary.

	/**
	 * This method is identical to WordPress' dbDelta function except that:
	 *
	 * - It doesn't execute any database schema modification query.
	 * - It returns an array with the database schema modification queries generated,
	 *   instead of information messages.
	 * - It accepts only CREATE TABLE and ALTER TABLE statements as input, any other
	 *   type of statement will throw an exception.
	 * - It doesn't execute any filter.
	 *
	 * @param array|string $queries An array of queries, or a string with all the quieries concatenated with ';'.
	 * @return array An associative array where the keys are table names, and values are CREATE/ALTER TABLE commands for that table.
	 * @throws Exception A query that is neither CREATE TABLE nor ALTER TABLE has been supplied in $queries.
	 */
	private static function get_dbdelta_queries( $queries ) {
		global $wpdb;

		// phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		...
		$cqueries = array(); // CREATE TABLE queries.
		$aqueries = array(); // ALTER TABLE queries.
		...
		foreach ( $queries as $qry ) {
			...
			} elseif ( preg_match( '|^ALTER TABLE ([^ ]*)|', $qry, $matches ) ) {
				$aqueries[ trim( $matches[1], '`' ) ][] = $qry;
			} elseif ( ! empty( $qry ) ) {
				throw new Exception( 'WC_Install::get_dbdelta_queries: invalid query, only CREATE TABLE and ALTER TABLE are allowed. Query stats with ' . str_replace( "\n", ' ', ( substr( $qry, 30 ) ) ) );
			}
		}
		...
		foreach ( $cqueries as $table => $qry ) {
			...
			foreach ( $tablefields as $tablefield ) {
				...
				if ( array_key_exists( $tablefield_field_lowercased, $cfields ) ) {
					...
					if ( $tablefield->Type !== $fieldtype ) {
						...
						if ( $do_change ) {
							...
							$aqueries[ $table ][] = "ALTER TABLE {$table} CHANGE COLUMN `{$tablefield->Field}` " . $cfields[ $tablefield_field_lowercased ];
						}
					}
					...
					if ( preg_match( "| DEFAULT '(.*?)'|i", $cfields[ $tablefield_field_lowercased ], $matches ) ) {
						...
						if ( $tablefield->Default !== $default_value ) {
							...
							$aqueries[ $table ][] = "ALTER TABLE {$table} ALTER COLUMN `{$tablefield->Field}` SET DEFAULT '{$default_value}'";
						}
					}
				...
				// else - This field exists in the table, but not in the creation queries?
			}

			// For every remaining field specified for the table.
			foreach ( $cfields as $fieldname => $fielddef ) {
				// Push a query line into $cqueries that adds the field to that table.
				$aqueries[ $table ][] = "ALTER TABLE {$table} ADD COLUMN $fielddef";
			}
			...
			if ( $tableindices ) {
				...
				foreach ( $tableindices as $tableindex ) {
					...
					$index_ary[ $keyname ]['unique']     = ( 0 === intval( $tableindex->Non_unique ) ) ? true : false;
			...
			foreach ( (array) $indices as $index ) {
				...
				$aqueries[ $table ][] = "ALTER TABLE {$table} ADD $index";
			}
