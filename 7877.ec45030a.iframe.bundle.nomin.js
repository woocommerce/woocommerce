 ...
 */
function isSameNames(names, otherNames) {
  return names.length === otherNames.length && names.every((name, index) => name === otherNames[index]);
}
...
function ensureMomentWeekdayNames() {
  const localeData = moment__WEBPACK_IMPORTED_MODULE_0___default().localeData();
  ...
  const englishLocaleData = moment__WEBPACK_IMPORTED_MODULE_0___default().localeData('en');
