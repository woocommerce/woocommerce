...
const run =    
	...
	const prNumber = parseInt(prNumberMatch[0]);
...
`);
	...
	if(prData['state'] !== 'OPEN') {
		error(`Error: this pull request isn't open (state: ${ prData['state'] })`);
		exit(1);
	}
	...
	var changelogFileData = await gql(`
query {
	repository(owner: "${ options.owner }", name: "${ options.name }") {
	...
	if(changelogFileData['repository']['object'] !== null) {
		...
		const existingFileContents: string = changelogFileData['repository']['object']['text'].trim();
		Logger.warn(`File 'plugins/woocommerce/changelog/pr-${ prNumber }' already exists with this content:\n`);
		console.log('-------\n' + existingFileContents + '\n-------\n');

		if(!options.createModifyCommit) {
			if(existingFileContents === changelogFileContents.trim()) {
				Logger.warn('The provided file contents is identical to the existing file contents, nothing to be done.');
			}
			console.log('Run with -c to create a commit that modifies the file.\n');
			exit(0);
		}
	}
...
mutation ($input: CreateCommitOnBranchInput!) {
	...
	const mutationData = {
		'input': {
			...
			'message': {
				'headline': fileAlreadyExists ? 'Modify changelog file' : 'Add changelog file'
			},
			'fileChanges': {
				'additions': [
					{
	...
	if(options.dryRun) {
		console.log('Dry run, this is what would be sent to the GitHub API:\n');
		console.log(JSON.stringify({'query': mutationCommand.trim().replace(/[\n\t]+/g, ' '), 'variables': mutationData}, null, 2) + '\n');
		console.log('Changelog file contents (encoded as Base64 in "contents"):\n\n-------\n' + changelogFileContents.trim() + '\n-------\n');
		exit(0);
	}
...
export const addChangelogFileCommand = new Command('add-changelog-file')
	...
	.addArgument(new Argument('<significance>', 'Significance, first letter is enough ("j" for "major")s').choices(significanceValues).argParser(x => significanceValues.includes(x) ? x : significances[x] ?? null ))
	.addArgument(new Argument('<type>', 'Type, first letter is enoughs').choices(types).argParser(x => types.includes(x) ? x : types.find(t => t[0] === x[0]) ?? null ))
	.argument( '<message words...>', 'Changelog file message, enclosing in quotes is not needed.')
	.option(
		'-o --owner <owner>',
		'Repository owner.',
		'woocommerce'
