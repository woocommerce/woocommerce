...
**Canonical example:** `Filterer::get_product_counts_query_using_lookup_table` in `src/Internal/ProductAttributesLookup/Filterer.php`. Its ids come from the lookup table itself, not from an upstream `WP_Query`, so the `wp_posts` join is what enforces `post_type` and `post_status`.
