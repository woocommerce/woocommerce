...
if [ "$GITHUB_EVENT_NAME" == "push" ] || [ "$GITHUB_EVENT_NAME" == "pull_request" ]; then
	...
	git reset --hard && git -c core.hooksPath=/dev/null checkout $HEAD_BRANCH> /dev/null && echo 'On ' $(git rev-parse HEAD)
	...
	git reset --hard && git -c core.hooksPath=/dev/null checkout $BASE_SHA> /dev/null && echo 'On ' $(git rev-parse HEAD)
	pnpm run --if-present clean:build
	# pnpm install --filter='@woocommerce/plugin-woocommerce...' --frozen-lockfile --config.dedupe-peer-dependents=false
	# pnpm --filter='@woocommerce/plugin-woocommerce' build
