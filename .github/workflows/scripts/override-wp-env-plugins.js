...
if ( wpEnvConfig.env?.tests?.plugins ) {
	...
	// development and tests instances, but the e2e suites only run against tests.
	// Mapping here avoids extracting the artifact into the dev instance that is
	// never exercised. Move this to a root-level `mappings` if WooCommerce is ever
	// needed in the dev environment too.
	overrideConfig.env = {
		tests: {
...
console.log(
	`Removed ${ removed } WooCommerce source entr${
		removed === 1 ? 'y' : 'ies'
	}; mapping ${ pluginSource } -> wp-content/plugins/woocommerce`
);
