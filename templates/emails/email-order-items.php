...
foreach ($items as $item) : 
	...
	<tr>
		<td style="text-align:left; vertical-align:middle; border: 1px solid #eee;"><?php 
			...
			// File URL
			echo 	($show_download_links && $_product->exists && $_product->is_downloadable()) ? '<br/><small>'.__('Download:', 'woocommerce').' <a href="' . $order->get_downloadable_file_url( $item['id'], $item['variation_id'] ) . '">' . $order->get_downloadable_file_url( $item['id'], $item['variation_id'] ) . '</a></small>' : '';
