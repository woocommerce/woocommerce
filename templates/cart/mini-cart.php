		...
		<?php
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $product && $product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					$product_price     = apply_filters( 'woocommerce_cart_item_price', wc_cart_product_price_html( $product ), $cart_item, $cart_item_key );
...
<?php if ( ! WC()->cart->is_empty() ) : ?>
	...
	<p class="total"><strong><?php _e( 'Subtotal', 'woocommerce' ); ?>:</strong> <?php wc_cart_subtotal_html(); ?></p>
