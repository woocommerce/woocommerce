...
- JavaScript/TypeScript block unit tests live in `<block-name>/tests/`, for example `<block-name>/tests/block.ts`.
- PHP unit tests for block-specific server rendering live in `<block-name>/tests/`.
