...
class woocommerce_cart {
    ...
    function init() {
        ...
        if (isset($_SESSION['coupons']))
            $this->applied_coupons = $_SESSION['coupons'];
        ...
        if (isset($_SESSION['cart']) && is_array($_SESSION['cart'])) :
            ...
            foreach ($cart as $key => $values) :
                ...
                if ($_product->exists && $values['quantity'] > 0) :
                    ...
                    $cart_data_item = apply_filters('woocommerce_populate_cart_data_item', $cart_data_item, $values);

                    $this->cart_contents[$key] = $cart_data_item;
    ...
    function find_product_in_cart($product_id, $variation = '', $variation_id='', $cart_id=false) {
        if ($cart_id !== false) {
     ...
     */
    function add_to_cart($product_id, $quantity = 1, $variation = '', $variation_id = '', $cart_id=false) {
        global $woocommerce;

        if (!$cart_id) {
            $cart_id = $this->generate_cart_id($product_id, $variation, $variation_id);
        }

        $found_cart_item_key = $this->find_product_in_cart($product_id, $variation, $variation_id, $cart_id);
        ...
        if ($found_cart_item_key) :
            ...
            $cdi = apply_filters('woocomerce_create_cart_data_item', $cart_data_item);
            if (!$cdi) {
                $cdi = $cart_data_item;
            }

            $this->cart_contents[$cart_id] = $cdi;
    ...
    function get_remove_url($cart_item_key) {
        ...
        $cart_page_id = get_option('woocommerce_cart_page_id');
        if ($cart_page_id)
            return $woocommerce->nonce_url('cart', add_query_arg('remove_item', $cart_item_key, get_permalink($cart_page_id)));
    }
    ...
    function needs_shipping() {
        ...
        if (!$woocommerce->shipping->enabled)
            return false;
        if (!is_array($this->cart_contents))
            return false;
    ...
    function calculate_totals() {
        ...
        if ($this->needs_shipping())
            $woocommerce->shipping->calculate_shipping(); else
    ...
    function get_cart_tax() {
        ...
        if ($cart_total_tax > 0)
            return woocommerce_price($cart_total_tax);
        return false;
    }
    ...
    function get_total_discount() {
        if ($this->discount_total)
            return woocommerce_price($this->discount_total); else
            return false;
    }
